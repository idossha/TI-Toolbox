...
/** One queued Ex/mEx run: the subject it belongs to, the target it came from, and its wire config. */
export interface ExSubmission {
  subject: string;
  target: ExTarget;
  config: ExConfigBody | MExConfigBody;
}

/**
 * Expand the page's state into one submission per (subject × target) — the U16 multi-subject run.
 *
 * Both per-subject facts are resolved by the caller through `resolve`, because both are genuinely
 * per subject and getting either from the primary subject would submit one subject's data under
 * another's name: the leadfield HDF5 lives under `derivatives/SimNIBS/sub-<id>/leadfields/`, and a
 * subcortical target's `atlas_path` points into `derivatives/freesurfer/sub-<id>/mri/`. A subject
 * with no leadfield for the selected net contributes nothing (the page blocks Run and names it);
 * it is never submitted with somebody else's matrix.
 */
export function exSubmissions(
  method: "ex" | "mex",
  subjects: string[],
  resolve: (subject: string) => { leadfieldHdf: string | null; targets: ExTarget[] },
  forms: { ex: ExFormState; mex: MExFormState },
  runName: string,
): ExSubmission[] {
  const out: ExSubmission[] = [];
  for (const subject of subjects) {
    const { leadfieldHdf, targets } = resolve(subject);
    if (!leadfieldHdf) continue;
    for (const target of targets) {
      out.push({
        ...
        config:
          method === "ex"
            ? buildExConfig(subject, leadfieldHdf, forms.ex, target, runName)
            : buildMExConfig(subject, leadfieldHdf, forms.mex, target, runName),
      });
    }
  }
  return out;
