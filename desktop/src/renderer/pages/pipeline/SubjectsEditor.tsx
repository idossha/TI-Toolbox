 ...
 * will make a wire to the Simulator refuse.
 *
 * The presence dots are all this says. It deliberately does **not** spell out what each subject is
 * "ready for": that sentence already exists twice — as the refusal on the drag, and in the receipt
 * — and a third copy here restated a conclusion the three dots beside it already support, in the
 * widest column of a four-column table.
 */
...
import { type Capability, type Readiness } from "./graph";
  ...
  const items: SelectionItem[] = useMemo(
    () =>
      rows.map((row) => ({
        ...
        label: row.id,
        value: row,
        // Every subject may be chosen; what varies is what the cohort can then be wired to, and
        // that is answered where it is acted on — the refusal on the drag, and the receipt.
        // `data-caps` carries the same facts for a spec without printing a fourth column.
        data: { caps: (readiness[row.id] ?? []).join(",") },
