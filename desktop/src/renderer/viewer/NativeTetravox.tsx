...
export function NativeTetravox({ path, compact = false }: { path?: string | null; compact?: boolean }) {
  ...
  return <div style={{ padding: compact ? 0 : "var(--space-4)", display: "grid", gap: "var(--space-3)" }} data-testid="native-tetravox">
    ...
    {(!compact || !status.data?.installed || status.data.source === "system") && <p className="field-help">{status.data?.installed ? `${status.data.source === "system" ? "Using your existing installation" : "Installed for TI-Toolbox"}${status.data.version ? ` · ${status.data.version}` : ""}` : status.isPending ? "Checking installation…" : "TetraVox is not installed."}</p>}
    ...
    {status.data?.installed && <Button onClick={() => open.mutate()} disabled={open.isPending}>{path ? "Open scene in TetraVox" : "Launch TetraVox"}</Button>}
