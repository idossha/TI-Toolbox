...
const SOURCE_LABEL: Record<NonNullable<TitNativeTetravoxStatus["source"]>, string> = {
  configured: "Using the application you chose",
  managed: "Installed for TI-Toolbox",
  system: "Using your existing installation",
  path: "Using the TetraVox on your PATH",
};
...
function describeProgress(progress: TitNativeTetravoxProgress): string | undefined {
...
export function NativeTetravox({ path, compact = false }: { path?: string | null; compact?: boolean }) {
  ...
  const busy = install.isPending || update.isPending || data?.installing;
  ...
  return <div style={{ padding: compact ? 0 : "var(--space-4)", display: "grid", gap: "var(--space-3)" }} data-testid="native-tetravox">
    ...
    {(!compact || !data?.installed || data.source !== "managed") && <p className="field-help">{describe(data, status.isPending)}</p>}
    ...
    {!compact && data?.source === "managed" && !data.updateAvailable && <Button variant="secondary" disabled={check.isPending || busy} onClick={() => check.mutate()}>{check.isPending ? "Checking…" : check.isSuccess ? "Up to date" : "Check for updates"}</Button>}
    ...
    {!compact && <p className="field-help">Use TetraVox to adjust the view and save camera or appearance changes.</p>}
  </div>;
}
