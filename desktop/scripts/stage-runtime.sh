...
# that already-wrong string. Reproduced once during this spike (docs/dev/DECISIONS.md § 2026-09-03 (One Docker image and a real development loop)):
...
# N1 (docs/dev/DECISIONS.md § 2026-09-03 (Native desktop research) §2) — a real pipeline stages its freshly-built runtime tree
...
if [ -z "${TIT_RUNTIME_DIR:-}" ]; then
  ...
  echo "  runtime tree, e.g. the output of docs/dev/DECISIONS.md § 2026-09-03 (One Docker image and a real development loop))" >&2
  exit 1
fi
