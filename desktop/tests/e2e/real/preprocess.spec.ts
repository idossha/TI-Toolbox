 ...
 *    was m2m/BIDS-only). Lane FX5 (`docs/dev/DECISIONS.md § 2026-09-03 (One Docker image and a real development loop)` §7) made
...
test("tissue analysis on sub-101: accepted, started, and completed with a real artifact", async () => {
  ...
  // `report` job every `pre` group plans (F0's known-broken kind — docs/dev/DECISIONS.md § 2026-09-03 (One Docker image and a real development loop)
