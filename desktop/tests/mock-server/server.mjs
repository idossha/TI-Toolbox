...
route("POST", "/api/view/export", async (ctx) => {
  const body = await ctx.body();
  if (!body.scene || !Array.isArray(body.scene.datasets)) return json(ctx.res, 422, {detail:"A scene is required"});
  const name = String(body.name ?? "scene").replace(/[^a-zA-Z0-9_-]/g, "-");
  const scene_path = `${PROJECT_ROOT}/code/ti-toolbox/viewer/${name}.tetravox.json`;
  const scene = structuredClone(body.scene);
  for (const dataset of scene.datasets) {
    if (typeof dataset.path === "string") dataset.path = dataset.path.replace(/^\/api\/files\/raw/, "").replace(PROJECT_ROOT, MOCK_HOST_ROOT);
  }
  json(ctx.res, 200, {scene_path, path:scene_path, host_path:scene_path.replace(PROJECT_ROOT, MOCK_HOST_ROOT), scene});
});
