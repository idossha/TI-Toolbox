...
# Configuration setup
ti_csc_dir="$project_dir/code/tit"
...
# Ensure tit directory exists and set permissions
if [ ! -d "$ti_csc_dir" ]; then
    mkdir -p "$ti_csc_dir"
    chmod 777 "$ti_csc_dir"
    echo -e "${GREEN}Created tit directory at $ti_csc_dir with permissions 777.${RESET}"
else
    chmod 777 "$ti_csc_dir"
fi
...
collect_output_settings() {
    ...
    output_dir="$project_dir/code/tit/group_analyses/group_${analysis_type}_${space_type}_${timestamp}"
