...
class PermutationCLI(BaseCLI):
    def __init__(self) -> None:
        ...
        self.add_argument(
            ArgumentDefinition(
                name="csv", type=str, help="Path to CSV file", required=True
            )
        )
        self.add_argument(
            ArgumentDefinition(
                name="name", type=str, help="Analysis name", required=True
            )
        )
        self.add_argument(
            ArgumentDefinition(
                ...
                help="group_comparison|correlation",
                required=True,
            )
        )
        self.add_argument(
        ...
        self.add_argument(
            ArgumentDefinition(
                name="alpha", type=float, help="Cluster-level alpha", default=0.05
            )
        )
        self.add_argument(
            ArgumentDefinition(
                name="n_jobs", type=int, help="Parallel jobs (-1=all)", default=-1
            )
        )
        self.add_argument(
            ArgumentDefinition(
                name="use_weights",
                type=bool,
                help="Use weights (correlation only)",
                default=False,
            )
        )
        self.add_argument(
        ...
        self.add_argument(
            ArgumentDefinition(
                name="nifti_pattern",
                type=str,
                help="Custom nifti pattern",
                required=False,
            )
        )
        self.add_argument(
            ArgumentDefinition(name="quiet", type=bool, help="Quiet", default=False)
        )
        self.add_argument(
            ArgumentDefinition(
                name="save_perm_log",
                type=bool,
                help="Save permutation log",
                default=False,
            )
        )
    ...
    def run_interactive(self) -> int:
        ...
        csv_path = utils.choose_or_enter(
            prompt="CSV path",
            options=csv_candidates,
            help_text="Select a CSV file or choose 'Enter manually…'",
        )
        name = utils.ask_required(
            "Analysis name", default=f"perm_{utils.now_compact()}"
        )
        ...
        if not utils.review_and_confirm(
            ...
            return 0
        return self.execute(
            {"csv": csv_path, "name": name, "analysis_type": analysis_type}
        )
