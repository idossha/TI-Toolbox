...
class AnalyzerCLI(BaseCLI):
    def __init__(self) -> None:
        ...
        self.add_argument(
            ArgumentDefinition(
                name="output_dir", type=str, help="Output directory", required=False
            )
        )
        self.add_argument(
            ArgumentDefinition(
        ...
        self.add_argument(
            ArgumentDefinition(
                name="coordinates", type=float, nargs=3, help="x y z", required=False
            )
        )
        self.add_argument(
            ArgumentDefinition(
        ...
        self.add_argument(
            ArgumentDefinition(
                name="whole_head",
                type=bool,
                help="Whole head (cortical)",
                default=False,
            )
        )
        self.add_argument(
            ArgumentDefinition(
                name="region", type=str, help="Region name (cortical)", required=False
            )
        )
    ...
    def run_interactive(self) -> int:
        ...
        try:
            field_path, _ = select_field_file(
                str(m2m_dir), simulation_name, space, analysis_type
            )
    ...
    def _default_output_dir(pm, cfg: Dict[str, Any]) -> Path:
        ...
        if not sim_dir:
            raise RuntimeError(
                f"Could not resolve simulation directory for sub-{cfg['subject']} / {cfg['simulation']}"
            )
