...
# ── subject info (the Subject Info Viewer panel) ─────────────────────────────


def _files_in(root: str, suffixes: tuple[str, ...] = ()) -> list[dict]:
    """Regular files directly under *root* as ``{name, size_bytes}`` entries.

    Sorted, non-recursive and permission-tolerant: this feeds a read-only
    panel, so an unreadable directory is an empty list rather than a 500.
    """
    out: list[dict] = []
    try:
    ...
    for entry in entries:
        ...
        try:
            ...
            size = 0
        out.append({"name": entry.name, "size_bytes": int(size)})
    return out


def _subdir_names(root: str) -> list[str]:
    try:
        return sorted(
            e.name
...
#: BIDS anatomical suffixes the panel reports one by one, because "does this
#: subject have a T2?" is the question that decides whether ``charm`` was run
#: with one -- and ``has_raw`` alone cannot answer it.
_ANAT_MODALITIES = (("T1w", "_T1w"), ("T2w", "_T2w"), ("CT", "_ct"))


def subject_info(pm: PathManager, sid: str) -> dict | None:
    """Everything the Subject Info Viewer shows for one subject.

    The v3 replacement for ``tit/gui/extensions/subject_info_viewer.py``'s
    ``get_detailed_subject_info()``, which walked the project directory in
    the GUI process. Same facts, one HTTP call, and every stage named the way
    the rest of the v1 catalog names it (``has_m2m``, not ``SimNIBS``).

    ``None`` when *sid* is not a subject of this project -- the route turns
    that into a 404 naming the id.
    """
    if sid not in subject_ids(pm) and sid not in sourcedata_only_subject_ids(pm):
        return None

    detail = subject_detail(pm, sid) or {"id": sid}
    m2m_dir = pm.m2m(sid)
    has_m2m = os.path.isdir(m2m_dir)
    simnibs_subject = os.path.dirname(m2m_dir)

    anat_dir = pm.bids_anat(sid)
    anat_files = _files_in(anat_dir, (".nii", ".nii.gz", ".json"))
    modalities = [
        name
        for name, token in _ANAT_MODALITIES
        if any(token in f["name"] and f["name"].endswith((".nii", ".nii.gz")) for f in anat_files)
    ]

    simulations: list[dict] = []
    analysis_total = 0
    for name in pm.list_simulations(sid) if has_m2m else []:
        runs = [entry["name"] for entry in (analyses(pm, sid, name) or [])]
        analysis_total += len(runs)
        sim_dir = pm.simulation(sid, name)
        simulations.append(
            {
                "name": name,
                "has_ti": os.path.isdir(os.path.dirname(pm.ti_mesh_dir(sid, name))),
                "has_mti": os.path.isdir(os.path.dirname(pm.mti_mesh_dir(sid, name))),
                "n_meshes": len(_files_in(pm.ti_mesh_dir(sid, name), (".msh",)))
                + len(_files_in(pm.mti_mesh_dir(sid, name), (".msh",))),
                "analyses": runs,
                "created": _mtime_iso(sim_dir),
            }
        )

    flex = pm.list_flex_search_runs(sid) if has_m2m else []
    ex = [r["run_name"] for r in (ex_runs(pm, sid, "ex") or [])]
    mex = [r["run_name"] for r in (ex_runs(pm, sid, "mex") or [])]
    freehand = [c["name"] for c in (freehand_configs(pm, sid) or [])]
    report_ids = [r["id"] for r in (reports(pm, sid) or [])]

    return {
        **detail,
        # ---- raw / anatomy -------------------------------------------------
        "sourcedata_files": _files_in(pm.sourcedata_subject(sid)),
        "anat_files": anat_files,
        "anat_modalities": modalities,
        "dwi_files": _files_in(pm.bids_dwi(sid), (".nii", ".nii.gz", ".bval", ".bvec")),
        # ---- head model ----------------------------------------------------
        "m2m_dirs": [
            name
            for name in _subdir_names(simnibs_subject)
            if name.startswith(f"m2m_{sid}")
        ],
        # ---- derivatives ---------------------------------------------------
        "simulations": simulations,
        "n_analyses": analysis_total,
        "flex_search": flex,
        "ex_search": ex,
        "mex_search": mex,
        "freehand_configs": freehand,
        "reports": report_ids,
    }
