...
def generate_surface_mesh(field_mesh_path, subject_dir):
    ...
    cmd = ["msh2cortex", "-i", field_mesh_path, "-m", subject_dir, "-o", mesh_dir]
...
def process_region_to_stl(
    surface_mesh, atlas, region_name, field_name, output_dir, atlas_type, temp_dir
):
    """
    ...
    try:
        ...
        vertices, faces = extract_roi_region_no_zeros(
            roi_mesh, roi_field_values, return_field_values=False
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Convert cortical regions from atlas to STL files"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--output-dir", required=True, help="Output directory for STL files"
    )
    ...
    parser.add_argument(
        "--field", default="TI_max", help="Field name to preserve (default: TI_max)"
    )
    parser.add_argument(
        "--skip-regions",
        action="store_true",
        help="Do not export individual region STLs",
    )
    parser.add_argument(
        "--skip-whole-gm", action="store_true", help="Do not export the whole GM STL"
    )
    parser.add_argument(
        ...
        try:
            ...
            if not args.skip_whole_gm:
                try:
                    ...
                    vertex_mapping = {
                        old: new for new, old in enumerate(unique_vertices)
                    }
                    remapped_triangles = np.array(
                        [[vertex_mapping[idx] for idx in tri] for tri in triangle_nodes]
                    )
                    ...
                    write_binary_stl(
                        ...
                        remapped_triangles,
                        whole_stl_path,
                        "Generated from SimNIBS ROI mesh",
                    )
                except Exception:
                    pass
