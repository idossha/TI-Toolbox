...
class ElectrodePlacementConfig:
    ...
    def validate(self) -> None:
        ...
        if not os.path.exists(self.electrode_csv_path):
            raise FileNotFoundError(
                f"Electrode CSV not found: {self.electrode_csv_path}"
            )

        if not os.path.exists(self.electrode_blend_path):
            raise FileNotFoundError(
                f"Electrode template not found: {self.electrode_blend_path}"
            )
        ...
        if self.electrode_diameter_mm <= 0:
            raise ValueError(
                f"electrode_diameter_mm must be > 0, got {self.electrode_diameter_mm}"
            )
        if self.electrode_height_mm <= 0:
            raise ValueError(
                f"electrode_height_mm must be > 0, got {self.electrode_height_mm}"
            )
        ...
        if self.offset_distance < 0:
            raise ValueError(
                f"offset_distance must be >= 0, got {self.offset_distance}"
            )
    ...
    def output_blend_path(self) -> str:
        ...
        return os.path.join(
            self.output_dir, f"{self.subject_id}_electrodes_{csv_name}.blend"
        )
    ...
    def output_glb_path(self) -> str:
        ...
        csv_name = os.path.splitext(os.path.basename(self.electrode_csv_path))[0]
        return os.path.join(
            self.output_dir, f"{self.subject_id}_electrodes_{csv_name}.glb"
        )
...
class ElectrodePlacer:
    ...
    def _load_templates(self):
        ...
        with bpy.data.libraries.load(self.config.electrode_blend_path, link=False) as (
            src,
            dst,
        ):
    ...
    def _calculate_orientation(self, surface_normal):
        ...
        up_proj = (
            world_up - surface_normal * world_up.dot(surface_normal)
        ).normalized()
    ...
    def place_electrodes(self) -> Tuple[bool, str]:
        ...
        self.logger.info(
            f"Starting electrode placement for subject {self.config.subject_id}"
        )
        ...
        try:
            ...
            if not self.config.show_full_net:
                ...
                if not keep_labels:
                    self.logger.warning(
                        "show_full_net=False but no montage_pairs provided; placing full net."
                    )
                    keep_labels = None
