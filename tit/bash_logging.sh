...
import os
import json
from tit.logger import get_logger, configure_external_loggers
...
log_preprocessing_start() {
    ...
    # Always output summary message for GUI (GUI will filter based on its debug mode)
    echo "Beginning pre-processing for subject: $subject_id"
    
    # Log to file (respects SUMMARY_ENABLED for file verbosity)
...
execute_with_summary() {
    ...
    # Run command with appropriate output handling based on debug mode
    if [ "$debug_mode" = "true" ]; then
        # Debug mode: show output in real-time AND capture for error checking
        ...
        # Summary mode: only capture to temp file (no console output)
