...
def run_flex_optimization(subject_id, project_dir, n_multistart, logger, **params):
    ...
    try:
        ...
        cmd = [
            ...
            params["eeg_net"],
            "--radius",
            str(params["electrode_radius"]),
            "--current",
            str(params["electrode_current"]),
            "--n-multistart",
            str(n_multistart),
            "--max-iterations",
        ...
        logger.info(
            f"Running flex-search: multistart={n_multistart}, goal={params['opt_goal']}"
        )
...
def main():
    ...
    parser.add_argument(
        "--multistart", type=str, help="Comma-separated multi-start values"
    )
    ...
    try:
        # Setup project
        subject_dir, subject_id, resolved_project_dir = setup_project(
        ...
        opt_params = {
            ...
            "run_final_simulation": merged.get("run_final_simulation", False)
            or args.run_final_simulation,
        }
        ...
        for n_multistart in multistart_values:
            ...
            result_data = run_flex_optimization(
                subject_id, resolved_project_dir, n_multistart, logger, **opt_params
            )
            ...
            result_file = (
                output_dir
                / f"flex_benchmark_{subject_id}_ms{n_multistart}_{timestamp}.json"
            )
        ...
        summary_file = (
            output_dir
            / f"flex_benchmark_{subject_id}_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
