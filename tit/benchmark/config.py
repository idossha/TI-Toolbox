...
class BenchmarkConfig:
    ...
    def _apply_defaults(self):
        ...
        if "project_dir" not in dicom:
            dicom["project_dir"] = str(
                Path("/mnt/tit-benchmark-dicom")
                if os.path.exists("/mnt")
        ...
        if "project_dir" not in flex:
            flex["project_dir"] = str(
                Path("/mnt/tit-benchmark-flex")
                if os.path.exists("/mnt")
...
def merge_config_with_args(
    config: BenchmarkConfig, args, benchmark_type: str
) -> Dict[str, Any]:
    """
...
""",
    ...
    parser.add_argument(
        "--generate", action="store_true", help="Generate an example configuration file"
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    parser.add_argument(
        "--config", type=Path, help="Path to configuration file to show"
    )
