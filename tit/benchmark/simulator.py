...
def select_electrodes(m2m_dir: Path, eeg_net: str, sim_mode: str, logger):
    ...
    try:
        ...
        if len(electrodes) < num_electrodes:
            logger.warning(
                f"Only found {len(electrodes)} electrodes, needed {num_electrodes}"
            )

        electrode_list = ",".join(electrodes[:num_electrodes])
        logger.info(
            f"Selected {num_electrodes} electrodes for mode '{sim_mode}': {electrode_list}"
        )
...
def create_montage_config(
    ...
    if sim_mode == "U":
        ...
        logger.info(
            f"Created unipolar montage '{montage_name}' with {len(pairs)} pairs"
        )
    else:  # M mode
        montage_data["nets"][eeg_net]["multi_polar_montages"][montage_name] = pairs
        logger.info(
            f"Created multipolar montage '{montage_name}' with {len(pairs)} pairs"
        )
...
def run_simulator(
    ...
    create_montage_config(
        project_dir, montage, electrode_list, eeg_net, sim_mode, logger
    )
    ...
    try:
        ...
        logger.info(
            f"Montage: {montage}, Mode: {sim_mode}, Conductivity: {conductivity}"
        )
...
def main():
    ...
    try:
        # Setup project
        subject_dir, simulation_dir = setup_project(
            project_dir, m2m_dir, subject_id, logger
        )
