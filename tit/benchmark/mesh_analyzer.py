...
Usage:
  python -m tit.benchmark.mesh_analyzer --config benchmark_config.yaml
  python -m tit.benchmark.mesh_analyzer --field-mesh /path/to/field.msh
"""
...
def run_mesh_analysis(
    ...
    if analysis_type == "sphere":
        metadata.update({"roi_center": roi_center, "roi_radius": roi_radius})
    elif analysis_type == "cortex":
        metadata.update({"atlas_type": atlas_type, "target_region": target_region})
    ...
    try:
        ...
        if analysis_type == "sphere":
            results = analyzer.analyze_sphere(
                center_coordinates=roi_center, radius=roi_radius, visualize=visualize
            )
        elif analysis_type == "cortex":
            results = analyzer.analyze_cortex(
                atlas_type=atlas_type, target_region=target_region, visualize=visualize
            )
        elif analysis_type == "whole_head":
            results = analyzer.analyze_whole_head(
                atlas_type=atlas_type, visualize=visualize
            )
        else:
        ...
        result.metadata["analysis_results"] = (
            str(results) if isinstance(results, dict) else "multiple_regions"
        )
...
def main():
    ...
    parser.add_argument(
        "--field-mesh", type=Path, help="Path to field mesh file (.msh)"
    )
    parser.add_argument(
        "--field-name", type=str, help="Field name to analyze (e.g., normE, TI_max)"
    )
    parser.add_argument(
        "--subject-dir", type=Path, help="Path to m2m subject directory"
    )
    parser.add_argument("--subject-id", type=str)
    parser.add_argument(
        "--analysis-type",
        type=str,
        choices=["sphere", "cortex", "whole_head"],
    ...
    parser.add_argument(
        "--atlas-type", type=str, help="Atlas type (e.g., DK40, HCP_MMP1)"
    )
    parser.add_argument(
        "--target-region", type=str, help="Target region name for cortical analysis"
    )
    parser.add_argument(
    ...
    try:
        ...
        result_file = (
            output_dir
            / f"mesh_analyzer_benchmark_{subject_id}_{analysis_type}_{timestamp}.json"
        )
        save_benchmark_result(result, result_file)

        latest_file = (
            output_dir
            / f"mesh_analyzer_benchmark_{subject_id}_{analysis_type}_latest.json"
        )
