...
Usage:
  python -m tit.benchmark.leadfield --config benchmark_config.yaml
  python -m tit.benchmark.leadfield --m2m-dir /path/to/m2m_101 --electrode-csv GSN-HydroCel-185.csv
"""
        ...
        benchmark_result.metadata.update(
            {
                ...
                "leadfield_shape": (
                    list(generator.lfm.shape) if generator.lfm is not None else None
                ),
            }
        )
...
def main():
    ...
    tissues = (
        [int(x) for x in tissues_config]
        if isinstance(tissues_config, list)
        else [int(x.strip()) for x in tissues_config.split(",")]
    )
    ...
    try:
        # Setup project
        subject_dir, subject_id, leadfields_dir = setup_project(
        ...
        result_file = (
            output_dir / f"leadfield_benchmark_{subject_id}_{net_name}_{timestamp}.json"
        )
        save_benchmark_result(result, result_file)

        latest_file = (
            output_dir / f"leadfield_benchmark_{subject_id}_{net_name}_latest.json"
        )
