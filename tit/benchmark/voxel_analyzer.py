...
def run_voxel_analysis(
    ...
    if analysis_type == "sphere":
        ...
        metadata.update({"atlas_file": str(atlas_file), "target_region": target_region})
    ...
    try:
        ...
        if analysis_type == "sphere":
            ...
            results = analyzer.analyze_whole_head(
                atlas_file=str(atlas_file), visualize=visualize
            )
        else:
...
def main():
    ...
    parser.add_argument(
        "--field-nifti", type=Path, help="Path to field NIfTI file (.nii.gz)"
    )
    ...
    parser.add_argument(
        "--atlas-file", type=Path, help="Path to atlas file (.nii.gz or .mgz)"
    )
    parser.add_argument(
    ...
    try:
        ...
        result_file = (
            output_dir
            / f"voxel_analyzer_benchmark_{subject_id}_{analysis_type}_{timestamp}.json"
        )
        save_benchmark_result(result, result_file)

        latest_file = (
            output_dir
            / f"voxel_analyzer_benchmark_{subject_id}_{analysis_type}_latest.json"
        )
