...
Usage:
  python -m tit.benchmark.tissue_analyzer --config benchmark_config.yaml
  python -m tit.benchmark.tissue_analyzer --nifti-path /path/to/Labeling.nii.gz
"""
...
def main():
    ...
    try:
        ...
        for tissue_type in tissue_types:
            ...
            result_file = (
                output_dir
                / f"tissue_analyzer_benchmark_{subject_id}_{tissue_type}_{timestamp}.json"
            )
        ...
        summary_file = (
            output_dir
            / f"tissue_analyzer_benchmark_{subject_id}_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
