...
Usage:
  ...
  python -m tit.benchmark.recon --config benchmark_config.yaml --parallel
  python -m tit.benchmark.recon --subjects 101,102,103 --parallel
"""
...
def setup_project(
    ...
    if not dataset_desc.exists():
        dataset_desc.write_text(
            '{"Name": "FreeSurfer Benchmark", "BIDSVersion": "1.6.0"}'
        )
...
def run_recon_subject(
    subject_dir: Path, recon_script: Path, logger, use_openmp=False, debug_mode=True
):
    ...
    try:
        logger.info(
            f"Running recon-all: subject={subject_id}, openmp={'enabled' if use_openmp else 'disabled'}"
        )
        run_recon_all(
...
def main():
    ...
    parser.add_argument(
        "--parallel",
        action="store_true",
        dest="parallel_override",
        help="Run multiple subjects in parallel (bash background jobs)",
    )
    parser.add_argument(
        "--use-openmp",
        action="store_true",
        dest="openmp_override",
        help="Use OpenMP threading for recon-all",
    )
    ...
    for subject_id in subject_ids:
        t1_path = (
            project_dir / f"sub-{subject_id}" / "anat" / f"sub-{subject_id}_T1w.nii.gz"
        )
    ...
    log_file = create_benchmark_log_file(
        "recon", output_dir, f"multi_{len(subject_ids)}"
    )
    ...
    try:
        ...
        for subject_id in subject_ids:
            ...
            t2_path = (
                ...
                / f"sub-{subject_id}"
                / "anat"
                / f"sub-{subject_id}_T2w.nii.gz"
            )
            subject_dir, _ = setup_project(
                project_dir,
                t1_path,
                t2_path if t2_path.exists() else None,
                subject_id,
                logger,
            )
        ...
        if run_parallel:
            ...
            for subject_dir in subject_dirs:
                ...
                result_data = run_recon_subject(
                    subject_dir, recon_script, logger, use_openmp, debug_mode
                )
        ...
        total_processing_time = sum(
            r["result"].duration_seconds for r in all_results if r["success"]
        )
        ...
        summary = {
            ...
            "average_time_per_subject": (
                total_processing_time / num_successful if num_successful > 0 else 0
            ),
            "parallel_efficiency": (
                (total_processing_time / overall_result.duration_seconds)
                if run_parallel and overall_result.duration_seconds > 0
                else 1.0
            ),
            "results": [
                {
        ...
        logger.info(
            f"Average time per subject: {summary['average_time_per_subject']:.2f}s"
        )
        logger.separator("=", 70)
        for r in all_results:
            status = "SUCCESS" if r["success"] else "FAILED"
            logger.info(
                f"  {r['subject_id']}: {r['result'].duration_formatted} - {status}"
            )
