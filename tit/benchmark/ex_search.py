...
def create_roi_files(m2m_dir: Path, roi_center: tuple, roi_radius: float, logger):
    ...
    x, y, z = (
        int(round(roi_center[0])),
        int(round(roi_center[1])),
        int(round(roi_center[2])),
    )
...
def select_electrodes(electrode_cap_path: Path, n_per_channel: int, logger):
    ...
    if len(electrodes) < total_needed:
        logger.warning(
            f"Only {len(electrodes)} electrodes available, need {total_needed}"
        )
    ...
    logger.info(
        f"Selected {n_per_channel} electrodes per channel from {electrode_cap_path.name}"
    )
...
def run_ti_simulation(
    ...
    logger.info(
        f"Running TI simulation: {n_electrodes} electrodes/channel, {total_current}mA, {step_size}mA step{limit_str}"
    )
...
def verify_results(
    subject_id, project_dir, roi_dir, roi_center, leadfield_path, logger
):
    ...
    result = subprocess.run(
        ...
        text=True,
        env=env,
        cwd=str(roi_dir.parent.parent.parent),
    )
...
def run_single_benchmark(
    ...
    limit_str = (
        f", Channel limit: {channel_limit}mA" if channel_limit is not None else ""
    )
    logger.info(
        f"Electrodes/channel: {n_electrodes}, Current: {total_current}mA, Step: {step_size}mA{limit_str}"
    )
    ...
    try:
        ...
        effective_channel_limit = (
            channel_limit if channel_limit is not None else total_current / 2.0
        )
        ...
        iterations_per_second = (
            total_iterations / result.duration_seconds
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark ex-search optimization pipeline"
    )
    ...
    parser.add_argument(
        "--n-electrodes", type=str, help="Comma-separated electrode counts per channel"
    )
    ...
    parser.add_argument(
        "--channel-limit",
        type=float,
        help="Channel current limit in mA (default: total_current/2)",
    )
    ...
    total_current = (
        args.total_current
        if args.total_current is not None
        else merged.get("total_current", 1.0)
    )
    step_size = (
        args.step_size if args.step_size is not None else merged.get("step_size", 0.1)
    )
    channel_limit = (
        args.channel_limit
        if args.channel_limit is not None
        else merged.get("channel_limit", None)
    )

    # Parse ROI parameters
    roi_center_config = merged.get("roi_center", [0, 0, 0])
    roi_center = (
        tuple(float(x) for x in roi_center_config)
        if isinstance(roi_center_config, list)
        else tuple(float(x.strip()) for x in roi_center_config.split(","))
    )
    ...
    logger.info(
        f"Current parameters: {total_current}mA total, {step_size}mA step, {channel_limit}mA limit"
        if channel_limit
        else f"Current parameters: {total_current}mA total, {step_size}mA step"
    )

    try:
        # Setup project
        subject_dir, subject_id, expected_leadfield_path = setup_project(
            project_dir, m2m_dir, leadfield_path, logger
        )
        ...
        for n_electrodes in n_electrodes_list:
            ...
            result_file = (
                output_dir
                / f"ex_search_benchmark_{subject_id}_n{n_electrodes}_{timestamp}.json"
            )
            ...
            "results": [
                {
                    ...
                    "success": r.success,
                    "total_iterations": (
                        r.metadata.get("performance", {}).get("total_iterations")
                        if r.success
                        else None
                    ),
                    "iterations_per_second": (
                        r.metadata.get("performance", {}).get("iterations_per_second")
                        if r.success
                        else None
                    ),
                    "total_montages_tested": (
                        r.metadata.get("electrode_configuration", {}).get(
                            "total_montages_tested"
                        )
                        if r.success
                        else None
        ...
        summary_file = (
            output_dir
            / f"ex_search_benchmark_{subject_id}_summary_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        ...
        for n, r in zip(n_electrodes_list, all_results):
            ...
            if r.success:
                iterations = r.metadata.get("performance", {}).get(
                    "total_iterations", "N/A"
                )
                iter_per_sec = r.metadata.get("performance", {}).get(
                    "iterations_per_second", "N/A"
                )
                logger.info(f"n_electrodes={n}: {r.duration_formatted} - {status}")
                logger.info(
                    f"  Total iterations: {iterations} | Throughput: {iter_per_sec} iter/sec"
                )
            else:
                logger.info(f"n_electrodes={n}: {r.duration_formatted} - {status}")
