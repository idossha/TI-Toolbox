    ...
    if _group_start_time is not None:
        ...
        if SUMMARY_MODE:
            if num_successful == num_total:
                print(
                    f"└─ Group analysis completed ({num_successful}/{num_total} subjects successful, Total: {duration_str})"
                )
            else:
                print(
                    f"└─ Group analysis completed with failures ({num_successful}/{num_total} subjects successful, Total: {duration_str})"
                )
            if output_path:
...
try:
    ...
    try:
        ...
        print(
            "Warning: compare_analyses not available. Group comparison functionality will be limited."
        )
...
def setup_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Run analysis across multiple subjects and compare results"
    )
    ...
    parser.add_argument("--region", help="Region name for cortical analysis")
    parser.add_argument(
        "--whole_head",
        action="store_true",
...
def validate_args(args):
    ...
    if args.analysis_type == "spherical":
        ...
        if not args.coordinate_space:
            raise ValueError(
                "Coordinate space (--coordinate-space) is required for spherical analysis"
            )
    ...
    for subject_args in args.subject:
        ...
        if not os.path.isdir(m2m_path):
            raise ValueError(
                f"Subject {subject_id} m2m directory not found: {m2m_path}"
            )
        ...
        if args.space == "voxel" and args.analysis_type == "cortical":
            ...
            if not os.path.exists(atlas_path):
                raise ValueError(
                    f"Subject {subject_id} atlas file not found: {atlas_path}"
                )
...
def build_main_analyzer_command(
    args, subject_args: List[str], subject_output_dir: str
) -> List[str]:
    """
...
def run_comprehensive_group_analysis(
    analysis_paths: List[str], project_name: Optional[str] = None
) -> str:
    """
    ...
    try:
        ...
        if group_logger:
            group_logger.debug(
                f"✔ Comprehensive group analysis completed successfully."
            )
    ...
    if args.analysis_type == "spherical":
        ...
        if args.whole_head:
            if args.space == "mesh":
                ...
                atlas_path = (
                    first_subject_args[3]
                    if len(first_subject_args) > 3
                    else "unknown_atlas"
                )
                atlas_name = (
                    os.path.basename(atlas_path).split(".")[0]
                    if atlas_path != "unknown_atlas"
                    else "unknown_atlas"
                )
...
def main():
    ...
    try:
        ...
        if group_logger:
            ...
            group_logger.debug(
                f"    Analysis = {args.analysis_type} (space={args.space})"
            )
            group_logger.debug(
                f"    Centralized group results will go to: {centralized_group_dir}"
            )
        ...
        if len(successful_dirs) >= 1 and not args.no_compare:
            ...
            if analysis_dirs:
                final_group_dir = run_comprehensive_group_analysis(
                    analysis_dirs, project_name
                )
                ...
                if args.quiet:
                    print(
                        f"├─ Group comparison: ✓ Complete ({len(analysis_dirs)} subjects processed, {group_duration_str})"
                    )

                if group_logger:
                    group_logger.debug(
                        f"Group comparison completed. Output directory: {final_group_dir}"
                    )
                ...
                if group_logger:
                    group_logger.debug(
                        "No valid analysis directories found for group comparison."
                    )
        elif args.no_compare:
            if group_logger:
                group_logger.debug(
                    "Group comparison skipped (--no-compare flag specified)."
                )
        else:
            if group_logger:
        ...
        if group_logger:
            ...
            if len(successful_dirs) >= 1 and not args.no_compare:
                group_logger.debug(
                    f"Comprehensive group results are available in the centralized location."
                )
                group_logger.debug("")  # Empty line for spacing
            elif args.no_compare:
                group_logger.debug(
                    f"Group comparison was skipped. Individual subject results are in their respective directories.\n"
                )
            else:
