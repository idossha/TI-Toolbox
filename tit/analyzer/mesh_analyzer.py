...
class MeshAnalyzer:
    ...
    def __init__(
        ...
        if logger is not None:
            ...
            subject_id = (
                os.path.basename(self.subject_dir).split("_")[1]
                if "_" in os.path.basename(self.subject_dir)
                else os.path.basename(self.subject_dir)
            )
            ...
            self.logger = logging_util.get_logger(
                "mesh_analyzer", log_file, overwrite=True
            )
    ...
    def _generate_surface_mesh(self):
        ...
        try:
            if is_ex_search or is_flex_search:
                ...
                existing_central = [
                    f
                    for f in os.listdir(surface_mesh_dir)
                    if f.endswith("_central.msh")
                ]
                if existing_central:
                    # Reuse existing surface mesh
                    surface_mesh_path = os.path.join(
                        surface_mesh_dir, existing_central[0]
                    )
                    self.logger.debug(
                        f"Found existing surface mesh to reuse: {surface_mesh_path}"
                    )
                else:
                    # Generate new one with current field's name
                    surface_mesh_path = os.path.join(
                        surface_mesh_dir, f"{base_name}_central.msh"
                    )
            else:
            ...
            if os.path.exists(surface_mesh_path):
                self.logger.debug(
                    f"Using existing surface mesh at: {surface_mesh_path}"
                )
                self._surface_mesh_path = surface_mesh_path
                return surface_mesh_path

            self.logger.info(
                f"Generating surface mesh for specific field: {input_name} using msh2cortex..."
            )
            ...
            try:
                ...
                if not os.path.exists(surface_mesh_path):
                    self.logger.error(
                        f"Expected surface mesh file not found at: {surface_mesh_path}"
                    )
                ...
                self._surface_mesh_path = surface_mesh_path
                self.logger.debug(
                    f"Surface mesh generated successfully for {input_name} at: {surface_mesh_path}"
                )
                ...
                self.logger.error(
                    f"Command output: {e.stdout.decode() if e.stdout else ''}"
                )
                self.logger.error(
                    f"Command error: {e.stderr.decode() if e.stderr else ''}"
                )
                raise RuntimeError("Failed to generate surface mesh using msh2cortex")
            except FileNotFoundError as e:
                self.logger.error(f"Error: {str(e)}")
                self.logger.error(
                    "msh2cortex completed but did not generate the expected file."
                )
            ...
            self.logger.error(
                f"Could not determine simulation name from field mesh path. Expected path structure: .../sub-<n>/Simulations/simulation_name/(TI|mTI)/mesh/field.msh"
            )
            raise ValueError(
                "Could not determine simulation name from field mesh path. Expected path structure: .../sub-<n>/Simulations/simulation_name/(TI|mTI)/mesh/field.msh"
            )
    ...
    def _extract_normal_field_values(self, roi_mask):
        ...
        try:
            ...
            if "TI_normal" not in normal_mesh.field:
                ...
                self.logger.warning(
                    f"TI_normal field not found in normal mesh. Available fields: {available_fields}"
                )
                return None, None
            ...
            normal_mean_value = np.average(
                normal_field_values_positive, weights=positive_node_areas
            )
            ...
            whole_brain_normal_average = np.average(
                normal_field_values[whole_brain_positive_mask],
                weights=whole_brain_node_areas,
            )
                ...
                try:
                    ...
                    try:
                        ...
                        if callable(roi_mask):
                            self.logger.error(
                                f"Region {region_name}: atlas[region_name] returned a callable object, not a mask"
                            )
                            raise TypeError(
                                f"Region {region_name}: Expected mask array, got callable object"
                            )
                        elif not hasattr(roi_mask, "__len__"):
                            self.logger.error(
                                f"Region {region_name}: atlas[region_name] returned non-array object: {type(roi_mask)}"
                            )
                            raise TypeError(
                                f"Region {region_name}: Expected mask array, got {type(roi_mask)}"
                            )
                    except Exception as e:
                        self.logger.error(
                            f"Failed to get ROI mask for region {region_name}: {str(e)}"
                        )
                        raise
                    ...
                    if roi_nodes_count == 0:
                        self.logger.warning(
                            f"Warning: No nodes found in the specified region '{region_name}'"
                        )
                        region_results = {
                            "mean_value": None,
                            "max_value": None,
                    ...
                    try:
                        ...
                        if callable(field_values):
                            self.logger.error(
                                f"Region {region_name}: field values returned a callable object"
                            )
                            raise TypeError(
                                f"Region {region_name}: Expected field values array, got callable object"
                            )
                        field_values_in_roi = field_values[roi_mask]
                    except Exception as e:
                        self.logger.error(
                            f"Failed to get field values for region {region_name}: {str(e)}"
                        )
                        self.logger.error(
                            f"Field values type: {type(field_values) if 'field_values' in locals() else 'undefined'}"
                        )
                    ...
                    if positive_count == 0:
                        self.logger.warning(
                            f"Warning: Region {region_name} has no positive values"
                        )
                        region_results = {
                            "mean_value": None,
                            "max_value": None,
                    ...
                    try:
                        ...
                        if callable(node_areas):
                            self.logger.error(
                                f"Region {region_name}: nodes_areas() returned a callable object"
                            )
                            raise TypeError(
                                f"Region {region_name}: Expected node areas array, got callable object"
                            )
                        positive_node_areas = node_areas[roi_mask][positive_mask]
                    except Exception as e:
                        self.logger.error(
                            f"Failed to get node areas for region {region_name}: {str(e)}"
                        )
                        self.logger.error(
                            f"Node areas type: {type(node_areas) if 'node_areas' in locals() else 'undefined'}"
                        )
                        raise
                    ...
                    if whole_brain_average is not None:
                        self.logger.info(
                            f"Whole brain average (denominator for focality): {whole_brain_average:.6f}"
                        )
                    ...
                    self.logger.debug(
                        f"Extracting TI_normal values for region: {region_name}"
                    )
                    normal_field_values_positive, normal_statistics = (
                        self._extract_normal_field_values(roi_mask)
                    )
                    ...
                    if normal_statistics is not None:
                        ...
                        self.logger.debug(
                            f"TI_normal values successfully added for region: {region_name}"
                        )
                    else:
                        self.logger.warning(
                            f"TI_normal values not available for region: {region_name}"
                        )
                    ...
                    if visualize:
                        self.logger.info(
                            f"Generating 3D visualization for region: {region_name}"
                        )
                        ...
                        if len(field_values_positive) > 0:
                            try:
                                self.logger.info(
                                    f"Generating focality histogram for region: {region_name}"
                                )
                                ...
                                if hasattr(node_areas, "value"):
                                    node_areas = node_areas.value
                                node_areas = np.asarray(node_areas)
                                positive_node_areas = node_areas[roi_mask][
                                    positive_mask
                                ]
                                ...
                                region_visualizer = MeshVisualizer(
                                    region_dir, self.logger
                                )
                                ...
                                self.logger.warning(
                                    f"Could not generate focality histogram for {region_name}: {str(e)}"
                                )

                except Exception as e:
                    self.logger.warning(
                        f"Warning: Failed to analyze region {region_name}: {str(e)}"
                    )
                    results[region_name] = {
                        "mean_value": None,
                        "max_value": None,
    ...
    def analyze_sphere(
        self, center_coordinates, radius, visualize=False, save_results=True
    ):
        """
        ...
        """
        self.logger.info(
            f"Starting spherical ROI analysis (radius={radius}mm) at coordinates {center_coordinates}"
        )
        ...
        try:
            ...
            if self.field_name not in gm_surf.field:
                ...
                raise ValueError(
                    f"Field '{self.field_name}' not found in surface mesh. Available fields: {available_fields}"
                )
            ...
            self.logger.info(
                f"Creating spherical ROI at {center_coordinates} with radius {radius}mm..."
            )
            node_coords = gm_surf.nodes.node_coord
            roi_indices = ROICoordinateHelper.find_voxels_in_sphere(
                node_coords, center_coordinates, radius
            )
            ...
            if roi_nodes_count == 0:
                self.logger.error(
                    f"Analysis Failed: No nodes found in ROI at [{center_coordinates[0]}, {center_coordinates[1]}, {center_coordinates[2]}], r={radius}mm"
                )
                self.logger.error("ROI is not capturing any grey matter surface nodes")
                self.logger.error(
                    "Suggestion: Adjust coordinates/radius or verify using freeview"
                )
                return None
            ...
            if positive_count == 0:
                self.logger.warning(
                    f"Warning: No positive values found in spherical ROI"
                )
                return None

            self.logger.info(
                f"Found {positive_count} nodes with positive values in the ROI"
            )
            self.logger.info("Calculating statistics...")
            ...
            if normal_statistics is not None:
                ...
                self.logger.warning(
                    "TI_normal values not available - results will only contain TI_max values"
                )
            ...
            if visualize:
                if self.visualizer is not None:
                    ...
                    try:
                        self.logger.info(
                            "Generating focality histogram for spherical ROI..."
                        )
                        ...
                        self.logger.warning(
                            f"Could not generate focality histogram for spherical ROI: {str(e)}"
                        )
                    ...
                    self.logger.warning(
                        "Visualization requested but MeshVisualizer not available"
                    )
            ...
            if save_results and self.visualizer is not None:
                ...
                self.visualizer.save_results_to_csv(
                    results, "spherical", region_name, "node"
                )

                # Save extra info CSV with focality data
                if focality_info:
                    self.visualizer.save_extra_info_to_csv(
                        focality_info, "spherical", region_name, "node"
                    )
            elif save_results:
                self.logger.warning(
                    "Cannot save results to CSV - MeshVisualizer not available"
                )
    ...
    def analyze_cortex(self, atlas_type, target_region, visualize=False):
        ...
        """
        self.logger.info(
            f"Starting cortical analysis for region '{target_region}' using {atlas_type} atlas"
        )
        ...
        try:
            ...
            if target_region not in atlas:
                ...
                raise ValueError(
                    f"Region '{target_region}' not found in {atlas_type} atlas. Available regions: {available_regions}"
                )
            ...
            if roi_nodes_count == 0:
                self.logger.warning(
                    f"No nodes found in the specified region '{target_region}'"
                )
                results = {
                    "mean_value": None,
                    "max_value": None,
                ...
                self.visualizer.save_results_to_csv(
                    results, "cortical", target_region, "node"
                )
            ...
            if positive_count == 0:
                self.logger.warning(
                    f"Warning: Region {target_region} has no positive values"
                )
                results = {
                    "mean_value": None,
                    "max_value": None,
            ...
            if visualize:
                ...
                try:
                    self.logger.info(
                        f"Generating focality histogram for region: {target_region}"
                    )
                    # Get node areas for volume weighting
                    node_areas = gm_surf.nodes_areas()
                    if hasattr(node_areas, "value"):
                    ...
                    self.logger.warning(
                        f"Could not generate focality histogram for {target_region}: {str(e)}"
                    )
            ...
            if focality_info:
                self.visualizer.save_extra_info_to_csv(
                    focality_info, "cortical", target_region, "node"
                )
    ...
    def _calculate_focality_metrics(self, field_data, element_sizes, region_name):
        ...
        try:
            ...
            if len(field_data) == 0:
                self.logger.warning(
                    f"No field data available for focality calculation in {region_name}"
                )
                return None
            ...
            if len(data) == 0:
                self.logger.warning(
                    f"No valid field data after NaN removal for {region_name}"
                )
                return None
            ...
            for percentile in percentiles:
                ...
                threshold_idx = np.searchsorted(
                    normalized_cumulative, percentile / 100.0
                )
            ...
            results = {
                ...
                "percentile_95": (
                    percentile_values[0] if len(percentile_values) > 0 else 0.0
                ),
                "percentile_99": (
                    percentile_values[1] if len(percentile_values) > 1 else 0.0
                ),
                "percentile_99_9": (
                    percentile_values[2] if len(percentile_values) > 2 else 0.0
                ),
            ...
            self.logger.info(
                f"Focality metrics calculated for {region_name}: 99.9%={percentile_values[2]:.4f}, focality_95={focality_values[3]:.4f} cm²"
            )
            ...
            self.logger.error(
                f"Error calculating focality metrics for {region_name}: {str(e)}"
            )
            return None
    ...
    def get_grey_matter_statistics(self):
        ...
        """
        self.logger.info(
            "Calculating grey matter field statistics from GM surface (central layer)..."
        )
        ...
        try:
            ...
            if self.field_name not in gm_surf.field:
                available_fields = list(gm_surf.field.keys())
                raise ValueError(
                    f"Field '{self.field_name}' not found in GM surface. Available fields: {available_fields}"
                )
            ...
            self.logger.info(
                f"Grey matter statistics from GM surface for field '{self.field_name}' (positive values only): "
                f"mean={grey_mean:.6f}, max={grey_max:.6f}, min={grey_min:.6f}"
            )
            self.logger.info(
                f"Total nodes with positive values: {len(field_values_positive)}"
            )
            ...
            self.logger.info(
                "Falling back to original mesh for grey matter statistics..."
            )
            ...
            try:
                ...
                if self.field_name not in field_mesh.field:
                    ...
                    raise ValueError(
                        f"Field '{self.field_name}' not found in original mesh. Available fields: {available_fields}"
                    )
                ...
                self.logger.info(
                    f"Grey matter statistics from original mesh (fallback) for field '{self.field_name}': "
                    f"mean={grey_mean:.6f}, max={grey_max:.6f}, min={grey_min:.6f}"
                )
                ...
                self.logger.error(
                    f"Fallback to original mesh also failed: {str(fallback_error)}"
                )
                return {"grey_mean": 0.0, "grey_max": 0.0, "grey_min": 0.0}
