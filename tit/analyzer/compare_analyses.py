    ...
    try:
        ...
        if analysis_dir_name.startswith("sphere_"):
            ...
            if len(parts) >= 4:
                ...
                base_name = (
                    f"sphere_{'_'.join(coords)}_{radius}"
                    if coords and radius
                    else analysis_dir_name
                )
        ...
        montage_name = (
            montage_name.replace("/", "_").replace("\\", "_").replace(" ", "_")
        )
        ...
        if group_logger:
            group_logger.warning(
                f"Error extracting montage/region info from {first_path}: {e}"
            )
...
def _get_output_path(
    input_paths: List[str], method_name: str, filename: str, custom_suffix: str = ""
) -> str:
    """
    ...
    if not filename.startswith("group_"):
        ...
        if subject_count == 1:
            ...
            descriptive_name = (
                f"group_{method_name}_{montage_name}_{region_name}_{subject_count}subj"
            )
    ...
    final_filename = (
        final_filename.replace(" ", "_").replace("/", "_").replace("\\", "_")
    )
...
def _load_subject_data(analyses: List[str]) -> dict:
    ...
    for analysis_path in analyses:
        try:
            ...
            if "normal_mean_value" in csv_data:
                metrics.update(
                    {
                        "normal_mean_value": float(
                            csv_data.get("normal_mean_value", 0)
                        ),
                ...
                group_logger.debug(
                    f"TI_normal fields not available for {unique_name} (legacy format)"
                )

            # Try to find the field file path used during analysis
            field_path = _find_field_path_from_analysis(
                analysis_path, subject_id, montage_name
            )
            ...
            grey_stats = _get_grey_matter_statistics(
                analysis_path, subject_id, montage_name, field_path
            )
            ...
            group_logger.debug(
                f"Loaded data for {unique_name}: mean={metrics['mean_value']:.6f}, "
    ...
    group_logger.debug(
        f"Successfully loaded data from {len(analysis_results)} analyses"
    )
    return analysis_results


def _find_field_path_from_analysis(
    analysis_path: str, subject_id: str, montage_name: str
) -> str:
    """
                    ...
                    for pattern in field_patterns:
                        ...
                        if match:
                            ...
                            if os.path.exists(field_path):
                                group_logger.debug(
                                    f"Found field path in {log_file}: {field_path}"
                                )
                                return field_path
        ...
        if space_type == "mesh":
            ...
            if os.path.exists(field_dir):
                msh_files = [
                    ...
                    if f.endswith(".msh") and not f.endswith(".msh.opt")
                ]
                ...
                if ti_files:
                    ...
                    group_logger.debug(
                        f"Constructed mesh field path (TI): {field_path}"
                    )
                    return field_path
                elif msh_files:
                    ...
                    group_logger.debug(
                        f"Constructed mesh field path (fallback): {field_path}"
                    )
                    return field_path
            ...
            if os.path.exists(field_dir):
                grey_files = [
                    f
                    for f in os.listdir(field_dir)
                    if f.startswith("grey_") and f.endswith((".nii", ".nii.gz"))
                ]
                if grey_files:
...
def _get_grey_matter_statistics(
    analysis_path: str, subject_id: str, montage_name: str, field_path: str = None
) -> dict:
    """
    ...
    try:
        ...
        if space_type == "mesh":
            ...
            if field_path and os.path.exists(field_path):
                ...
                try:
                    ...
                    group_logger.warning(
                        f"Could not import mesh_analyzer for {analysis_path}: {str(e)}"
                    )
                    return {"grey_mean": 0.0, "grey_max": 0.0, "grey_min": 0.0}
            else:
            ...
            if field_path and os.path.exists(field_path):
                ...
                try:
                    ...
                    group_logger.warning(
                        f"Could not import voxel_analyzer for {analysis_path}: {str(e)}"
                    )
                    return {"grey_mean": 0.0, "grey_max": 0.0, "grey_min": 0.0}
            else:
                ...
                if not os.path.exists(field_dir):
                    group_logger.warning(
                        f"Voxel field directory not found: {field_dir}"
                    )
                    return {"grey_mean": 0.0, "grey_max": 0.0, "grey_min": 0.0}
                ...
                if not grey_files:
                    group_logger.warning(
                        f"No grey matter NIfTI files found in {field_dir}"
                    )
                    return {"grey_mean": 0.0, "grey_max": 0.0, "grey_min": 0.0}
        ...
        group_logger.error(
            f"Error getting grey matter statistics for {analysis_path}: {str(e)}"
        )
        return {"grey_mean": 0.0, "grey_max": 0.0, "grey_min": 0.0}


def _compute_subject_stats(
    analysis_results: dict, region_name: str = None
) -> pd.DataFrame:
    """
    ...
    has_normal_data = any(
        "normal_mean_value" in data["metrics"] for data in analysis_results.values()
    )
    ...
    if has_normal_data:
        numeric_cols.extend(
            [f"Normal_Mean", f"Normal_Max", f"Normal_Min", f"Normal_Focality"]
        )
    ...
    group_logger.info(
        f"TI_max averages - ROI_Mean: {averages[f'ROI_Mean']:.6f}, ROI_Max: {averages[f'ROI_Max']:.6f}, "
        f"ROI_Min: {averages[f'ROI_Min']:.6f}, ROI_Focality: {averages[f'ROI_Focality']:.6f}"
    )
    group_logger.info(
        f"TI_max std devs - ROI_Mean: {std_devs[f'ROI_Mean']:.6f}, ROI_Max: {std_devs[f'ROI_Max']:.6f}, "
    ...
    if has_normal_data:
        group_logger.info(
            f"TI_normal averages - Normal_Mean: {averages[f'Normal_Mean']:.6f}, Normal_Max: {averages[f'Normal_Max']:.6f}, "
            f"Normal_Min: {averages[f'Normal_Min']:.6f}, Normal_Focality: {averages[f'Normal_Focality']:.6f}"
        )
        group_logger.info(
            f"TI_normal std devs - Normal_Mean: {std_devs[f'Normal_Mean']:.6f}, Normal_Max: {std_devs[f'Normal_Max']:.6f}, "
...
def _write_summary_csv(
    stats_df: pd.DataFrame, output_path: str, region_name: str = None
) -> str:
    """
    ...
    group_logger.debug(
        f"Enhanced ROI-specific summary CSV successfully written with {len(stats_df)} rows"
    )
    return csv_path
...
def _generate_comparison_plot(
    ...
    subject_df = stats_df[
        ~stats_df["Subject_ID"].str.contains("AVERAGE|DIFF%", na=False)
    ].copy()
    ...
    if len(subject_df) < 2:
        group_logger.warning(
            "At least 2 subjects required for standard deviation analysis"
        )
        return ""
    ...
    for i, (metric, label, color, ax) in enumerate(
        zip(metrics, metric_labels, colors, axes)
    ):
            ...
            if std_val > 0:
                ...
                if z_score > 2:
                    ax.annotate(
                        ...
                        ha="center",
                        va="bottom" if val > mean_val else "top",
                        fontweight="bold",
                        fontsize=8,
                        color="darkred",
                        bbox=dict(
...
def compare_analyses(
    ...
    group_logger.info(
        f"Starting comprehensive analysis comparison for {len(analyses)} analyses"
    )
    group_logger.info(
        f"Options - Generate CSV: {generate_csv}, Generate Plot: {generate_plot}"
    )
    ...
    try:
        ...
        json_path = _create_analysis_info_json(
            analysis_results, run_output_dir, region_name
        )
        ...
        if generate_plot:
            ...
            if subject_count > 0:
                plot_path = _generate_comparison_plot(
                    stats_df, run_output_dir, region_name
                )
                if plot_path:
...
def collect_arguments() -> Tuple[List[str], str]:
    ...
    parser = argparse.ArgumentParser(
        description="Compare multiple SimNIBS analysis outputs"
    )
    parser.add_argument(
        "-analyses",
        nargs="+",
        required=True,
        help="Absolute paths to analysis directories (minimum 2)",
    )
...
def average_nifti_images(
    nifti_paths: List[str],
    output_filename: str = "average_output.nii.gz",
    output_dir: str = None,
) -> str:
    """
    ...
    try:
        ...
        averaged_img = nib.Nifti1Image(
            averaged_data, reference_affine, reference_header
        )
        ...
        if output_dir:
            ...
            output_path = _get_output_path(
                nifti_paths, "averaging", output_filename, "avg"
            )
        ...
        if group_logger:
            group_logger.debug(
                f"Successfully created averaged NIfTI image: {output_path}"
            )
            group_logger.info(
                f"Output shape: {averaged_data.shape}, Mean value: {np.mean(averaged_data):.6f}"
            )
...
def intersection_high_values_nifti(
    nifti_paths: List[str],
    ...
    if min_overlap < 1 or min_overlap > len(nifti_paths):
        raise ValueError(
            f"min_overlap must be between 1 and {len(nifti_paths)}, got {min_overlap}"
        )

    if percentile_low >= percentile_high:
        raise ValueError(
            f"percentile_low ({percentile_low}) must be less than percentile_high ({percentile_high})"
        )
    ...
    if group_logger:
        group_logger.info(
            f"Processing {len(nifti_paths)} NIfTI images for high-value intersection"
        )
        group_logger.info(
            f"Percentile range: {percentile_low}%-{percentile_high}%, Min overlap: {min_overlap}/{len(nifti_paths)}"
        )
    ...
    try:
        ...
        for i, path in enumerate(nifti_paths):
            ...
            if np.any(finite_nonzero_mask):
                ...
                high_value_masks[i] = (
                    (data >= p_low) & (data <= p_high) & finite_nonzero_mask
                )
                num_high_voxels = np.sum(high_value_masks[i])
                if group_logger:
                    group_logger.debug(
                        f"Image {i+1}: percentiles {p_low:.6f}-{p_high:.6f}, high-value voxels: {num_high_voxels}"
                    )
            else:
                if group_logger:
                    group_logger.warning(
                        f"No finite non-zero values found in {os.path.basename(path)}"
                    )
        ...
        overlap_count = np.sum(
            high_value_masks, axis=0
        )  # Count how many images have high values at each voxel
        ...
        if group_logger:
            group_logger.info(
                f"Intersection voxels: {num_intersection_voxels} ({intersection_percent:.2f}%)"
            )

        if num_intersection_voxels == 0:
            if group_logger:
                group_logger.warning(
                    "No voxels meet intersection criteria - output will be filled with fill_value"
                )
        ...
        if num_intersection_voxels > 0:
            ...
            if group_logger:
                group_logger.info(
                    "Computing weighted averages for intersection voxels..."
                )
            ...
            for i in range(len(intersection_indices[0])):
                ...
                if len(reference_shape) == 3:
                    spatial_idx = (
                        intersection_indices[0][i],
                        intersection_indices[1][i],
                        intersection_indices[2][i],
                    )
                    ...
                    spatial_idx = (
                        ...
                        intersection_indices[1][i],
                        intersection_indices[2][i],
                        intersection_indices[3][i],
                    )
                else:
                    raise ValueError(
                        f"Unsupported image dimensionality: {len(reference_shape)}D. Only 3D and 4D images are supported."
                    )
        ...
        intersection_img = nib.Nifti1Image(
            output_data, reference_affine, reference_header
        )
        ...
        if output_dir:
            ...
            output_path = _get_output_path(
                nifti_paths, "intersection", output_filename, suffix
            )
        ...
        if num_intersection_voxels > 0:
            ...
            if group_logger:
                group_logger.debug(
                    f"Successfully created intersection NIfTI: {output_path}"
                )
                group_logger.info(
                    f"Intersection statistics - voxels: {num_intersection_voxels}, range: {value_range}, mean: {mean_value:.6f}"
                )
        else:
            if group_logger:
                group_logger.info(
                    f"Created intersection NIfTI filled with {fill_value}: {output_path}"
                )
...
def _generate_focality_summary(
    analysis_dirs: List[str], output_dir: str, region_name: str
) -> str:
    """
    ...
    try:
        ...
        for analysis_dir in analysis_dirs:
            ...
            if not extra_info_files:
                group_logger.warning(
                    f"No extra_info CSV files found for subject {subject_id}"
                )
                continue
        ...
        columns_to_include = ["Subject_ID"] + [
            col for col in existing_cols if col != "Subject_ID"
        ]
        ...
        group_logger.info(
            f"Focality summary saved with {len(df_summary)} entries to: {output_path}"
        )
...
def run_all_group_comparisons(
    analysis_dirs: List[str], project_name: str = None
) -> str:
    """
    ...
    if len(analysis_dirs) < 2:
        group_logger.warning(
            f"Only {len(analysis_dirs)} analysis directory provided. Some comparisons require multiple subjects."
        )
    ...
    group_logger.info(
        f"Starting comprehensive group analysis for {len(analysis_dirs)} subjects"
    )
    group_logger.info(f"Analysis focus: {montage_name} montage, {region_name} region")
    ...
    if len(analysis_dirs) >= 1:  # Allow single subject for basic statistics
        ...
        try:
            comparison_results = compare_analyses(
                analysis_dirs, run_output_dir, generate_plot=True, generate_csv=True
            )
            if comparison_results:
                group_logger.info(
                    f"Statistical comparison completed for {comparison_results.get('total_subjects', 0)} subjects"
                )
    ...
    if len(analysis_dirs) >= 1:
        ...
        try:
            focality_summary_path = _generate_focality_summary(
                analysis_dirs, run_output_dir, region_name
            )
            if focality_summary_path:
                group_logger.info(
                    f"Focality summary generated: {focality_summary_path}"
                )
        except Exception as e:
    ...
    for analysis_dir in analysis_dirs:
        ...
        if not subject_id or not extracted_montage:
            group_logger.warning(
                f"Could not extract subject ID or montage from path: {analysis_dir}"
            )
            continue
        ...
        group_logger.info(
            f"Found {len(grey_files)} grey matter NIfTI files in {nifti_dir}"
        )
        ...
        if not matching_files:
            group_logger.info(
                f"No exact pattern match found, trying flexible matching for {subject_id}"
            )
            for file in grey_files:
                # Check if file contains the subject and montage info in any order
                if (
                    subject_short in file
                    and extracted_montage in file
                    and "TI" in file
                    and "max" in file
                ):
                    matching_files.append(file)
        ...
        if matching_files:
            ...
            group_logger.info(
                f"Found standardized NIfTI for {subject_id}: {nifti_filename} (pattern: {used_pattern})"
            )
        else:
            group_logger.warning(
                f"No standardized grey matter MNI NIfTI found for {subject_id} in {nifti_dir}"
            )
            group_logger.warning(f"Tried patterns: {potential_patterns}")
            group_logger.warning(f"Available grey matter files: {grey_files}")

    group_logger.info(
        f"Found {len(nifti_files)} standardized grey matter MNI NIfTI files for image-based analysis"
    )
    ...
    if len(nifti_files) >= 2:
        ...
        group_logger.info(
            "Only 1 NIfTI file found - skipping averaging (requires at least 2 files)"
        )
    else:
    ...
    if len(nifti_files) >= 2:
        ...
        try:
            ...
            group_logger.info(
                f"Strict intersection (all subjects) saved to: {intersection_all}"
            )
        ...
        group_logger.info(
            "Only 1 NIfTI file found - skipping intersection (requires at least 2 files)"
        )
    else:
    ...
    group_logger.info(
        f"Group analysis complete. All results saved to: {run_output_dir}"
    )
    if nifti_results:
...
def _create_analysis_info_json(
    analysis_results: dict, output_path: str, region_name: str = None
) -> str:
    """
    ...
    for unique_name, data in analysis_results.items():
        ...
        try:
            ...
            group_logger.warning(
                f"Could not determine field file for {unique_name}: {str(e)}"
            )
        ...
        analysis_entry = {
            ...
            "csv_file": (
                os.path.basename(data["csv_path"]) if "csv_path" in data else "unknown"
            ),
        }
    ...
    group_logger.info(
        f"Analysis information JSON file created with {len(analysis_info['analyses'])} entries"
    )
    return json_path
...
if __name__ == "__main__":
    ...
    try:
        results = compare_analyses(
            analyses, output_dir, generate_plot=True, generate_csv=True
        )
        ...
        if results:
            if group_logger:
                group_logger.info("\n" + "=" * 60)
                group_logger.debug(
                    "ROI-SPECIFIC ANALYSIS COMPARISON COMPLETED SUCCESSFULLY"
                )
                group_logger.info("=" * 60)
                group_logger.info(
                    f"Total subjects analyzed: {results.get('total_subjects', 0)}"
                )
                ...
                if avg_stats:
                    ...
                    group_logger.info(
                        f"  • ROI Focality: {avg_stats.get('focality', 0):.6f}"
                    )
            ...
            if group_logger:
                group_logger.warning(
                    "Analysis comparison completed but no results were generated."
                )
