...
class VoxelAnalyzer:
    ...
    def __init__(
        ...
        if nib is None:
            raise ImportError(
                "nibabel is required for voxel analysis but is not installed"
            )
        ...
        if logger is not None:
            ...
            self.logger = logging_util.get_logger(
                "voxel_analyzer", log_file, overwrite=True
            )
    ...
    def analyze_whole_head(self, atlas_file, visualize=False):
        ...
        try:
            ...
            if len(field_arr.shape) == 4:
                self.logger.debug(
                    f"Detected 4D field data with shape {field_arr.shape}"
                )
                ...
                if field_arr.shape[3] == 1:
                    ...
                    self.logger.warning(
                        f"4D field has {field_arr.shape[3]} volumes. Using only the first volume."
                    )
                    field_arr = field_arr[:, :, :, 0]
            else:
            ...
            if atlas_arr.shape != field_shape_3d:
                self.logger.debug(
                    "Atlas and field dimensions don't match, attempting to resample..."
                )
                ...
                if atlas_arr.shape != field_shape_3d:
                    raise ValueError(
                        f"Failed to resample atlas to match field dimensions: {atlas_arr.shape} vs {field_shape_3d}"
                    )
            else:
                self.logger.debug(
                    "Atlas and field dimensions already match - skipping resampling"
                )
            ...
            for region_id, info in region_info.items():
                ...
                try:
                    ...
                    if mask_count == 0:
                        self.logger.warning(
                            f"Warning: Region {region_name} (ID: {region_id}) contains 0 voxels in the atlas"
                        )
                        region_results = {
                            "mean_value": None,
                            "max_value": None,
                    ...
                    if filtered_count == 0:
                        self.logger.warning(
                            f"Warning: Region {region_name} (ID: {region_id}) has no voxels with positive values"
                        )
                        region_results = {
                            "mean_value": None,
                            "max_value": None,
                    ...
                    if visualize:
                        ...
                        if len(field_values) > 0:
                            try:
                                ...
                                field_img_tuple = self.load_brain_image(
                                    self.field_nifti
                                )
                                ...
                                whole_head_filtered = field_arr[
                                    whole_head_positive_mask
                                ]
                ...
                self.visualizer.save_whole_head_results_to_csv(
                    results, atlas_type, "voxel"
                )
    ...
    def analyze_sphere(self, center_coordinates, radius, visualize=False):
        ...
        if roi_voxels_count == 0:
            ...
            tissue_type = (
                "grey matter"
                if "grey" in field_name
                else "white matter" if "white" in field_name else "brain tissue"
            )
...
• {"Adjust coordinates/radius" if total_roi_voxels == 0 else "Check field data"} or verify using freeview\033[0m"""
        ...
        if visualize:
            ...
            output_filename = os.path.join(
                self.output_dir, f"sphere_overlay_{region_name}.nii.gz"
            )
        ...
        if self.visualizer is not None:
            ...
            self.visualizer.save_results_to_csv(
                results, "spherical", region_name, "voxel"
            )

            # Save extra info CSV with focality data
            if focality_info:
                self.visualizer.save_extra_info_to_csv(
                    focality_info, "spherical", region_name, "voxel"
                )
        else:
            self.logger.warning(
                "Cannot save results to CSV - VoxelVisualizer not available"
            )
    ...
    def resample_to_match(
        self, source_img, target_shape, target_affine, source_path=None
    ):
        ...
        if not self.quiet:
            self.logger.info(
                f"Resampling image from shape {source_img.shape} to {target_shape}"
            )

        # If source_path is provided, try to find an existing resampled file
        if source_path:
            resampled_path = self._get_resampled_atlas_filename(
                source_path, target_shape
            )
            if os.path.exists(resampled_path):
                if not self.quiet:
                    self.logger.info(
                        f"Found existing resampled atlas: {resampled_path}"
                    )
                try:
                    ...
                    if resampled_data.shape[:3] == target_shape[:3]:
                        ...
                        if is_target_4d and len(resampled_data.shape) == 3:
                            if not self.quiet:
                                self.logger.info(
                                    f"Reshaping 3D data {resampled_data.shape} to match 4D target {target_shape}"
                                )
                            ...
                            resampled_img = nib.Nifti1Image(
                                resampled_data, resampled_img.affine, header=new_header
                            )

                        return resampled_img, resampled_data
                    else:
                        self.logger.warning(
                            f"Existing resampled atlas has wrong shape: {resampled_data.shape[:3]} vs expected {target_shape[:3]}"
                        )
                except Exception as e:
                    self.logger.error(
                        f"Error loading existing resampled atlas: {str(e)}"
                    )
                    if not self.quiet:
        ...
        try:
            ...
            cmd = [
                "mri_convert",
                "--reslice_like",
                template_path,  # Use template for resampling
            ...
            if source_path and not temp_source_created:
                resampled_save_path = self._get_resampled_atlas_filename(
                    source_path, target_shape
                )
                if not self.quiet:
                    self.logger.info(
                        f"Saving resampled atlas for future use: {resampled_save_path}"
                    )
            ...
            if (
                "temp_source_created" in locals()
                and temp_source_created
                and "source_path" in locals()
            ):
                try:
    ...
    def analyze_cortex(
        ...
        if mask_count == 0:
            ...
            self.visualizer.save_results_to_csv(
                results, "cortical", region_name, "voxel"
            )
        ...
        if focality_info:
            self.visualizer.save_extra_info_to_csv(
                focality_info, "cortical", region_name, "voxel"
            )
        ...
        try:
            ...
            if len(percentile_values) > 2:
                ...
                for cutoff in focality_cutoffs:
                    ...
                    volume = (
                        np.sum(volumes[above_threshold])
    ...
    def get_atlas_regions(self, atlas_file):
        ...
        def generate_labels_file():
            ...
            try:
                ...
                self.logger.warning(
                    f"Warning: Could not extract region information using mri_segstats: {str(e)}"
                )
                self.logger.debug(
                        ...
                        if not in_header and line.strip():
                            ...
                            if len(parts) >= 5:
                                try:
                                    region_id = int(
                                        parts[1]
                                    )  # SegId is the second column
                                    n_voxels = int(
                                        parts[2]
                                    )  # NVoxels is the third column
        ...
        if os.path.exists(output_file):
            ...
            if parse_labels_file():
                if not self.quiet:
                    self.logger.info(
                        f"Successfully parsed {len(region_info)} regions from existing file"
                    )
                return region_info
            else:
                if not self.quiet:
                    self.logger.info(
                        "Existing file is invalid or empty, regenerating..."
                    )
        ...
        if generate_labels_file():
            if parse_labels_file():
                if not self.quiet:
                    self.logger.info(
                        f"Successfully generated and parsed {len(region_info)} regions"
                    )
                return region_info
    ...
    def get_grey_matter_statistics(self):
        ...
        try:
            ...
            if not self.quiet:
                self.logger.info(
                    f"Grey matter statistics for field '{os.path.basename(self.field_nifti)}' (positive values only): "
