...
def compare_subject_metrics(
    ...
    if comparison_results:
        ...
        logger.debug(
            f"Compared {len(comparison_results)} subjects that appear in all CSV files"
        )
...
def generate_group_statistics(
    csv_data: Dict[str, pd.DataFrame], output_dir: str, logger: logging.Logger
...
def compare_group_statistics(
    group_stats: Dict[str, pd.DataFrame], output_dir: str, logger: logging.Logger
) -> pd.DataFrame:
    """
    ...
    for metric in all_metrics:
        ...
        for file_name in file_names:
            if metric in group_stats[file_name].columns:
                metric_comparison[f"{file_name}_mean"] = group_stats[file_name].loc[
                    "mean", metric
                ]
                metric_comparison[f"{file_name}_std"] = group_stats[file_name].loc[
                    "std", metric
                ]
                metric_comparison[f"{file_name}_median"] = group_stats[file_name].loc[
                    "median", metric
                ]
        ...
        if len(file_names) == 2:
            ...
            if (
                metric in group_stats[file1].columns
                and metric in group_stats[file2].columns
            ):
                mean_diff = (
                    group_stats[file1].loc["mean", metric]
                ...
                pooled_std = np.sqrt(
                    (
                        group_stats[file1].loc["std", metric] ** 2
                        + group_stats[file2].loc["std", metric] ** 2
                    )
                    / 2
                )
                if pooled_std != 0:
...
def create_visualization_plots(
    ...
    if not comparison_df.empty and len(file_names) == 2:
        ...
        for metric in target_metrics:
            ...
            if col1 in comparison_df.columns and col2 in comparison_df.columns:
                ...
                if len(valid_data) == 0:
                    logger.warning(
                        f"No subjects with valid data for both conditions in {metric}"
                    )
                    plt.close()
                    continue
                ...
                plt.close()
                logger.debug(
                    f"Subject comparison plot for {metric} saved to: {plot_file} ({len(valid_data)} subjects)"
                )
    ...
    if not comparison_df.empty and group_stats and len(file_names) == 2:
        ...
        for metric in target_metrics:
            ...
            col2 = f"{file_names[1]}_{metric}"
            if (
                col1 in comparison_df.columns
                and col2 in comparison_df.columns
                and all(metric in stats_df.columns for stats_df in group_stats.values())
            ):
                available_metrics.append(metric)
        ...
        if available_metrics:
            ...
            for i, metric in enumerate(available_metrics):
                ...
                for val1, val2 in zip(data1, data2):
                    ax.plot(
                        [1, 2], [val1, val2], color="gray", alpha=0.3, linewidth=0.5
                    )
                ...
                ax.set_xticklabels(
                    [
                        (
                            file_names[0][:15] + "..."
                            if len(file_names[0]) > 15
                            else file_names[0]
                        ),
                        (
                            file_names[1][:15] + "..."
                            if len(file_names[1]) > 15
                            else file_names[1]
                        ),
                    ],
                    rotation=45,
...
def generate_summary_report(
    ...
    with open(report_file, "w") as f:
        ...
        for file_name, df in csv_data.items():
            f.write(f"{file_name}:\n")
            f.write(
                f"  - Subjects: {len(df['Subject_ID'].unique()) if 'Subject_ID' in df.columns else 'N/A'}\n"
            )
        ...
        if not comparison_df.empty:
            ...
            f.write("-" * 28 + "\n")
            f.write(
                f"Number of subjects found in ALL CSV files: {len(comparison_df)}\n"
            )
            ...
            diff_columns = [
                col for col in comparison_df.columns if col.endswith("_diff")
            ]
            if diff_columns:
                ...
                for col in diff_columns:
                    ...
                    f.write(
                        f"  {col.replace('_diff', '')}: Max={max_diff:.4f}, Mean={mean_diff:.4f}\n"
                    )
                f.write("\n")
        else:
            f.write("SUBJECTS COMPARISON ANALYSIS\n")
        ...
        if not group_comparison_df.empty:
            ...
            if "mean_difference" in group_comparison_df.columns:
                sorted_diffs = group_comparison_df.sort_values(
                    "mean_difference", key=abs, ascending=False
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Compare multiple CSV files with analysis results"
    )
    parser.add_argument("csv_files", nargs="+", help="Paths to CSV files to compare")
    parser.add_argument(
        "-o",
        "--output",
        default="csv_comparison_results",
    ...
    parser.add_argument(
        "--no-report", action="store_true", help="Skip generating summary report"
    )
    ...
    try:
        ...
        comparison_df = compare_subject_metrics(
            csv_data, common_subjects, args.output, logger
        )
        ...
        if not args.no_plots:
            create_visualization_plots(
                csv_data, comparison_df, group_stats, args.output, logger
            )
