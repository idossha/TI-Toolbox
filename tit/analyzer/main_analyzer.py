...
def log_analysis_complete(
    analysis_type, subject_id, results_summary="", output_path=""
):
    ...
    if _analysis_start_time is not None:
        ...
        if SUMMARY_MODE:
            if results_summary:
                print(
                    f"└─ Analysis completed successfully for subject: {subject_id} ({results_summary}, Total: {duration_str})"
                )
            else:
                print(
                    f"└─ Analysis completed successfully for subject: {subject_id} (Total: {duration_str})"
                )
            if output_path:
                # Show relative path from /mnt/ for cleaner display
                display_path = (
                    output_path.replace("/mnt/", "")
...
def log_analysis_failed(analysis_type, subject_id, error_msg):
    ...
    if _analysis_start_time is not None:
        ...
        if SUMMARY_MODE:
            print(
                f"└─ Analysis failed for subject: {subject_id} ({duration_str}) - {error_msg}"
            )
        else:
            logger.error(
                f"Analysis failed for subject: {subject_id} ({duration_str}) - {error_msg}"
    ...
    if args.analysis_type == "spherical":
        ...
        if args.whole_head:
            if args.space == "mesh":
                ...
                atlas_name = (
                    os.path.basename(args.atlas_path) if args.atlas_path else "atlas"
                )
...
def setup_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze neuroimaging data in mesh or voxel space"
    )

    # Required arguments
    parser.add_argument(
        "--m2m_subject_path", required=True, help="Path to the m2m subject folder"
    )
    parser.add_argument(
        "--space",
        required=True,
    ...
    parser.add_argument(
        "--atlas_name", help="Atlas name for mesh-based cortical analysis (e.g., DK40)"
    )
    parser.add_argument(
        "--atlas_path", help="Path to atlas file for voxel-based cortical analysis"
    )
    parser.add_argument(
        "--coordinates", nargs=3, help="x y z coordinates for spherical analysis"
    )
    parser.add_argument(
        "--coordinate-space",
        choices=["MNI", "subject"],
    ...
    parser.add_argument("--radius", type=float, help="Radius for spherical analysis")
    parser.add_argument(
        "--region",
    ...
    parser.add_argument(
        "--visualize", action="store_true", help="Generate visualization outputs"
    )
    parser.add_argument(
        "--log_file",
...
def validate_args(args):
    ...
    if args.space == "mesh":
        ...
        if args.field_path and os.path.exists(args.field_path):
            ...
            if not args.montage_name:
                logger.error(
                    "--montage_name is required for mesh analysis when --field_path is not provided"
                )
                raise ValueError(
                    "--montage_name is required for mesh analysis when --field_path is not provided"
                )
            # Construct and validate mesh field path
            args.field_path = construct_mesh_field_path(
                args.m2m_subject_path, args.montage_name
            )
            if not os.path.exists(args.field_path):
                logger.error(
                    f"Constructed mesh field file not found: {args.field_path}"
                )
                raise ValueError(
                    f"Constructed mesh field file not found: {args.field_path}"
                )
    ...
    if args.analysis_type == "spherical":
        ...
        if not hasattr(args, "coordinate_space") or not args.coordinate_space:
            logger.error(
                "Coordinate space is required for spherical analysis (--coordinate-space MNI or --coordinate-space subject)"
            )
        ...
        if args.space == "mesh":
            if not args.atlas_name:
                ...
                raise ValueError(
                    "Atlas name is required for mesh-based cortical analysis"
                )
        else:  # voxel
            if not args.atlas_path:
                logger.error("Atlas path is required for voxel-based cortical analysis")
                raise ValueError(
                    "Atlas path is required for voxel-based cortical analysis"
                )
        ...
        if not args.whole_head and not args.region:
            logger.error(
                "Either --whole_head flag or --region must be specified for cortical analysis"
            )
            raise ValueError(
                "Either --whole_head flag or --region must be specified for cortical analysis"
            )
        if args.whole_head and args.region:
            logger.warning(
                "Warning: --region is ignored when --whole_head is specified"
            )
...
def main():
    ...
    try:
        ...
        subject_id = (
            os.path.basename(args.m2m_subject_path).split("_")[1]
            if "_" in os.path.basename(args.m2m_subject_path)
            else os.path.basename(args.m2m_subject_path)
        )
        ...
        if args.analysis_type == "spherical" and args.coordinate_space == "MNI":
            ...
            logger.info(
                f"Transforming MNI coordinates [{', '.join(formatted_orig_coords)}] to subject {subject_id} space"
            )
            flush_output()
            ...
            try:
                ...
                subject_coords = ROICoordinateHelper.transform_mni_to_subject(
                    mni_coords, args.m2m_subject_path
                )

                # Validate transformation result
                if subject_coords is None or len(subject_coords) != 3:
                    raise ValueError(
                        f"MNI transformation failed: returned {subject_coords}"
                    )
                ...
                logger.info(
                    f"Transformed MNI coordinates for {subject_id}: [{', '.join(formatted_coords)}]"
                )
                flush_output()
            ...
            if analyzer is None:
                if args.quiet:
                    log_analysis_step_failed(
                        "Field data loading",
                        subject_id,
                        "Failed to initialize mesh analyzer",
                    )
            ...
            if analyzer is None:
                if args.quiet:
                    log_analysis_step_failed(
                        "Field data loading",
                        subject_id,
                        "Failed to initialize voxel analyzer",
                    )
            ...
            if args.whole_head:
                if args.space == "mesh":
                    ...
                    results = analyzer.analyze_whole_head(
                        atlas_type=args.atlas_name, visualize=args.visualize
                    )
                else:  # voxel
                    if not args.quiet:
                        ...
                        flush_output()
                    results = analyzer.analyze_whole_head(
                        atlas_file=args.atlas_path, visualize=args.visualize
                    )
            ...
            if args.analysis_type == "spherical":
                if isinstance(results, dict) and results.get("voxel_count"):
                    ...
                    log_analysis_step_complete(
                        "Spherical Analysis", subject_id, step_details
                    )
                else:
                ...
                if isinstance(results, dict):
                    if any(
                        k in results for k in ["mean_value", "max_value", "min_value"]
                    ):
                        # Single region results
                        log_analysis_step_complete(
                            "Cortical Analysis", subject_id, "1 region analyzed"
                        )
                    else:
                        ...
                        valid_regions = len(
                            [
                                ...
                                and r.get("mean_value") is not None
                            ]
                        )
                        ...
                        log_analysis_step_complete(
                            "Cortical Analysis",
                            subject_id,
                            f"{valid_regions}/{total_regions} regions processed",
                        )
                else:
        ...
        if not args.quiet:
            if isinstance(results, dict):
                if any(k in results for k in ["mean_value", "max_value", "min_value"]):
                    # Single region results
                    logger.info(
                        "Analysis completed successfully: Single region analysis"
                    )
                    flush_output()
                else:
                    ...
                    valid_regions = len(
                        [
                            r
                            for r in results.values()
                            if isinstance(r, dict) and r.get("mean_value") is not None
                        ]
                    )
                    total_regions = len(results)
                    logger.info(
                        f"Analysis completed successfully: {valid_regions}/{total_regions} regions processed"
                    )
                    flush_output()
            else:
        ...
        if isinstance(results, dict) and any(
            ...
            if any(
                k in results
                for k in ["normal_mean_value", "normal_max_value", "normal_min_value"]
            ):
                print("\nTI_normal Values:")
            ...
            for region_name, region_data in results.items():
                if (
                    isinstance(region_data, dict)
                    and region_data.get("mean_value") is not None
                ):
                    print(f"\n{region_name}:")
                    print("  TI_max Values:")
                    ...
                    if any(
                        ...
                        print_stat_if_exists(
                            region_data, "normal_mean_value", "  Normal Mean Value"
                        )
                        print_stat_if_exists(
                            region_data, "normal_max_value", "  Normal Max Value"
                        )
                        print_stat_if_exists(
                            region_data, "normal_min_value", "  Normal Min Value"
                        )
                        print_stat_if_exists(
                            region_data, "normal_focality", "  Normal Focality"
                        )
        ...
        if args.quiet:
            ...
            display_path = (
                args.output_dir.replace("/mnt/", "")
                if args.output_dir.startswith("/mnt/")
            ...
            log_analysis_complete(
                args.analysis_type, subject_id, results_summary, args.output_dir
            )
