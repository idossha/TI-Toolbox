...
class MessageParser:
    ...
    def _determine_message_type(self, line: str) -> str:
        ...
        if any(
            ...
            for keyword in ["error:", "critical:", "failed", "exception", "traceback"]
        ):
            return "error"
        elif any(keyword in line_lower for keyword in ["warning:", "warn"]):
            return "warning"
        elif any(
            keyword in line_lower for keyword in ["executing:", "running", "command"]
        ):
            return "command"
        elif any(
            keyword in line_lower
            for keyword in ["completed successfully", "completed.", "✓ complete"]
        ):
            return "success"
        elif any(
            keyword in line_lower
            for keyword in ["processing", "starting", "generating", "loading"]
        ):
            return "info"
