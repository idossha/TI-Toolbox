...
class ProcessError(TIToolboxError):
    ...
    def __init__(self, message: str, return_code: Optional[int] = None, 
                 stderr: Optional[str] = None, command: Optional[str] = None):
        details = {}
...
class MeshError(TIToolboxError):
    ...
    def __init__(self, message: str, mesh_file: Optional[str] = None, 
...
def validate_subject(subject_id: str, project_dir: str) -> None:
    ...
    if not subject_dir.exists():
        raise SubjectError(
            f"Subject directory not found: {subject_id}",
    ...
    for key in required_keys:
        if key not in montage_config:
            raise ValidationError(
                f"Missing required key in montage configuration: {key}",
                field=key
            )
