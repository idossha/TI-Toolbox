...
"""
TI-Toolbox-2.0 Simulation Report Generator
...
class SimulationReportGenerator:
    ...
    def __init__(self, project_dir, simulation_session_id=None):
        ...
        self.simulation_session_id = (
            simulation_session_id or datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        )
    ...
    def _get_default_conductivities(self):
        ...
        return {
            ...
            100: {
                ...
                "conductivity": 29.4,
                "reference": "NeuroConn electrodes: Wacker Elastosil R 570/60 RUSS",
            },
            500: {
                "name": "Saline",
                "conductivity": 1.0,
                "reference": "Saturnino et al., 2015",
            },
        }
    ...
    def _collect_software_versions(self):
        ...
        try:
            # TI-Toolbox version (if available)
            version_file = self.project_dir / "tit" / "VERSION"
    ...
    def add_simulation_parameters(
        ...
        self.report_data["simulation_parameters"] = {
            ...
            "simulation_mode_text": (
                "Unipolar" if simulation_mode == "U" else "Multipolar"
            ),
            "eeg_net": eeg_net,
    ...
    def add_electrode_parameters(self, shape, dimensions, thickness):
        ...
        self.report_data["electrode_parameters"] = {
            ...
            "area_mm2": (
                ...
                if shape == "rect"
                else np.pi * (dimensions[0] / 2) * (dimensions[1] / 2)
            ),
            "timestamp": datetime.datetime.now().isoformat(),
        }
    ...
    def add_montage(
        ...
        montage_data = {
            ...
            "electrodes_used": list(
                set([e for pair in (electrode_pairs or []) for e in pair])
            ),
            "timestamp": datetime.datetime.now().isoformat(),
        }
                    ...
                    for output_type in ["TI", "mTI", "high_Frequency"]:
                        ...
                        if type_dir.exists():
                            ...
                            if montage_imgs_dir.exists():
                                ...
                                for pattern in montage_img_patterns:
                                    ...
                                    if montage_img_path.exists():
                                        montage_outputs["montage_image"] = str(
                                            montage_img_path
                                        )
                                        break
                            ...
                            if mesh_dir.exists():
                                ...
                                montage_outputs["files"][output_type].extend(
                                    [str(f) for f in mesh_files]
                                )
                            ...
                            if nifti_dir.exists():
                                ...
                                if ti_max_files:
                                    montage_outputs["nifti_visualizations"] = (
                                        ti_max_files
                                    )
    ...
    def _generate_brain_visualization(
        self, subject_id, montage_name, mesh_file=None, nifti_file=None
    ):
        ...
        try:
            ...
            self.add_warning(
                f"Failed to generate visualization: {str(e)}", subject_id, montage_name
            )
            return None
    ...
    def generate_report(self, output_path=None):
        ...
        for subject_data in self.report_data["subjects"]:
            if subject_data.get("m2m_path") and os.path.exists(
                subject_data["m2m_path"]
            ):
                subject_data["simulation_outputs"] = self._scan_simulation_outputs(
                    subject_data["subject_id"], subject_data["m2m_path"]
                )
        ...
        if output_path is None:
            ...
            if len(self.report_data["subjects"]) == 1:
                ...
                output_path = (
                    reports_dir
                    / f"simulation_session_{self.simulation_session_id}.html"
                )
...
<head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <title>TI-Toolbox Simulation Report - Session {simulation_session_id}</title>
    <style>
        {css_styles}
    </style>
</head>
<body>
    <div class="container">
        <header>
            <h1>TI-Toolbox Simulation Report</h1>
        ...
        <footer>
                    <p>Report generated by TI-Toolbox 2.0 Simulation Pipeline</p>
        <p>For questions or issues, please refer to the TI-Toolbox documentation.</p>
        </footer>
    </div>
...
""".format(
    ...
    def _generate_summary_section(self):
        ...
        completed_subjects = len(
            [s for s in self.report_data["subjects"] if s["status"] == "completed"]
        )
    ...
    def _generate_electrode_parameters_table(self, electrode_params):
        ...
        if not electrode_params:
            return (
                "<h3>Electrode Parameters</h3><p>No electrode parameters recorded.</p>"
            )
    ...
    def _generate_results_section(self):
        ...
        if not self.report_data["simulation_results"]:
            ...
            for result_key, result in self.report_data["simulation_results"].items():
                ...
                duration_text = (
                    f"{result['duration']:.2f}s" if result["duration"] else "N/A"
                )
                ...
                if output_files:
                    for file_type, files in output_files.items():
                        if isinstance(files, list):
                            ...
                            html += (
                                f"<li>{exists_indicator} {file_type}: {file_name}</li>"
                            )
                else:
        ...
        for result_key, result in self.report_data["simulation_results"].items():
            ...
            for file_list in [ti_files, nifti_files]:
                for nifti_file in file_list:
                    ...
                    if (
                        os.path.exists(nifti_file)
                        and "grey" in os.path.basename(nifti_file).lower()
                        and "subject_ti_max" in os.path.basename(nifti_file).lower()
                    ):
                ...
                for file_list in [ti_files, nifti_files]:
                    for nifti_file in file_list:
                        if (
                            os.path.exists(nifti_file)
                            and "grey" in os.path.basename(nifti_file).lower()
                            and "ti_max" in os.path.basename(nifti_file).lower()
                        ):
                            grey_file = nifti_file
                            break
                    if grey_file:
            ...
            if grey_file:
                ...
                try:
                    ...
                    for orientation in ["axial", "sagittal", "coronal"]:
                        if (
                            orientation in generated_images
                            and generated_images[orientation]
                        ):
                            html += f"""
    ...
    def _generate_methods_section(self):
        ...
        Electromagnetic field simulations were performed using the TI-Toolbox 2.0 
...
All simulations were performed using SimNIBS (www.simnibs.org) with head models derived 
        from individual anatomical MRI data processed through the TI-Toolbox preprocessing pipeline.
            </div>
        </section>
        """
        return html
    ...
    def _generate_static_overlay_images(
        self, subject_id, montage_name, t1_file, overlay_file, output_dir
    ):
...
def create_simulation_report(
    project_dir, simulation_session_id=None, simulation_log=None
):
