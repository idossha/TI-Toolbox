...
"""EEG source-forward and fsaverage field-mapping tab.

Two modes, both dispatched to ``simnibs_python -m tit.source`` in a background
thread:
...
class SourceTab(QtWidgets.QWidget):
    ...
    def _build_ui(self):
        ...
        # Mode selector
        mode_box = QtWidgets.QGroupBox("Pipeline")
        mode_layout = QtWidgets.QHBoxLayout(mode_box)
        self.mode_forward = QtWidgets.QRadioButton("Build forward solution")
        self.mode_fsavg = QtWidgets.QRadioButton("Map fields to fsaverage")
        self.mode_forward.setChecked(True)
        self.mode_forward.toggled.connect(self._update_mode)
        mode_layout.addWidget(self.mode_forward)
        mode_layout.addWidget(self.mode_fsavg)
        ...
        # Forward parameters
        self.forward_box = QtWidgets.QGroupBox("Forward parameters")
        ...
        fwd_form.addRow("FEM workers (cpus):", self.cpus_spin)
        self.fwd_overwrite = QtWidgets.QCheckBox("Overwrite existing outputs")
        fwd_form.addRow("", self.fwd_overwrite)
        layout.addWidget(self.forward_box)

        # fsaverage-map parameters
        self.fsavg_box = QtWidgets.QGroupBox("fsaverage map parameters")
        ...
        fsavg_form.addRow("Workers:", self.workers_spin)
        self.fsavg_overwrite = QtWidgets.QCheckBox("Overwrite cached projections")
        fsavg_form.addRow("", self.fsavg_overwrite)
        layout.addWidget(self.fsavg_box)
    ...
    def _update_mode(self):
        forward = self.mode_forward.isChecked()
        self.forward_box.setVisible(forward)
        self.fsavg_box.setVisible(not forward)
    ...
    def _build_config(self):
        ...
        if self.mode_forward.isChecked():
            ...
            return {
                ...
                "overwrite": self.fwd_overwrite.isChecked(),
            }
        ...
        return {
            ...
            "overwrite": self.fsavg_overwrite.isChecked(),
        }
    ...
    def _on_run(self):
        ...
        self.thread.process_finished.connect(self._on_finished)
        self.thread.start()

    def _on_finished(self, success, returncode):
        self.run_button.setEnabled(True)
        if success:
            self._append("Done.", "success")
            mode = "forward" if self.mode_forward.isChecked() else "fsavg_map"
