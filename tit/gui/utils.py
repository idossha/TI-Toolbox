...
def is_important_message(text, message_type, tab_type="general"):
    ...
    important_patterns = {
        ...
        "preprocess": [
            ...
            # Process step patterns - match the actual output format from bash_logging.sh
            "├─ dicom conversion:",
            "├─ simnibs charm:",
            "├─ freesurfer recon-all:",
            "├─ tissue analysis:",
            # Also match any step with Started/Complete format (case-insensitive matching in code below)
            ": started",
            ": ✓ complete",
            ": ✗ failed",
            # Completion patterns
            "└─ pre-processing completed",
            "└─ pre-processing completed successfully for subject:",
            "└─ pre-processing failed for subject:",
            # Atlas segmentation patterns
            "├─ atlas",
    ...
    if tab_type in important_patterns:
        ...
        if any(
            pattern.lower() in text_lower for pattern in important_patterns[tab_type]
        ):
            return True
