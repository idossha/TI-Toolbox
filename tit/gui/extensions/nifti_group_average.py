...
class SubjectRow(QtWidgets.QWidget):
    ...
    def __init__(
        self, parent=None, subjects_list=None, simulations_dict=None, groups_list=None
    ):
...
class NiftiGroupAverageWidget(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        self.diff_pairs_edit.setPlaceholderText(
            "e.g., Group1-Group2, Group1-Group3 (optional)"
        )
        self.diff_pairs_edit.setToolTip(
            "Comma-separated pairs in format: GroupA-GroupB\nLeave empty to compute all possible differences"
        )
    ...
    def import_from_csv(self):
        ...
        file_path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self, "Import Subject Configuration", "", "CSV Files (*.csv)"
        )
        ...
        try:
            ...
            with open(file_path, "w", newline="") as f:
                writer = csv.DictWriter(
                    f, fieldnames=["subject_id", "simulation_name", "group"]
                )
                writer.writeheader()
    ...
    def run_analysis(self):
        ...
        # Show summary
        groups_summary = "\n".join(
            [f"  • {group}: {count} subjects" for group, count in groups_dict.items()]
        )
    ...
    def on_finished(self, results):
        ...
        self.update_output(
            f"Differences computed: {len(results['differences'])}", "info"
        )
...
class AnalysisThread(QtCore.QThread):
    ...
    def run(self):
        ...
        try:
            ...
            groups_data, template_img, groups_ids = (
                nifti.load_grouped_subjects_ti_toolbox(
            ...
            for group_name, data_4d in groups_data.items():
                ...
                self.output_signal.emit(
                    f"  • {group_name}: {data_4d.shape[-1]} subjects ({', '.join(subject_ids)})"
                )
            ...
            for group_name, data_4d in groups_data.items():
                ...
                img = nib.Nifti1Image(
                    avg_data, template_img.affine, template_img.header
                )
            ...
            if diff_pairs_str:
                ...
                for pair in pairs:
                    ...
                    # Compute difference
                    diff_data = (
                        group_averages[group1] - group_averages[group2]
                    ).astype(np.float32)
                    ...
                    img = nib.Nifti1Image(
                        diff_data,
                        group_images[group1].affine,
                        group_images[group1].header,
                    )
                    nib.save(img, output_path)
                ...
                json.dump(
                    {
                        ...
                        "groups": {
                            group: groups_ids[group] for group in groups_data.keys()
                        },
