...
class SubjectRow(QtWidgets.QWidget):
    ...
    def get_config(self):
        ...
        if self.mode == "classification":
            config["response"] = (
                1 if self.response_combo.currentText() == "Responder" else 0
            )
        else:  # correlation
            try:
...
class ClusterPermutationWidget(QtWidgets.QWidget):
    ...
    def setup_ui(self):
        ...
        toolbar_layout.addSpacing(
            20
        )  # Add some space between mode selection and buttons
        ...
        if ConsoleWidget and RunStopButtons:
            ...
            self.action_buttons = RunStopButtons(
                parent=self, run_text="Run Analysis", stop_text="Stop"
            )
        ...
        self.classification_radio.toggled.connect(self.on_mode_changed)
        self.correlation_radio.toggled.connect(self.on_mode_changed)
        self.classification_radio.setChecked(
            True
        )  # Default to classification (after connecting signals)
    ...
    def on_mode_changed(self):
        ...
        for row in self.subject_rows[:]:  # Copy list to avoid issues during iteration
            ...
            self.subjects_container_layout.insertWidget(len(self.subject_rows), new_row)
    ...
    def setup_config_ui(self):
        ...
        if mode == "classification":
            self.analysis_name_edit.setPlaceholderText(
                "e.g., hippocampus_responders_vs_nonresponders"
            )
        else:
            self.analysis_name_edit.setPlaceholderText(
                "e.g., hippocampus_effect_size_correlation"
            )
        ...
        self.nifti_pattern_edit.setText(
            "grey_{simulation_name}_TI_MNI_MNI_TI_max.nii.gz"
        )
        self.nifti_pattern_edit.setToolTip(
            "Use {simulation_name} as a variable (subject is in directory path)"
        )
        ...
        if mode == "classification":
            ...
            self.alternative_combo.setToolTip(
                "Two-sided: ≠, Greater: Resp > Non-Resp, Less: Resp < Non-Resp"
            )
            ...
            self.correlation_alternative_combo.addItems(
                ["Two-sided", "Greater (Positive)", "Less (Negative)"]
            )
            ...
            self.config_layout.addWidget(
                self.correlation_alternative_combo, row, 1, 1, 3
            )
            row += 1
        ...
        self.n_jobs_edit.setPlaceholderText(
            f"available cores: 1 to {max_cores} (all cores)"
        )
        # Set up input validation - only integers from 1 to max_cores
        self.n_jobs_edit.setValidator(
            QtGui.QIntValidator(1, max_cores, self.n_jobs_edit)
        )
    ...
    def add_subject_row(self):
        ...
        self.subjects_container_layout.insertWidget(len(self.subject_rows), row)
    ...
    def import_from_csv(self):
        ...
        file_path, _ = QtWidgets.QFileDialog.getOpenFileName(
            self, "Import Subject Configuration", initial_dir, "CSV Files (*.csv)"
        )
        ...
        try:
            ...
            self.update_output(
                f"Imported {len(self.subject_rows)} subjects from CSV", "success"
            )

        except Exception as e:
            QtWidgets.QMessageBox.critical(
                self, "Import Error", f"Failed to import CSV: {str(e)}"
            )
    ...
    def export_to_csv(self):
        ...
        if not self.subject_rows:
            QtWidgets.QMessageBox.warning(
                self, "No Data", "No subjects configured to export."
            )
            return

        file_path, _ = QtWidgets.QFileDialog.getSaveFileName(
            self, "Export Subject Configuration", "", "CSV Files (*.csv)"
        )
        ...
        try:
            ...
            self.update_output(
                f"Exported {len(self.subject_rows)} subjects to CSV", "success"
            )
    ...
    def run_analysis(self):
        ...
        if not self.analysis_name_edit.text().strip():
            QtWidgets.QMessageBox.warning(
                self, "Missing Input", "Please enter an analysis name."
            )
            return

        if len(self.subject_rows) < 2:
            QtWidgets.QMessageBox.warning(
                self, "Insufficient Data", "Please add at least 2 subjects."
            )
            return
        ...
        if (
            ...
            self.parent_window.set_tab_busy(
                self, True, stop_btn=stop_btn, keep_enabled=keep_enabled
            )
        ...
        self.worker_thread = AnalysisThread(
            subject_configs, analysis_name, config, console_widget
        )
    ...
    def on_finished(self, results):
        ...
        self.update_output(
            f"Significant clusters: {results['n_significant_clusters']}", "info"
        )
        self.update_output(
            f"Significant voxels: {results['n_significant_voxels']}", "info"
        )
        self.update_output(
            f"Analysis time: {results['analysis_time']:.1f} seconds", "info"
        )
        ...
        if not hasattr(self, "_toolbar_buttons"):
            ...
            for i in range(self.subjects_group.layout().itemAt(0).layout().count()):
                ...
                if (
                    item
                    and item.widget()
                    and isinstance(item.widget(), QtWidgets.QPushButton)
                ):
