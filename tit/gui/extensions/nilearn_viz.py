...
class PublicationImageWorker(QtCore.QThread):
    ...
    def run(self):
        try:
            ...
            self.output_signal.emit(
                f"Cutoff range: {self.min_cutoff:.2f} - {self.max_cutoff:.2f} V/m, Atlas: {self.atlas_name}"
            )
            ...
            try:
                ...
                self.output_signal.emit(
                    f"Successfully loaded and averaged data from {len(subject_ids)} subjects"
                )
                ...
                if self.use_percentiles:
                    ...
                    if len(data_nonzero) == 0:
                        self.error_signal.emit(
                            "No non-zero field values found for percentile calculation"
                        )
                        return
                    ...
                    self.output_signal.emit(
                        f"  {self.min_cutoff:.1f}% → {actual_min_cutoff:.2f} V/m (min cutoff)"
                    )
                    self.output_signal.emit(
                        f"  {self.max_cutoff:.1f}% → {actual_max_cutoff:.2f} V/m (max cutoff)"
                    )
            ...
            with open(txt_filepath, "w") as f:
                ...
                for i, config in enumerate(subject_configs, 1):
                    f.write(
                        f"{i}. Subject: {config['subject_id']}, Simulation: {config['simulation_name']}\n"
                    )
                f.write("\n")
                f.write("PARAMETERS:\n")
                f.write("-" * 12 + "\n")
                if self.use_percentiles:
                    f.write(
                        f"Minimum Cutoff: {self.min_cutoff:.1f}% ({actual_min_cutoff:.2f} V/m)\n"
                    )
                    f.write(
                        f"Maximum Cutoff: {self.max_cutoff:.1f}% ({actual_max_cutoff:.2f} V/m)\n"
                    )
                else:
                ...
                if len(data_nonzero) > 0:
                    ...
                    f.write(
                        f"Data Percentiles: 50th={p50:.2f}, 75th={p75:.2f}, 90th={p90:.2f}, 95th={p95:.2f}, 99th={p99:.2f} V/m\n"
                    )
                if self.selected_regions:
                    f.write(
                        f"Selected Regions: {', '.join(map(str, self.selected_regions))}\n"
                    )
                else:
            ...
            if result or glass_brain_result:
                ...
                self._update_txt_file_with_stats(
                    txt_filepath,
                    averaged_img,
                ...
                self.error_signal.emit(
                    "Visualization failed - check console output above"
                )
    ...
    def _update_txt_file_with_stats(
        self,
        txt_filepath,
        averaged_img,
        use_percentiles,
...
class PublicationImageDialog(QtWidgets.QDialog):
    ...
    def _setup_ui(self):
        ...
        self.group_selection_widget = QtWidgets.QGroupBox(
            "Subject-Simulation Selection"
        )
        ...
        self.subdir_edit.setPlaceholderText(
            "enter name for directory /derivatives/ti-toolbox/nilearn_visuals/{name}"
        )
        self.subdir_edit.setToolTip(
            "Files will be saved in derivatives/ti-toolbox/nilearn_visuals/[sub-directory]/"
        )
        ...
        self.percentile_checkbox.setToolTip(
            "When checked, cutoff values are treated as percentiles (0-100%) from maximum field value"
        )
        ...
        self.atlas_combo.addItems(
            ["harvard_oxford_sub", "harvard_oxford", "aal", "schaefer_2018"]
        )
        ...
        self.action_buttons = RunStopButtons(
            self, run_text="Generate Images", stop_text="Stop"
        )
        ...
        try:
            ...
            if simulations:
                ...
                self.console.update_console(
                    f"Found {sim_count} simulation(s) for subject {subject_id}", "info"
                )
            else:
                self.console.update_console(
                    f"No simulations found for subject {subject_id}", "warning"
                )

        except Exception as e:
            self.console.update_console(
                f"Error loading simulations for {subject_id}: {str(e)}", "error"
            )
                ...
                for i, label in enumerate(labels):
                    if label and label.lower() not in ["background", "unknown", ""]:
                        self.region_combo.addItem(
                            label, i
                        )  # Regions are 0-indexed in atlas data

        except Exception as e:
            self.console.update_console(
                f"Error loading atlas regions: {str(e)}", "warning"
            )
    ...
    def _run_visualization(self):
        ...
        selected_regions = (
            [selected_region_data] if selected_region_data is not None else None
        )
        ...
        if not subject_simulation_pairs:
            self.console.update_console(
                "Please add at least one subject-simulation pair", "error"
            )
            return
        ...
        if len(subject_simulation_pairs) == 1:
            ...
            self.console.update_console(
                f"Simulation: {pair['simulation_name']}", "info"
            )
        else:
            self.console.update_console(
                f"Averaging {len(subject_simulation_pairs)} subject-simulation pairs:",
                "info",
            )
            for i, pair in enumerate(subject_simulation_pairs, 1):
                self.console.update_console(
                    f"  {i}. Subject: {pair['subject_id']}, Simulation: {pair['simulation_name']}",
                    "info",
                )
        ...
        if use_percentiles:
            self.console.update_console(
                f"Cutoff percentiles: {min_cutoff:.1f}% - {max_cutoff:.1f}%", "info"
            )
        else:
            self.console.update_console(
                f"Cutoff range: {min_cutoff:.2f} - {max_cutoff:.2f} V/m", "info"
            )

        self.console.update_console(
            f"Output sub-directory: nilearn_visuals/{subdir_name}", "info"
        )
        ...
        glass_brain_cmap = "hot"
        self.console.update_console(
            "Glass brain visualization: Enabled (colormap: hot)", "info"
        )
        ...
        # Start worker thread
        self.worker = PublicationImageWorker(
            subject_simulation_pairs,
            min_cutoff,
            max_cutoff,
            atlas_name,
        ...
        self.worker.output_signal.connect(
            lambda text: self.console.update_console(text, "default")
        )
    ...
    def _on_worker_finished(self, exit_code):
        ...
        if exit_code == 0:
            self.console.update_console(
                "Publication image generation completed successfully!", "success"
            )
        else:
            self.console.update_console(
                f"Publication image generation failed with exit code {exit_code}",
                "error",
            )
