...
def map_electrodes_to_net(optimized_positions, net_positions, net_labels, channel_array_indices):
    """
...
def print_mapping_summary(mapping_result):
    ...
    avg_distance = total_distance / len(mapping_result['distances']) if mapping_result['distances'] else 0
    max_distance = max(mapping_result['distances']) if mapping_result['distances'] else 0
    ...
    print("\nDetailed mapping:")
    print("-"*80)
    print(f"{'Idx':<5} {'Channel':<8} {'Array':<6} {'Mapped Label':<15} {'Distance (mm)':<15}")
    print("-"*80)

    for i, (label, dist, indices) in enumerate(zip(
...
Examples:
  ...
  %(prog)s -i electrode_positions.json -n EGI_template.csv -o electrode_mapping.json -v
    ...
    parser.add_argument(
        '-v', '--verbose',
        action='store_true',
        help='Print detailed mapping summary'
    )
    ...
    try:
        optimized_positions, channel_array_indices = load_electrode_positions_json(args.input)
