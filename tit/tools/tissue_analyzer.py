...
class TissueAnalyzer(ABC):
    ...
    def _load_label_names(self):
        ...
        for lut_path in possible_lut_paths:
            try:
                ...
                LOGGER.info(
                    f"Successfully loaded {len(label_names)} label names from: {lut_path}"
                )
                return label_names
        ...
        LOGGER.warning(
            "Could not find or load labeling_LUT.txt file. Labels will be shown as numbers only."
        )
    ...
    def extract_tissue_mask(self):
        ...
        if brain_voxels == 0:
            LOGGER.warning(
                f"No brain tissue found for filtering. Using all {self.tissue_name}."
            )
            self.all_tissue_mask = tissue_mask
            self.brain_mask = np.zeros_like(tissue_mask)
            self.tissue_mask = tissue_mask
        ...
        LOGGER.debug(
            f"Filtered out {filtered_ratio:.1f}% of total {self.tissue_name} (kept {100-filtered_ratio:.1f}%)"
        )
        LOGGER.debug(
            f"Voxel counts - Total {self.tissue_name}: {total_tissue_voxels:,}, Filtered: {filtered_tissue_voxels:,}, Excluded: {total_tissue_voxels - filtered_tissue_voxels:,}"
        )
    ...
    def _filter_to_brain_region(self, tissue_mask, brain_mask):
        ...
        LOGGER.debug(
            f"Brain bounding box: X({min_x}-{max_x}), Y({min_y}-{max_y}), Z({min_z}-{max_z})"
        )
        ...
        LOGGER.debug(
            f"Filtered region box: X({region_min_x}-{region_max_x}), Y({region_min_y}-{region_max_y}), Z({region_min_z}-{region_max_z})"
        )
        ...
        LOGGER.debug(
            f"Applied Z-coordinate filter: excluding regions below Z={z_threshold}"
        )
    ...
    def _print_voxel_summary(self):
        ...
        LOGGER.debug("-" * 60)
        LOGGER.debug(
            f"{self.tissue_name} labels analyzed:           {self.tissue_labels}"
        )
        LOGGER.debug(
            f"Total {self.tissue_name} voxels:              {np.sum(self.all_tissue_mask):,}"
        )
        LOGGER.debug(
            f"Filtered {self.tissue_name} voxels:           {np.sum(self.tissue_mask):,}"
        )
        LOGGER.debug(
            f"Excluded {self.tissue_name} voxels:           {np.sum(self.all_tissue_mask) - np.sum(self.tissue_mask):,}"
        )
        ...
        for label in self.tissue_labels:
            ...
            filtered_label_voxels = np.sum(
                (self.data == label) & (self.tissue_mask > 0)
            )
            label_name = self.get_label_name(label)
            LOGGER.debug(
                f"  Label {label:2d} ({label_name}): {label_voxels:8,} total, {filtered_label_voxels:8,} filtered"
            )

        LOGGER.debug("-" * 60)
        LOGGER.debug(
            f"{self.tissue_name} filtering efficiency:      {np.sum(self.tissue_mask) / np.sum(self.all_tissue_mask) * 100:.1f}%"
        )
        LOGGER.debug(
            f"Filtered {self.tissue_name} volume:           {np.sum(self.tissue_mask) * self.voxel_volume / 1000:.3f} cm³"
        )
        LOGGER.debug("=" * 60)
    ...
    def calculate_volumes(self):
        ...
        LOGGER.debug(
            f"[{self.tissue_name} Analysis]   - Tissue mask voxels: {np.sum(self.tissue_mask):,}"
        )
        LOGGER.debug(
            f"[{self.tissue_name} Analysis]   - Voxel volume: {self.voxel_volume:.3f} mm³"
        )
        LOGGER.debug(
            f"[{self.tissue_name} Analysis]   - Total volume: {tissue_volume:.1f} mm³ ({tissue_volume_cm3:.3f} cm³)"
        )
    ...
    def calculate_thickness_3d(self, mask, tissue_name):
        ...
        if np.sum(mask) == 0:
            LOGGER.debug(
                f"[{self.tissue_name} Analysis] No tissue mask data for thickness calculation"
            )
            return {"max": 0, "min": 0, "mean": 0, "std": 0, "thickness_map": None}

        LOGGER.debug(
            f"[{self.tissue_name} Analysis] Calculating thickness using 3D distance transform..."
        )
        LOGGER.debug(
            f"[{self.tissue_name} Analysis]   - Input mask voxels: {np.sum(mask):,}"
        )
        LOGGER.debug(
            f"[{self.tissue_name} Analysis]   - Voxel dimensions: {self.voxel_dims} mm"
        )
        ...
        LOGGER.debug(
            f"[{self.tissue_name} Analysis]   - Raw distance values: {len(thickness_values):,} points"
        )
        LOGGER.debug(
            f"[{self.tissue_name} Analysis]   - Distance range: {np.min(distance_map[mask > 0]):.3f} - {np.max(distance_map[mask > 0]):.3f} mm"
        )
        LOGGER.debug(
            f"[{self.tissue_name} Analysis]   - Thickness range: {np.min(thickness_values):.3f} - {np.max(thickness_values):.3f} mm"
        )
    ...
    def create_thickness_visualization(self, thickness_map, mask, tissue_name, stats):
        ...
        masked_thickness = np.where(
            mask[:, :, mid_z] > 0, thickness_map[:, :, mid_z], np.nan
        )
        aspect_ratio = vy / vx
        im1 = ax1.imshow(
            masked_thickness.T,
        ...
        masked_thickness = np.where(
            mask[:, mid_y, :] > 0, thickness_map[:, mid_y, :], np.nan
        )
        aspect_ratio = vz / vx
        im2 = ax2.imshow(
            masked_thickness.T,
        ...
        masked_thickness = np.where(
            mask[mid_x, :, :] > 0, thickness_map[mid_x, :, :], np.nan
        )
        aspect_ratio = vy / vz
        im3 = ax3.imshow(
            masked_thickness.T,
        ...
        sm = plt.cm.ScalarMappable(
            cmap=self.color_scheme, norm=plt.Normalize(vmin=vmin, vmax=vmax)
        )
        sm.set_array([])
        cbar = fig.colorbar(sm, cax=cbar_ax)
        cbar.set_label(
            "Thickness (mm)", rotation=90, fontweight="bold", labelpad=15, fontsize=12
        )
        ...
        ax4.set_xlabel(
            f"{self.tissue_name} Thickness (mm)", fontweight="bold", fontsize=12
        )
        ax4.set_ylabel("Probability Density", fontweight="bold", fontsize=12)
        ax4.set_title(
            "D. Thickness Distribution", fontsize=14, fontweight="bold", pad=15
        )
        ax4.legend(
...
Voxel dimensions: {self.voxel_dims} mm"""
        ...
        plt.tight_layout()
        plt.subplots_adjust(
            left=0.05, right=0.90, top=0.90, bottom=0.08, wspace=0.15, hspace=0.30
        )

        # Save the figure in both PNG and PDF formats
        output_path_png = (
            self.output_dir / f"{self.tissue_name.lower()}_thickness_analysis.png"
        )
        output_path_pdf = (
            self.output_dir / f"{self.tissue_name.lower()}_thickness_analysis.pdf"
        )
        ...
        LOGGER.debug(
            f"Thickness visualization saved to: {output_path_png} and {output_path_pdf}"
        )
        return output_path_png

    def create_methodology_illustration(
        self, all_tissue_mask, brain_mask, filtered_tissue_mask
    ):
            ...
            for col in range(3):
                ...
                if col == 0:  # Axial view (X-Y plane)
                    ...
                    brain_slice = (
                        ...
                        else np.zeros_like(tissue_slice)
                    )
                ...
                if row == 0:  # Brain reference regions
                    ...
                    img[hemisphere_slice.T > 0] = [
                        0.5,
                        0.5,
                        0.5,
                    ]  # Grey for grey matter (hemispheres)
                    img[brainstem_slice.T > 0] = [0, 1, 0]  # Green for brainstem
                    tissue_color_name = {
                        "CSF": "Blue",
                        "Bone": "White",
                        "Skin": "Orange",
                    }.get(self.tissue_name, "Colored")
                    ...
                    img[tissue_slice.T > 0] = (
                        dimmed_color  # Dimmed tissue color for excluded tissue
                    )
                    img[filtered_slice.T > 0] = (
                        self.tissue_color
                    )  # Full tissue color for included tissue
                    img[hemisphere_slice.T > 0] = [
                        0.5,
                        0.5,
                        0.5,
                    ...
                    kept_percentage = (
                        np.sum(filtered_tissue_mask) / np.sum(all_tissue_mask) * 100
                    )
                    tissue_color_name = {
                        "CSF": "Blue",
                        "Bone": "White",
                ...
                if col == 0:
                    ax.text(
                        ...
                        horizontalalignment="left",
                        bbox=dict(
                            boxstyle="round,pad=0.3", facecolor=legend_color, alpha=0.7
                        ),
                    )
        ...
        plt.tight_layout()
        plt.subplots_adjust(
            left=0.05, right=0.98, top=0.91, bottom=0.08, wspace=0.08, hspace=0.25
        )

        # Save the figure in both PNG and PDF formats
        output_path_png = (
            self.output_dir / f"{self.tissue_name.lower()}_extraction_methodology.png"
        )
        output_path_pdf = (
            self.output_dir / f"{self.tissue_name.lower()}_extraction_methodology.pdf"
        )
        ...
        LOGGER.debug(
            f"{self.tissue_name} extraction methodology illustration saved to: {output_path_png} and {output_path_pdf}"
        )
        return output_path_png
    ...
    def create_combined_publication_figure(
        ...
        # Main title
        fig.suptitle(
            f"{self.tissue_name} Analysis Pipeline",
            fontsize=16,
            fontweight="bold",
            y=0.95,
        )
        ...
        for row in range(3):
            ...
            if row == 2:
                ...
                sm = plt.cm.ScalarMappable(
                    cmap=thickness_cmap, norm=plt.Normalize(vmin=vmin, vmax=vmax)
                )
                sm.set_array([])
                cbar = fig.colorbar(sm, cax=cbar_ax)
                cbar.set_label(
            ...
            for col in range(3):
                ...
                if col == 0:  # Axial
                    ...
                    brainstem_slice = brainstem_mask[:, :, mid_z]
                    if row == 2:
                        thickness_slice = np.where(
                            thickness_mask[:, :, mid_z] > 0,
                            thickness_map[:, :, mid_z],
                            np.nan,
                        )
                    aspect_ratio = vy / vx
                ...
                ax.set_title(
                    f"{chr(65+row)}{col+1}. {view_labels[col]} ({row_title})",
                    fontsize=11,
                    fontweight="bold",
                    pad=8,
                )
        ...
        output_path_png = (
            self.output_dir
            / f"{self.tissue_name.lower()}_combined_publication_figure.png"
        )
        output_path_pdf = (
            self.output_dir
            / f"{self.tissue_name.lower()}_combined_publication_figure.pdf"
        )
        ...
        plt.savefig(
            ...
            format="pdf",
            transparent=False,
            metadata={"Creator": "TI-toolbox"},
        )
        plt.close()

        LOGGER.debug(
            f"Combined publication figure saved to: {output_path_png} and {output_path_pdf}"
        )
        return output_path_png
    ...
    def generate_summary_report(self, volume_results, thickness_stats):
        ...
        report_path = (
            self.output_dir / f"{self.tissue_name.lower()}_analysis_summary.txt"
        )
        ...
        with open(report_path, "w") as f:
            ...
            f.write("-" * 20 + "\n")
            f.write(
                f"{self.tissue_name} volume:       {volume_results[f'{self.tissue_name.lower()}_volume_cm3']:.3f} cm³ ({volume_results[f'{self.tissue_name.lower()}_volume_mm3']:.1f} mm³)\n"
            )
            f.write(
                f"{self.tissue_name} voxels:       {np.sum(self.tissue_mask):,}\n\n"
            )
            ...
            f.write("-" * 18 + "\n")
            f.write(
                f"- Analysis focused on {self.tissue_name} only (lower anatomy excluded)\n"
            )
            f.write(
                "- Spatial filtering based on brain cortex and brain stem locations\n"
            )
            ...
            f.write("-" * 15 + "\n")
            f.write(
                f"- Analyzed {self.tissue_name} tissue with labels {self.tissue_labels}\n"
            )
            f.write("- Thickness calculated using 3D distance transform\n")
            f.write(
                "- Thickness represents twice the distance to nearest tissue boundary\n"
            )
            f.write(
                "- Volume calculations based on voxel dimensions from NIfTI header\n"
            )
            f.write("- Visualizations show middle slices and thickness distributions\n")
            f.write(
                "- Output designed for derivatives/ti-toolbox/{self.tissue_name.lower()}_analysis/sub-*/ structure\n"
            )
    ...
    def run_full_analysis(self):
        ...
        LOGGER.debug(
            f"[{self.tissue_name} Analysis] Output directory: {self.output_dir}"
        )
        LOGGER.debug(
            f"[{self.tissue_name} Analysis] Tissue labels: {self.tissue_labels}"
        )
        LOGGER.debug(
            f"[{self.tissue_name} Analysis] Brain reference labels: {self.brain_labels}"
        )
        LOGGER.debug(
            f"[{self.tissue_name} Analysis] Padding voxels: {self.padding_voxels}"
        )
        ...
        thickness_stats = self.calculate_thickness_3d(
            filtered_tissue_mask, self.tissue_name
        )

        # Log analysis results at DEBUG level
        LOGGER.debug(f"[{self.tissue_name} Analysis] Volume results: {volume_results}")
        LOGGER.debug(
            f"[{self.tissue_name} Analysis] Thickness statistics: {thickness_stats}"
        )

        # Create thickness visualization with histogram
        LOGGER.debug(
            f"[{self.tissue_name} Analysis] Creating thickness visualization..."
        )
        ...
        LOGGER.info(
            f"[{self.tissue_name} Analysis] Complete. Volume: {volume_results[f'{self.tissue_name.lower()}_volume_cm3']:.2f} cm³ | Thickness: {thickness_stats['mean']:.2f}±{thickness_stats['std']:.2f} mm (range: {thickness_stats['min']:.2f}-{thickness_stats['max']:.2f} mm)"
        )
        LOGGER.info(
            f"[{self.tissue_name} Analysis] Voxel summary - Total {self.tissue_name}: {np.sum(self.all_tissue_mask):,}, Filtered: {np.sum(self.tissue_mask):,}, Brain reference: {np.sum(self.brain_mask):,}"
        )
        ...
        for label in self.tissue_labels:
            ...
            label_name = self.get_label_name(label)
            LOGGER.info(
                f"[{self.tissue_name} Analysis]   Label {label:2d} ({label_name}): {label_voxels:,} total, {filtered_label_voxels:,} filtered"
            )

        LOGGER.info(
            f"[{self.tissue_name} Analysis] Results saved to: {self.output_dir}"
        )
        LOGGER.info(
            f"[{self.tissue_name} Analysis] Note: Output is now organized under derivatives/ti-toolbox/{self.tissue_name.lower()}_analysis/sub-*/ structure"
        )

        return {"volumes": volume_results, "thickness_stats": thickness_stats}
...
def main():
    parser = argparse.ArgumentParser(
        description="Unified tissue analysis tool for segmented NIfTI data."
    )
    parser.add_argument("nifti_path", help="Path to the segmented NIfTI file")
    parser.add_argument(
        "-t",
        "--tissue",
        choices=["csf", "bone", "skin"],
        default="csf",
