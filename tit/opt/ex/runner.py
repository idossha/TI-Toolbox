...
class LeadfieldAlgorithms:
    ...
    @staticmethod
    def calculate_roi_metrics_for_field(
        ti_max_full, roi_indices, roi_volumes, gm_indices, gm_volumes
    ):
        return calculate_roi_metrics(
            ti_max_full[roi_indices],
            roi_volumes,
            ti_field_gm=ti_max_full[gm_indices],
            gm_volumes=gm_volumes,
        )


class TIAlgorithms:
    @staticmethod
    def calculate_ti_field(
        leadfield,
        idx_lf,
        roi_indices,
        roi_volumes,
        ...
        ef1 = TI.get_field(
            [e1_plus, e1_minus, current_ch1_mA / 1000], leadfield, idx_lf
        )
        ef2 = TI.get_field(
            [e2_plus, e2_minus, current_ch2_mA / 1000], leadfield, idx_lf
        )
        ti_max_full = TI.get_maxTI(ef1, ef2)

        roi_metrics = LeadfieldAlgorithms.calculate_roi_metrics_for_field(
            ti_max_full, roi_indices, roi_volumes, gm_indices, gm_volumes
        )
...
class LeadfieldProcessor:
    def __init__(self, leadfield_hdf, roi_file, roi_name, logger):
        self.leadfield_hdf, self.roi_file, self.roi_name, self.logger = (
            leadfield_hdf,
            roi_file,
            roi_name,
            logger,
    ...
    def load_leadfield(self):
        ...
        self.leadfield, self.mesh, self.idx_lf, load_time = (
            LeadfieldAlgorithms.load_leadfield(self.leadfield_hdf)
        )
    ...
    def find_roi_elements(self, roi_radius=3.0):
        ...
        self.roi_indices, self.roi_volumes = LeadfieldAlgorithms.find_roi_elements(
            self.mesh, self.roi_coords, roi_radius
        )
    ...
    def find_grey_matter_elements(self):
        ...
        self.gm_indices, self.gm_volumes = (
            LeadfieldAlgorithms.find_grey_matter_elements(self.mesh)
        )
        self.logger.info(f"Found {len(self.gm_indices)} GM elements")

    def calculate_roi_metrics_for_field(self, ti_max_full):
        return LeadfieldAlgorithms.calculate_roi_metrics_for_field(
            ti_max_full,
            self.roi_indices,
            self.roi_volumes,
            self.gm_indices,
...
class CurrentRatioGenerator:
    def __init__(self, total_current, current_step, channel_limit, logger):
        self.total_current, self.current_step, self.channel_limit, self.logger = (
            total_current,
            current_step,
            channel_limit or (total_current / 2.0),
            logger,
        )

    def generate_ratios(self):
        ratios, exceeded = generate_current_ratios(
            self.total_current, self.current_step, self.channel_limit
        )
        if exceeded:
...
class MontageGenerator:
    def __init__(
        ...
        self.e1_plus, self.e1_minus, self.e2_plus, self.e2_minus = (
            e1_plus,
            e1_minus,
            e2_plus,
            e2_minus,
        )
        self.current_ratios, self.all_combinations, self.logger = (
            current_ratios,
            all_combinations,
            logger,
        )
    ...
    def get_total_combinations(self):
        return calculate_total_combinations(
            self.e1_plus,
            self.e1_minus,
            self.e2_plus,
            self.e2_minus,
    ...
    def generate_montages(self):
        return generate_montage_combinations(
            self.e1_plus,
            self.e1_minus,
            self.e2_plus,
            self.e2_minus,
    ...
    def log_configuration_summary(self):
        ...
        self.logger.info(f"\n{'='*80}")
        self.logger.info(
            "Starting TI Calculations"
            + (" (All Combinations)" if self.all_combinations else " (Bucketed)")
        )
        ...
        if self.all_combinations:
            valid = [
                (e1p, e1m, e2p, e2m)
                for e1p, e1m, e2p, e2m in product(self.e1_plus, repeat=4)
                if len(set([e1p, e1m, e2p, e2m])) == 4
            ]
            self.logger.info(
                f"Electrodes: {len(self.e1_plus)} | Valid combinations: {len(valid)}"
            )
        else:
            self.logger.info(
                f"E1+/-: {len(self.e1_plus)}/{len(self.e1_minus)} | E2+/-: {len(self.e2_plus)}/{len(self.e2_minus)}"
            )
...
class TISimulator:
    ...
    def calculate_ti_field(
        self, e1_plus, e1_minus, current_ch1_mA, e2_plus, e2_minus, current_ch2_mA
    ):
        return TIAlgorithms.calculate_ti_field(
            self.leadfield_processor.leadfield,
            self.leadfield_processor.idx_lf,
            self.leadfield_processor.roi_indices,
            self.leadfield_processor.roi_volumes,
            self.leadfield_processor.gm_indices,
            self.leadfield_processor.gm_volumes,
            e1_plus,
            e1_minus,
            current_ch1_mA,
            e2_plus,
...
class SimulationRunner:
    def __init__(self, leadfield_processor, montage_generator, output_dir, logger):
        (
            self.leadfield_processor,
            self.montage_generator,
            self.output_dir,
            self.logger,
    ...
    def create_montage_name(
        self, e1_plus, e1_minus, e2_plus, e2_minus, current_ch1_mA, current_ch2_mA
    ):
    ...
    def run_simulation(self):
        ...
        for processed, (
            ...
            e2_plus,
            e2_minus,
            (current_ch1_mA, current_ch2_mA),
        ) in enumerate(self.montage_generator.generate_montages(), 1):
            if stop_flag.value:
                self.logger.warning("Interrupted")
                break

            montage_name = self.create_montage_name(
            ...
            self.logger.info(
                f"  Progress: {100*processed/total_combinations:.1f}% | Rate: {rate:.2f} montages/sec | ETA: {eta/60:.1f} min"
            )

            try:
                sim_start = time.time()
                result_data = self.ti_simulator.calculate_ti_field(
                ...
                self.logger.info(
                    f"  Completed in {time.time()-sim_start:.2f}s | I1={current_ch1_mA:.1f}mA, I2={current_ch2_mA:.1f}mA | "
        ...
        self.logger.info(
            f"Total time: {total_time/60:.1f} minutes ({total_time/processed:.2f}s per montage)"
        )
