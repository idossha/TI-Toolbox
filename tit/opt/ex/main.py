...
def main():
    ...
    if not project_dir or not subject_name:
        print(
            "\033[0;31mError: PROJECT_DIR and SUBJECT_NAME environment variables must be set\033[0m"
        )
        sys.exit(1)
    ...
    try:
        ...
        env_config, electrodes, currents, all_combinations = (
            config["environment"],
            config["electrodes"],
            config["currents"],
        ...
        net = (
            str(env_config.get("SELECTED_EEG_NET") or "unknown_net").strip()
            or "unknown_net"
        )
        run_name = f"{roi_csv}_{net}"
        output_dir = pm.path(
            "ex_search_run", subject_id=subject_name, run_name=run_name
        )
        ...
        leadfield_processor = LeadfieldProcessor(
            env_config["LEADFIELD_HDF"], roi_file, roi_name, logger
        )
        leadfield_processor.initialize()

        current_generator = CurrentRatioGenerator(
            currents["total_current"],
            currents["current_step"],
            currents["channel_limit"],
            logger,
        )
