...
def _make_stdout_logger() -> logging.Logger:
    ...
    return logging.getLogger("tit.opt.ex_search")
...
def main() -> None:
    ...
    config = deserialize_config(ExConfig, data)
    lock_cm = _hold_locks("ex", [config.subject_id], data)

    exit_code = 1
    try:
        with lock_cm:
            events.emit_stage("ex_search")
