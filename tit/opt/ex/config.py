...
def validate_env():
    ...
    if not os.path.exists(config["LEADFIELD_HDF"]):
        raise ConfigError("Leadfield file not found")
    return config
...
class ElectrodeConfig:
    ...
    def get_config(self, all_combinations):
        ...
        missing_vars = []
        if not e1_plus_env:
            missing_vars.append("E1_PLUS")
        if not e1_minus_env:
            missing_vars.append("E1_MINUS")
        if not e2_plus_env:
            missing_vars.append("E2_PLUS")
        if not e2_minus_env:
            missing_vars.append("E2_MINUS")
        ...
        all_valid = all(
            validate_electrode(e) for channel in electrodes.values() for e in channel
        )
        ...
        if all_combinations:
            ...
            if not all(
                ...
                self.logger.warning(
                    "All combinations mode: using E1_PLUS electrodes for all channels"
                )
            return {
...
class CurrentConfig:
    ...
    def get_param(self, name, default, validation_fn=None):
        ...
        if env_val:
            try:
                ...
                self.logger.warning(f"Invalid {name} in env: {env_val}")
    ...
    def get_config(self):
        ...
        step = self.get_param(
            "current_step", 0.1, lambda x: validate_current(x, 0, total)
        )
...
def parse_args():
    ...
    if all_combinations_env:
        ...
        args.all_combinations = (
            all_combinations_env == "1" or all_combinations_env.lower() == "true"
        )
        return args
