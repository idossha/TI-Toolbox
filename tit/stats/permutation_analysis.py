...
def load_subject_data(
    subject_configs, nifti_file_pattern=None, analysis_type="group_comparison"
):
    """
...
def load_subject_data_group_comparison(subject_configs, nifti_file_pattern=None):
    ...
    responders, template_img, responder_ids = nifti.load_group_data_ti_toolbox(
        responder_configs, nifti_file_pattern=nifti_file_pattern, dtype=np.float32
    )

    # Load non-responders
    non_responders, _, non_responder_ids = nifti.load_group_data_ti_toolbox(
        non_responder_configs, nifti_file_pattern=nifti_file_pattern, dtype=np.float32
    )
...
def load_subject_data_correlation(subject_configs, nifti_file_pattern=None):
    ...
    subject_data, template_img, subject_ids = nifti.load_group_data_ti_toolbox(
        subject_configs, nifti_file_pattern=nifti_file_pattern, dtype=np.float32
    )
    ...
    print(
        f"Effect sizes: mean={np.mean(effect_sizes):.3f}, std={np.std(effect_sizes):.3f}"
    )
    print(
        f"Effect size range: [{np.min(effect_sizes):.3f}, {np.max(effect_sizes):.3f}]"
    )
    if weights is not None:
        print(
            f"Weights: mean={np.mean(weights):.3f}, range=[{np.min(weights):.3f}, {np.max(weights):.3f}]"
        )
    ...
    if analysis_type == "group_comparison":
        ...
        for col in required_cols:
            if col not in df.columns:
                raise ValueError(
                    f"CSV file missing required column: '{col}' for group comparison"
                )
        ...
        for col in required_cols:
            if col not in df.columns:
                raise ValueError(
                    f"CSV file missing required column: '{col}' for correlation analysis"
                )
        ...
        for _, row in df.iterrows():
            ...
            if (
                pd.isna(row["subject_id"])
                or pd.isna(row["simulation_name"])
                or pd.isna(row["effect_size"])
            ):
                skipped_rows += 1
                continue
...
def run_analysis(
    ...
    if pm:
        ...
        output_dir = os.path.join(
            project_dir, "derivatives", "tit", "stats", analysis_type, analysis_name
        )
    ...
    analysis_title = (
        ...
        if analysis_type == "group_comparison"
        else "CORRELATION-BASED CLUSTER PERMUTATION TESTING"
    )
    subtitle = (
        ""
        if analysis_type == "group_comparison"
        else "(ACES-style analysis for continuous outcomes)"
    )
    ...
    if CONFIG.get("nifti_file_pattern") is None:
        ...
        if tissue_type == "grey":
            ...
            raise ValueError(
                f"Invalid tissue_type: '{tissue_type}'. Must be 'grey', 'white', or 'all'"
            )
    ...
    if analysis_type == "group_comparison":
        ...
        logger.info(
            f"  Alternative hypothesis: {alt_text.get(CONFIG['alternative'], CONFIG['alternative'])}"
        )
    else:
        logger.info(f"  Correlation type: {CONFIG['correlation_type'].capitalize()}")

    cluster_stat_name = (
        "Cluster Size" if CONFIG["cluster_stat"] == "size" else "Cluster Mass"
    )
...
def _run_group_comparison_analysis(
    subject_configs,
    CONFIG,
    output_dir,
    logger,
    ...
    logger.info(
        f"Loaded {non_responders.shape[-1]} {CONFIG['group2_name']}: {non_resp_ids}"
    )
    ...
    non_resp_mean = (
        np.mean(non_responders[non_responders > 0]) if np.any(non_responders > 0) else 0
    )
    ...
    logger.info(f"\nDATA DIAGNOSTICS:")
    logger.info(
        f"  Responders - Mean (non-zero): {resp_mean:.6f}, Max: {resp_max:.6f}, Non-zero voxels: {resp_nonzero}"
    )
    logger.info(
        f"  Non-Responders - Mean (non-zero): {non_resp_mean:.6f}, Max: {non_resp_max:.6f}, Non-zero voxels: {non_resp_nonzero}"
    )
    ...
    if t_nonzero > 0:
        ...
        logger.info(
            f"  Mean |t-stat|: {np.mean(np.abs(t_statistics[t_statistics != 0])):.4f}"
        )
    else:
    ...
    if n_observed_clusters > 0:
        ...
        observed_cluster_sizes = np.bincount(
            cluster_labels_flat, minlength=n_observed_clusters + 1
        )[1:]
    ...
    logger.info("-" * 70)
    log_callback(
        f"Running permutation testing ({CONFIG['n_permutations']} permutations)..."
    )
    step_start = time.time()
    ...
    if clusters:
        ...
        logger.info(
            f"MNI center: ({clusters[0]['center_mni'][0]:.1f}, "
    ...
    correlation_plot_file = os.path.join(
        output_dir, "cluster_size_mass_correlation.pdf"
    )
    plot_cluster_size_mass_correlation(
        correlation_data["sizes"], correlation_data["masses"], correlation_plot_file
    )
    ...
    img = nib.Nifti1Image(
        sig_mask.astype(np.uint8), template_img.affine, template_img.header
    )
    ...
    generate_summary(
        ...
        observed_cluster_sizes=(
            observed_cluster_sizes if n_observed_clusters > 0 else None
        ),
    )
...
def _run_correlation_analysis(
    ...
    # Data diagnostics
    data_mean = (
        np.mean(subject_data[subject_data > 0]) if np.any(subject_data > 0) else 0
    )
    ...
    if n_observed_clusters > 0:
        ...
        observed_cluster_masses = ndimage_sum(
            t_statistics, observed_labeled, cluster_indices
        )
        ...
        for i in range(min(10, len(observed_cluster_sizes_sorted))):
            if CONFIG["cluster_stat"] == "size":
                logger.info(
                    f"  Cluster {i+1:2d}: {observed_cluster_sizes_sorted[i]:6d} voxels, mass={observed_cluster_masses_sorted[i]:.2f}"
                )
            else:
                logger.info(
                    f"  Cluster {i+1:2d}: mass={observed_cluster_masses_sorted[i]:.2f}, {observed_cluster_sizes_sorted[i]:6d} voxels"
                )
    ...
    img = nib.Nifti1Image(
        r_values.astype(np.float32), template_img.affine, template_img.header
    )
    ...
    img = nib.Nifti1Image(
        t_statistics.astype(np.float32), template_img.affine, template_img.header
    )
    ...
    img = nib.Nifti1Image(
        r_thresholded.astype(np.float32), template_img.affine, template_img.header
    )
...
Tissue Types:
  --tissue-type controls which NIfTI files are loaded:
    ...
    parser.add_argument(
        "--name", "-n", required=True, help="Analysis name (used for output directory)"
    )
    parser.add_argument(
        "--analysis-type",
        required=True,
