...
# 'fork' can cause issues with libraries that use threads/shared state
try:
    mp_context = multiprocessing.get_context('spawn')
except ValueError:
    # Fallback for systems that don't support spawn
...
def _run_montage_worker(args: dict) -> dict:
    ...
    try:
        # Add parent directory to path for imports (needed in spawned process)
        ...
        from sim.config import SimulationConfig, MontageConfig, ConductivityType, ElectrodeConfig, IntensityConfig, ParallelConfig
        from sim.session_builder import SessionBuilder
        from sim.post_processor import PostProcessor
        from core import get_path_manager
        from tools import logging_util
        ...
        log_dir = os.path.join(derivatives_dir, 'ti-toolbox', 'logs', f'sub-{config.subject_id}')
        ...
        from sim.simulator import _run_single_montage
