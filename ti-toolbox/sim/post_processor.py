...
from sim.config import SimulationMode, MontageConfig
...
class PostProcessor:
    ...
    def _calculate_ti_field(self, hf_dir: str, output_dir: str, montage_name: str) -> str:
        """
        ...
        """
        # Load mesh files
        m1_file = os.path.join(hf_dir, f"{self.subject_id}_TDCS_1_{self.conductivity_type}.msh")
        m2_file = os.path.join(hf_dir, f"{self.subject_id}_TDCS_2_{self.conductivity_type}.msh")
    ...
    def _process_ti_normal(self, hf_dir: str, output_dir: str, montage_name: str):
        ...
        central_1 = os.path.join(overlays_dir, f"{self.subject_id}_TDCS_1_{self.conductivity_type}_central.msh")
        central_2 = os.path.join(overlays_dir, f"{self.subject_id}_TDCS_2_{self.conductivity_type}_central.msh")

        if not (os.path.exists(central_1) and os.path.exists(central_2)):
            self.logger.debug("Central surface meshes not found, skipping TI_normal calculation")
            return
    ...
    def _save_ti_intermediate(self, base_mesh, ti_vectors, output_dir: str, filename: str):
        """
        ...
        mout.view(visible_tags=[1002, 1006], visible_fields=["TI_vectors"]).write_opt(output_path)
    ...
    def _extract_fields(self, input_mesh: str, output_dir: str, base_name: str):
        ...
        """
        self.logger.debug(f"Extracting GM/WM fields from {os.path.basename(input_mesh)}")
        ...
        if os.path.exists(field_extract_script):
            try:
                result = subprocess.run(
                    ['simnibs_python', field_extract_script, input_mesh,
                     '--gm_output_file', gm_output,
                     '--wm_output_file', wm_output],
                    capture_output=True,
                    text=True,
                    timeout=300
                )
    ...
    def _transform_to_nifti(self, mesh_dir: str, output_dir: str):
        ...
        if os.path.exists(mesh2nii_script):
            try:
                result = subprocess.run(
                    ['bash', mesh2nii_script, self.subject_id, self.m2m_dir, mesh_dir, output_dir],
                    capture_output=True,
                    text=True,
                    timeout=600
                )
    ...
    def _direct_nifti_conversion(self, mesh_dir: str, output_dir: str):
        ...
        for mesh_file in mesh_files:
            ...
            try:
                ...
                result = subprocess.run(
                    ['subject2mni', '-i', mesh_file, '-m', self.m2m_dir, '-o', mni_output],
                    capture_output=True,
                    text=True,
                    timeout=300
                )
    ...
    def _organize_ti_files(
        ...
        if os.path.exists(fields_summary):
            self._safe_move(fields_summary, os.path.join(hf_analysis_dir, "fields_summary.txt"))
    ...
    def _safe_move(self, src: str, dest: str):
        ...
        try:
            if os.path.exists(src):
                ...
                self.logger.debug(f"Moved {os.path.basename(src)} to {os.path.dirname(dest)}")
        except Exception as e:
