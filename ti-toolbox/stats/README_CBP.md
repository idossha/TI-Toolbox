...
- Conservative: 5000+ (publication-quality)
...
Test if responders have greater current intensity than non-responders:

```python
config = {
    'alternative': 'greater',  # Test Resp > Non-Resp
    'n_permutations': 5000,    # More permutations for publication
}

results = cluster_permutation.run_analysis(
    subject_configs=subjects,
    analysis_name='hippocampus_greater',
    config=config
)
```

### Example 3: White Matter Analysis
...
A cluster is considered significant if its statistic (size or mass) exceeds the cluster threshold determined by permutation testing at the specified alpha level.

The `analysis_summary.txt` file contains:
- Cluster locations in MNI coordinates
- Cluster sizes and statistics
- Overlap with brain atlases (if configured)
- Statistical test details

### P-value Map

The `pvalues_map.nii.gz` file shows uncorrected p-values in -log10 scale:
...
Load the output files in FSLeyes or another NIfTI viewer:

1. Load `significant_voxels_mask.nii.gz` to see significant regions
2. Overlay `difference_map.nii.gz` to see effect directions
3. Compare `average_responders.nii.gz` and `average_non_responders.nii.gz`
...
1. Check the analysis log file for detailed error messages
2. Review the GUI output console
3. Contact TI-Toolbox support through the Contact tab

## Version History

- **v1.0** (2025): Initial integration with TI-Toolbox
