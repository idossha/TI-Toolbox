...
# When running from CLI, we also want both file and console
...
def create_leadfield(m2m_dir, eeg_cap_path, net_name, suffix='vol'):
    ...
    # Create output directory inside ex-search subdirectory: ex-search/leadfield_vol_{net_name}
    ex_search_dir = os.path.join(subject_bids_dir, "leadfields")
    os.makedirs(ex_search_dir, exist_ok=True)
    output_dir = os.path.join(ex_search_dir, f"leadfield_{suffix}_{net_name}")
