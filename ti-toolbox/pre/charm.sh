...
run_command() {
    ...
    # Debug output to verify DEBUG_MODE is being read correctly
    echo "[DEBUG] run_command: DEBUG_MODE=$DEBUG_MODE, debug_mode=$debug_mode" >&2
    
    # Run cmd with real-time output
    local temp_output=$(mktemp)
    local exit_code=0
    
    # Always tee to both temp file and log file for real-time logging
    # In debug mode, also show on console; in summary mode, only log to files
    if [ "$debug_mode" = "true" ]; then
        # Debug mode: show all output to console AND log files
        echo "[DEBUG] Running in DEBUG mode - output will be shown on console" >&2
        if ! eval "$cmd" 2>&1 | stdbuf -oL -eL tee "$temp_output" | stdbuf -oL -eL tee -a "$LOG_FILE"; then
            exit_code=1
        fi
    else
        # Summary mode: only log to files (no console output), but still real-time to log
        echo "[DEBUG] Running in SUMMARY mode - output will only go to log file" >&2
        if ! eval "$cmd" 2>&1 | stdbuf -oL -eL tee "$temp_output" >> "$LOG_FILE"; then
            exit_code=1
        fi
    fi
