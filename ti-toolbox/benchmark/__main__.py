...
Usage:
    python -m ti_toolbox.benchmark         # Show help
    python -m ti_toolbox.benchmark charm   # Run charm benchmark
    python -m ti_toolbox.benchmark --help  # Show help
"""
...
Examples:
  ...
  python -m ti_toolbox.benchmark charm --no-debug
  
  # Show help for specific benchmark
  python -m ti_toolbox.benchmark charm --help
"""
    )
    ...
    if args.benchmark == "charm":
        from benchmark.charm import main as charm_main
        sys.argv = [sys.argv[0]] + remaining
        charm_main()
    elif args.benchmark == "recon":
        from benchmark.recon import main as recon_main
        sys.argv = [sys.argv[0]] + remaining
        recon_main()
    elif args.benchmark == "dicom":
        from benchmark.dicom import main as dicom_main
        sys.argv = [sys.argv[0]] + remaining
        dicom_main()
    elif args.benchmark == "flex":
        from benchmark.flex import main as flex_main
        sys.argv = [sys.argv[0]] + remaining
        flex_main()
    else:
        parser.print_help()
