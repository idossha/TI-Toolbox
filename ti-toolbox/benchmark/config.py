...
class BenchmarkConfig:
    ...
    def _find_default_config(self) -> Optional[Path]:
        ...
        search_paths = [
            ...
            Path.home() / ".ti-toolbox" / self.DEFAULT_CONFIG_FILENAME,
        ]
    ...
    def _apply_defaults(self):
        ...
        if 'charm_script' not in charm:
            charm['charm_script'] = str(toolbox_root / "ti-toolbox" / "pre" / "charm.sh")
        if 'project_dir' not in charm:
            charm['project_dir'] = str(Path("/mnt/ti-toolbox-benchmark") if os.path.exists("/mnt") 
                                       else Path("/tmp/ti-toolbox-benchmark"))
        ...
        if 'recon_script' not in recon:
            recon['recon_script'] = str(toolbox_root / "ti-toolbox" / "pre" / "recon-all.sh")
        if 'project_dir' not in recon:
            recon['project_dir'] = str(Path("/mnt/ti-toolbox-benchmark-recon") if os.path.exists("/mnt")
                                       else Path("/tmp/ti-toolbox-benchmark-recon"))
        ...
        if 'dicom_script' not in dicom:
            dicom['dicom_script'] = str(toolbox_root / "ti-toolbox" / "pre" / "dicom2nifti.sh")
        ...
        if 'project_dir' not in flex:
            flex['project_dir'] = str(Path("/mnt/ti-toolbox-benchmark-flex") if os.path.exists("/mnt")
                                      else Path("/tmp/ti-toolbox-benchmark-flex"))
    ...
    def save_example_config(self, output_path: Path):
        ...
        example_config = {
            ...
            'debug_mode': True,
            'charm': {
                'project_dir': '/tmp/ti-toolbox-benchmark',
            ...
            'recon': {
                ...
                'ernie_data': '/path/to/resources/example_data/ernie',
                'recon_script': '/path/to/ti-toolbox/pre/recon-all.sh',
                'parallel': False,
                'clean': False
            },
            'dicom': {
...
Examples:
  ...
  python -m ti_toolbox.benchmark.config --show --config my_config.yaml
"""
    )
    ...
    if args.generate:
        ...
        print("  2. Home directory: ~/.ti-toolbox/benchmark_config.yaml")
