...
class TestUtilityFunctions:
    ...
    def test_validate_coordinates(self):
        ...
        with pytest.raises(
            ValueError, match="Coordinates must be exactly three values"
        ):
...
class TestLoggingFunctions:
    ...
    def test_log_analysis_complete(self, mock_logger):
        ...
        with patch("time.time", return_value=1010.0):
            log_analysis_complete(
                "spherical", "test_subject", "1 region", "/path/output"
            )
    ...
    def test_log_analysis_complete_summary_mode(self, mock_print):
        ...
        expected_calls = [
            call(
                "└─ Analysis completed successfully for subject: test_subject (1 region, Total: 10s)"
            ),
    ...
    def test_log_analysis_step_complete_summary_mode(self, mock_print):
        ...
        mock_print.assert_called_once_with(
            "├─ Field loading: ✓ Complete (10s) - 5 voxels"
        )
        assert "test_subject_Field loading" not in ma._start_times
    ...
    def test_log_analysis_step_failed(self, mock_logger):
        ...
        mock_logger.error.assert_called_once_with(
            "Field loading: Failed (10s) - File not found"
        )
        assert "test_subject_Field loading" not in ma._start_times
...
class TestMeshFieldPathConstruction:
    ...
    @patch("os.path.exists")
    @patch("os.path.basename")
    def test_construct_mesh_field_path_montage_variations(
        self, mock_basename, mock_exists
    ):
        ...
        result = construct_mesh_field_path(
            "/path/to/m2m_subject", "test_montage_TINormal"
        )
...
class TestArgumentValidation:
    ...
    def test_validate_args_mesh_spherical_valid(self, mock_exists, mock_isdir):
        ...
        with patch(
            "tit.analyzer.main_analyzer.construct_mesh_field_path",
            return_value="/path/to/field.msh",
        ):
            with patch("tit.analyzer.main_analyzer.validate_file_extension"):
                with patch(
                    "tit.analyzer.main_analyzer.validate_coordinates",
                    return_value=[10.0, 20.0, 30.0],
                ):
                    with patch(
                        "tit.analyzer.main_analyzer.validate_radius", return_value=5.0
                    ):
    ...
    def test_validate_args_mesh_missing_montage(self, mock_exists, mock_isdir):
        ...
        with pytest.raises(
            ValueError, match="--montage_name is required for mesh analysis"
        ):
            validate_args(args)
    ...
    def test_validate_args_voxel_missing_field_path(self, mock_isdir):
        ...
        with pytest.raises(
            ValueError, match="--field_path is required for voxel analysis"
        ):
            validate_args(args)
    ...
    def test_validate_args_cortical_mesh_missing_atlas(self, mock_exists, mock_isdir):
        ...
        with patch(
            ...
            with pytest.raises(
                ValueError,
                match="Atlas name is required for mesh-based cortical analysis",
            ):
                validate_args(args)
    ...
    def test_validate_args_cortical_voxel_missing_atlas(self, mock_exists, mock_isdir):
        ...
        with pytest.raises(
            ValueError, match="Atlas path is required for voxel-based cortical analysis"
        ):
            validate_args(args)

    @patch("os.path.isdir")
    @patch("os.path.exists")
    def test_validate_args_cortical_missing_region_and_whole_head(
        ...
        with patch(
            ...
            with pytest.raises(
                ValueError,
                match="Either --whole_head flag or --region must be specified",
            ):
                validate_args(args)
...
class TestMainFunction:
    ...
    def test_main_mesh_spherical_analysis(
        self,
        mock_setup_parser,
        mock_get_logger,
        mock_voxel_analyzer,
        ...
        mock_analyzer_instance.analyze_sphere.assert_called_once_with(
            center_coordinates=[10.0, 20.0, 30.0], radius=5.0, visualize=False
        )
    ...
    @patch("tit.analyzer.main_analyzer.logging_util.get_logger")
    @patch("tit.analyzer.main_analyzer.setup_parser")
    def test_main_voxel_cortical_analysis(
        self,
        mock_setup_parser,
        mock_get_logger,
        mock_voxel_analyzer,
    ...
    @patch("tit.analyzer.main_analyzer.logging_util.get_logger")
    @patch("tit.analyzer.main_analyzer.setup_parser")
    def test_main_voxel_cortical_whole_head(
        self,
        mock_setup_parser,
        mock_get_logger,
        mock_voxel_analyzer,
        ...
        mock_analyzer_instance.analyze_whole_head.assert_called_once_with(
            atlas_file="/path/to/atlas.nii.gz", visualize=False
        )
    ...
    @patch("tit.analyzer.main_analyzer.logging_util.get_logger")
    @patch("tit.analyzer.main_analyzer.setup_parser")
    def test_main_whole_head_analysis(
        self,
        mock_setup_parser,
        mock_get_logger,
        mock_voxel_analyzer,
    ...
    def test_main_quiet_mode(
        ...
        assert any(
            "Beginning analysis for subject" in str(call)
    ...
    @patch("tit.analyzer.main_analyzer.logging_util.get_logger")
    @patch("tit.analyzer.main_analyzer.setup_parser")
    def test_main_analyzer_initialization_failure(
        self,
        mock_setup_parser,
        mock_get_logger,
        mock_voxel_analyzer,
