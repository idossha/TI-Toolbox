...
def test_analyzer_direct_builds_expected_argv(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(
        analyzer_cli.utils, "run_main_with_argv", fake_run_main_with_argv
    )
    ...
    assert (
        "--analysis_type" in argv
        and argv[argv.index("--analysis_type") + 1] == "spherical"
    )
...
def test_group_analyzer_from_project_builds_subject_specs(monkeypatch, tmp_path: Path):
    ...
    for sid in ["101", "102"]:
        (proj / "derivatives" / "SimNIBS" / f"sub-{sid}" / f"m2m_{sid}").mkdir(
...
def test_create_leadfield_direct_delegates(monkeypatch, tmp_path: Path):
    ...
    (
        tmp_path / "derivatives" / "SimNIBS" / "sub-101" / "m2m_101" / "eeg_positions"
    ).mkdir(parents=True, exist_ok=True)
    cap = (
        tmp_path
        / "derivatives"
    ...
    class DummyGen:
        ...
        def generate_leadfield(
            self, output_dir=None, tissues=None, eeg_cap_path=None, cleanup=True
        ):
...
def test_ex_search_direct_sets_env_and_calls(monkeypatch, tmp_path: Path):
    ...
    (eeg_pos / "GSN-HydroCel-185").write_text(
        "x,y,z,label\n0,0,0,F3\n0,0,0,F4\n0,0,0,P3\n0,0,0,P4\n"
    )
...
def test_cluster_permutation_direct_delegates(monkeypatch, tmp_path: Path):
    ...
    rc = _run_cli(
        cli, ["--csv", str(csv), "--name", "a", "--analysis-type", "group_comparison"]
    )
    assert rc == 0
...
def test_simulator_direct_delegates_to_run_simulation(monkeypatch, tmp_path: Path):
    ...
    (proj / "derivatives" / "SimNIBS" / "sub-101" / "m2m_101").mkdir(
        parents=True, exist_ok=True
    )
...
def test_simulator_direct_comprehensive_argument_coverage(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(
        "tit.sim.montage_loader.list_montage_names",
        lambda *args, **kwargs: ["test_montage"],
    )
    ...
    captured_calls.clear()
    flex_dir = (
        proj / "derivatives" / "SimNIBS" / "sub-101" / "flex-search" / "test_search"
    )
    flex_dir.mkdir(parents=True, exist_ok=True)
    (flex_dir / "electrode_positions.json").write_text(
        '{"optimized_positions": [[0,0,0], [10,10,10], [20,20,20], [30,30,30]]}'
    )
...
def test_analyzer_direct_comprehensive_argument_coverage(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(
        "tit.analyzer.main_analyzer.main",
        lambda: captured_calls.append("analyzer_main_called"),
    )
    monkeypatch.setattr(
        "tit.analyzer.field_selector.select_field_file", fake_select_field_file
    )
    monkeypatch.setattr(
        "tit.cli.analyzer.utils.run_main_with_argv", fake_run_main_with_argv
    )
...
def test_group_analyzer_direct_comprehensive_argument_coverage(
    ...
    # Mock the actual functions that get called
    monkeypatch.setattr(
        "tit.cli.group_analyzer._run_group_analyzer_with_argv", fake_run_group_analyzer
    )
    monkeypatch.setattr(
        "tit.analyzer.field_selector.select_field_file", fake_select_field_file
    )
...
def test_create_leadfield_direct_comprehensive_argument_coverage(
    monkeypatch, tmp_path: Path
):
        ...
        class MockGen:
            def generate_leadfield(
                ...
                captured_calls.append(
                    ("generate_leadfield", str(output_dir), tissues, str(eeg_cap_path))
                )
                from pathlib import Path
    ...
    monkeypatch.setattr(
        "tit.opt.leadfield.LeadfieldGenerator", mock_leadfield_generator
    )
    ...
    assert rc == 0
    assert len(captured_calls) == 1
    assert captured_calls[0][2] == [
        1,
        2,
    ]  # Default tissues (index 2: tissues parameter)

    # Test 2: Custom tissues
    captured_calls.clear()
    rc = _run_cli(
        cli, ["--sub", "101", "--eeg", "GSN-HydroCel-185", "--tissues", "1", "2", "3"]
    )
    assert rc == 0
    ...
    captured_calls.clear()
    rc = _run_cli(
        cli, ["--sub", "101", "--eeg", "GSN-HydroCel-185.csv", "--tissues", "1,2"]
    )
    assert rc == 0
...
def test_pre_process_direct_comprehensive_argument_coverage(
    monkeypatch, tmp_path: Path
):
    ...
    labeling_dir_102 = (
        proj / "derivatives" / "SimNIBS" / "sub-102" / "m2m_102" / "segmentation"
    )
...
def test_vis_blender_direct_comprehensive_argument_coverage(
    monkeypatch, tmp_path: Path
):
    ...
    monkeypatch.setattr(
        "tit.cli.vis_blender._setup_logging_with_file", mock_setup_logging_with_file
    )
...
def test_cluster_permutation_direct_comprehensive_argument_coverage(
    monkeypatch, tmp_path: Path
):
    ...
    monkeypatch.setattr(
        "tit.stats.permutation_analysis.run_analysis", fake_run_analysis
    )
    ...
    csv_file2.write_text(
        "subject_id,simulation_name,response,predictor\n101,simA,1.5,2.3\n102,simA,2.1,1.8\n"
    )
    rc = _run_cli(
        cli,
        [
...
def test_cli_list_commands_no_execution(monkeypatch, tmp_path: Path):
    ...
    (proj / "derivatives" / "SimNIBS" / "sub-101" / "m2m_101" / "eeg_positions").mkdir(
        parents=True, exist_ok=True
    )
    eeg_cap = (
        proj
