...
def _plain_host_not_wsl():
    ...
    A private ``MonkeyPatch``, not the ``monkeypatch`` fixture: requesting that fixture from
    an autouse fixture sets it up before ``_reset_path_manager`` and so tears it down after,
    leaving a test's ``sys.modules`` stubs in place while ``_reset_path_manager`` imports
    ``tit.paths`` (CircleCI 1115: five teardown ImportErrors in tests/smoke)."""
...
def _isolated_cpu_limit(tmp_path_factory):
    """Point the global CPU-limit setting at an empty scratch file, so no test reads (or a PUT
    writes) the developer's real ``~/.config/ti-toolbox/cpu-limit.json`` and every test sees the
    70 % default unless it sets one. Private ``MonkeyPatch`` for the same teardown-order reason as
    ``_plain_host_not_wsl``."""
    import tit.cpu
