...
def mock_simnibs():
    ...
    # Mock mesh_io
    mesh_io = MagicMock()
    mesh_io.read_msh = Mock(side_effect=lambda path: MockMesh({
...
def mock_core_calc():
    ...
    def mock_get_TI_vectors(e1, e2):
        return np.ones((len(e1), 3)) if hasattr(e1, '__len__') else np.array([[1, 0, 0]])
...
def setup_test_structure(tmp_path):
    ...
    sim_dir = tmp_path / "derivatives" / "SimNIBS" / f"sub-{subject_id}" / "Simulations" / "test_montage"
    ...
    (overlays_dir / f"{subject_id}_TDCS_1_scalar_central.msh").write_text("central mesh 1")
    (overlays_dir / f"{subject_id}_TDCS_2_scalar_central.msh").write_text("central mesh 2")
...
class TestPostProcessorInitialization:
    ...
    def test_initialization(self, load_post_processor, mock_logger, setup_test_structure):
    ...
    def test_tools_dir_path(self, load_post_processor, mock_logger, setup_test_structure):
...
class TestPostProcessorTIFileOrganization:
    ...
    def test_organize_ti_files_moves_mesh_files(self, load_post_processor, mock_logger, setup_test_structure):
        ...
        hf_mesh_dir = os.path.join(setup_test_structure['sim_dir'], "high_Frequency_mesh")
        hf_nifti_dir = os.path.join(setup_test_structure['sim_dir'], "high_Frequency_nifti")
        hf_analysis_dir = os.path.join(setup_test_structure['sim_dir'], "high_Frequency_analysis")
        surface_overlays_dir = os.path.join(setup_test_structure['sim_dir'], "surface_overlays")
        documentation_dir = os.path.join(setup_test_structure['sim_dir'], "documentation")

        for d in [hf_mesh_dir, hf_nifti_dir, hf_analysis_dir, surface_overlays_dir, documentation_dir]:
    ...
    def test_organize_ti_files_moves_volumes(self, load_post_processor, mock_logger, setup_test_structure):
    ...
    def test_organize_ti_files_moves_overlays(self, load_post_processor, mock_logger, setup_test_structure):
        ...
        assert os.path.exists(os.path.join(surface_overlays_dir, "001_TDCS_1_scalar_central.msh"))
...
class TestPostProcessorMTIFileOrganization:
    ...
    def test_organize_mti_files_renames_mesh_files(self, load_post_processor, mock_logger, setup_test_structure):
...
class TestPostProcessorExtractFields:
    ...
    def test_extract_fields_uses_script(self, mock_subprocess, load_post_processor, mock_logger, tmp_path):
    ...
    @patch('subprocess.run')
    def test_extract_fields_handles_timeout(self, mock_subprocess, load_post_processor, mock_logger, tmp_path):
...
class TestPostProcessorNiftiTransformation:
    ...
    @patch('subprocess.run')
    def test_transform_to_nifti_uses_script(self, mock_subprocess, load_post_processor, mock_logger, tmp_path):
...
class TestPostProcessorT1Conversion:
    ...
    @patch('subprocess.run')
    def test_convert_t1_to_mni_success(self, mock_subprocess, load_post_processor, mock_logger, setup_test_structure):
    ...
    def test_convert_t1_to_mni_skips_if_exists(self, load_post_processor, mock_logger, setup_test_structure):
        ...
        mni_file = os.path.join(m2m_path, f"T1_{setup_test_structure['subject_id']}_MNI.nii.gz")
    ...
    def test_convert_t1_to_mni_skips_if_no_t1(self, load_post_processor, mock_logger, tmp_path):
