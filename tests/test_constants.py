...
class TestDirectoryConstants:
    ...
    def test_no_hardcoded_paths(self):
        ...
        for dir_const in dirs_to_check:
            assert not dir_const.startswith('/'), f"{dir_const} should not be absolute path"
            assert not dir_const.startswith('\\'), f"{dir_const} should not be absolute path"
...
class TestDockerConstants:
    ...
    def test_docker_mount_prefix(self):
        ...
        assert const.DOCKER_MOUNT_PREFIX.startswith('/') or \
               const.DOCKER_MOUNT_PREFIX.startswith('C:') or \
...
class TestConstantTypes:
    ...
    def test_env_var_constants_are_strings(self):
        ...
        for env_const in env_constants:
            assert isinstance(env_const, str), \
...
class TestConstantConsistency:
    ...
    def test_no_duplicate_values(self):
        ...
        dir_constants = {name: getattr(const, name)
        ...
        assert len(unique_values) > len(values) * 0.7, \
...
class TestConstantImmutability:
    ...
    def test_constants_have_meaningful_names(self):
        ...
        public_constants = [name for name in dir(const)
                          if not name.startswith('_') and name.isupper()]

        # Should have a reasonable number of constants
        assert len(public_constants) > 10, \
            "Expected more than 10 public constants"
