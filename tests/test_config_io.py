...
class TestWriteReadRoundTrip:
    ...
    def test_montage_channels_none_serializes_as_null(self):
        from tit.sim.config import Montage, SimulationConfig
        ...
        data = serialize_config(config)
        assert data["montages"][0]["channels"] is None

    def test_montage_channels_round_trips_through_json(self):
        """Montage.channels (a plain list of index-group tuples) must
        survive serialize_config -> JSON file -> read_config_json intact,
        modulo the usual JSON tuple->list coercion."""
        from tit.sim.config import Montage, SimulationConfig
        ...
        config = SimulationConfig(
            ...
            montages=[
                Montage(
                    name="lee_style",
                    mode=Montage.Mode.NET,
                    electrode_pairs=[("E1", "E2"), ("E3", "E4"), ("E5", "E6")],
                    eeg_net="GSN-HydroCel-185.csv",
                    channels=channels,
                )
            ],
        ...
        try:
            data = read_config_json(path)
            assert data["montages"][0]["channels"] == [
                [[0, 2], [1, 3]],
                [[4], []],
            ]
        finally:
...
# ── Montage.channels through the GUI -> JSON -> subprocess path ─────────────
...
class TestMontageChannelsRoundTrip:
    ...
    def test_channels_survive_round_trip(self):
        back = self._rebuild(self._montage(channels=[([0, 2], [1, 3])]))
        assert back.channels == [([0, 2], [1, 3])]

    def test_non_beating_carrier_survives_round_trip(self):
        back = self._rebuild(self._montage(channels=[([0], [1]), ([2], [])]))
        assert back.channels == [([0], [1]), ([2], [])]

    def test_absent_channels_deserialize_to_none(self):
        assert self._rebuild(self._montage()).channels is None
