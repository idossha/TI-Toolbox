...
class TestBuildLogger:
    ...
    def test_logger_creation(
        self, mock_strftime, mock_makedirs, mock_get_logger, mock_get_pm
    ):
        ...
        mock_makedirs.assert_called_once_with(
            "/test/derivatives/ti-toolbox/logs/sub-001", exist_ok=True
        )
...
class TestLoadPayload:
    ...
    def test_load_valid_json(self, tmp_path):
        ...
        payload = {"config": {"subject_id": "001"}, "montages": [], "debug": False}
...
class TestMain:
    ...
    def test_missing_subject_id_in_config(self, tmp_path):
        ...
        exit_code = main(
            argv=["--config", str(config_file), "--results", str(results_file)]
        )
    ...
    def test_successful_execution(
        self, mock_pg, mock_run_sim, mock_logger, mock_pm, tmp_path
    ):
        ...
        payload = {
            "config": {
                ...
                "intensities": {"pair1": 1.0, "pair2": 1.0, "pair3": 1.0, "pair4": 1.0},
                "electrode": {
                    "shape": "ellipse",
                    "dimensions": [8.0, 8.0],
                    "thickness": 4.0,
    ...
    @patch("tit.sim.subprocess_runner._ensure_own_process_group")
    def test_simulation_exception_handling(
        self, mock_pg, mock_run_sim, mock_logger, mock_pm, tmp_path
    ):
    ...
    def test_debug_flag(self, mock_pg, tmp_path):
        ...
        with (
            ...
            main(argv=["--config", str(config_file), "--results", str(results_file)])
