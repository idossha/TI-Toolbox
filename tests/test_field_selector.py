...
def _make_project(tmp_path: Path, subject_id: str, montage: str) -> tuple[str, str, Path]:
    project_dir = tmp_path / "mnt" / "proj"
    m2m_dir = project_dir / "derivatives" / "SimNIBS" / f"sub-{subject_id}" / f"m2m_{subject_id}"
    sim_dir = project_dir / "derivatives" / "SimNIBS" / f"sub-{subject_id}" / "Simulations" / montage
...
def test_select_field_file_mesh_prefers_mti_when_present(tmp_path, monkeypatch):
    ...
    mti_mesh = project_dir / "derivatives" / "SimNIBS" / f"sub-{subject_id}" / "Simulations" / montage / "mTI" / "mesh"
...
def test_select_field_file_voxel_finds_grey_file(tmp_path, monkeypatch):
    subject_id, montage, project_dir = _make_project(tmp_path, "001", "montage1")
    m2m_dir = project_dir / "derivatives" / "SimNIBS" / f"sub-{subject_id}" / f"m2m_{subject_id}"
    nifti_dir = project_dir / "derivatives" / "SimNIBS" / f"sub-{subject_id}" / "Simulations" / montage / "TI" / "niftis"
