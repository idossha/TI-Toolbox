...
class TestUtilityFunctions:
    ...
    def test_log_group_analysis_complete_success(self, mock_print, mock_time):
        ...
        for call in expected_calls:
            assert any(
                call in str(print_call) for print_call in mock_print.call_args_list
            )
    ...
    def test_log_group_subject_status_failed(self, mock_print):
        ...
        group_analyzer.log_group_subject_status(
            "subj001", "failed", "1m 15s", "Error message"
        )
        mock_print.assert_called_once_with(
            "├─ Subject subj001: ✗ Failed (1m 15s) - Error message"
        )
...
class TestArgumentValidation:
    ...
    def test_validate_args_no_subjects(self):
        ...
        with pytest.raises(
            ValueError, match="At least one --subject must be specified"
        ):
            group_analyzer.validate_args(args)
    ...
    def test_validate_args_spherical_missing_coordinates(self):
        ...
        with pytest.raises(
            ValueError, match="Coordinates are required for spherical analysis"
        ):
            group_analyzer.validate_args(args)
    ...
    def test_validate_args_spherical_missing_radius(self):
        ...
        with pytest.raises(
            ValueError, match="Radius is required for spherical analysis"
        ):
            group_analyzer.validate_args(args)
    ...
    def test_validate_args_spherical_wrong_subject_args_count(self):
        ...
        with pytest.raises(
            ValueError,
            match="Subject 1: Spherical analysis requires exactly 3 arguments",
        ):
            group_analyzer.validate_args(args)
    ...
    def test_validate_args_cortical_mesh_missing_atlas_name(self):
        ...
        with pytest.raises(
            ValueError, match="Atlas name is required for mesh-based cortical analysis"
        ):
            group_analyzer.validate_args(args)
    ...
    def test_validate_args_cortical_voxel_wrong_subject_args_count(self):
        ...
        args = MagicMock()
        args.subject = [
            ["subj001", "/path/to/m2m", "/path/to/field.nii.gz"]
        ]  # Missing atlas_path
        args.analysis_type = "cortical"
        args.space = "voxel"
        args.region = "prefrontal"
        ...
        with pytest.raises(
            ValueError,
            match="Subject 1: Voxel cortical analysis requires exactly 4 arguments",
        ):
            group_analyzer.validate_args(args)
    ...
    def test_validate_args_cortical_missing_region_and_whole_head(self):
        ...
        with pytest.raises(
            ValueError,
            match="Either --whole_head flag or --region must be specified for cortical analysis",
        ):
            group_analyzer.validate_args(args)
    ...
    def test_validate_args_m2m_directory_not_found(self):
        ...
        with patch("os.path.isdir", return_value=False):
            with pytest.raises(
                ValueError, match="Subject subj001 m2m directory not found"
            ):
                group_analyzer.validate_args(args)
    ...
    def test_validate_args_voxel_cortical_atlas_not_found(self):
        ...
        with (
            ...
            with pytest.raises(
                ValueError, match="Subject subj001 atlas file not found"
            ):
                group_analyzer.validate_args(args)
...
class TestOutputDirectoryComputation:
    ...
    def test_compute_subject_output_dir_spherical_mesh(self):
        ...
        subject_args = [
            "subj001",
            "/path/to/m2m",
            "/path/to/Simulations/montage1/TI/mesh/field.msh",
        ]
    ...
    def test_compute_subject_output_dir_cortical_voxel_whole_head(self):
        ...
        subject_args = [
            "subj001",
            "/path/to/m2m",
            "/path/to/Simulations/montage1/TI/voxel/field.nii.gz",
            "/path/to/atlas.nii.gz",
        ]
    ...
    def test_build_main_analyzer_command_spherical_mesh(self):
        ...
        with (
            ...
            mock_path.return_value.__truediv__ = lambda self, other: Path(
                f"/path/to/analyzer/{other}"
            )
            mock_path.return_value.parts = (
                "/path/to/Simulations/montage1/TI/mesh/field.msh"
            ).split("/")

            cmd = group_analyzer.build_main_analyzer_command(
                args, subject_args, subject_output_dir
            )
...
class TestSubjectAnalysis:
    ...
    def test_run_subject_analysis_success(
        self, mock_run, mock_build_cmd, mock_compute_dir
    ):
        ...
        with (
            ...
            success, output_dir = group_analyzer.run_subject_analysis(
                args, subject_args
            )
            ...
            mock_build_cmd.assert_called_once_with(
                args, subject_args, "/path/to/output"
            )
    ...
    @patch("group_analyzer.build_main_analyzer_command")
    @patch("subprocess.run")
    def test_run_subject_analysis_failure(
        self, mock_run, mock_build_cmd, mock_compute_dir
    ):
    ...
    def test_run_subject_analysis_quiet_mode(
        self, mock_popen, mock_build_cmd, mock_compute_dir
    ):
...
class TestPathCollection:
    ...
    def test_collect_analysis_paths_success(self):
        ...
        with (
            patch("os.path.isdir", return_value=True),
            patch(
                "os.listdir", side_effect=[["file1.csv", "file2.txt"], ["results.csv"]]
            ),
            patch("group_analyzer.group_logger", None),
        ):
...
class TestGroupAnalysis:
    ...
    def test_run_comprehensive_group_analysis_success(self, mock_run_comparisons):
        ...
        with patch("group_analyzer.group_logger", None):
            result = group_analyzer.run_comprehensive_group_analysis(
                analysis_paths, project_name
            )
...
class TestGroupSubfolderName:
    ...
    def test_determine_group_subfolder_name_spherical(self):
        ...
        with patch("group_analyzer.Path") as mock_path:
            ...
            result = group_analyzer.determine_group_subfolder_name(
                args, first_subject_args
            )
...
class TestMainFunction:
    ...
    @patch("os.makedirs")
    def test_main_successful_analysis(
        self,
        mock_makedirs,
        mock_run_group,
        mock_collect,
    ...
    def test_main_validation_error(self, mock_validate, mock_setup_parser):
        ...
        with patch("group_analyzer.group_logger", None), patch("sys.exit") as mock_exit:
            group_analyzer.main()
    ...
    def test_main_no_compare_flag(
        ...
        with patch("group_analyzer.group_logger", mock_logger):
            ...
            assert any(
                "Group comparison was skipped" in str(call)
