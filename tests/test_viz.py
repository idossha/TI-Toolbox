...
def mock_path_manager():
    ...
    mock_pm = MagicMock()
    mock_pm.get_subject_dir = MagicMock(
        return_value="/fake/project/derivatives/SimNIBS/sub-001"
    )
    mock_pm.get_simulation_dir = MagicMock(
        return_value="/fake/project/derivatives/SimNIBS/sub-001/Simulations/montage1"
    )
...
class TestCreateGlassBrainEntryPointGroup:
    ...
    @patch("os.makedirs")
    def test_glass_brain_group_success(
        self, mock_makedirs, mock_plot, mock_savefig, mock_nifti_img
    ):
    ...
    def test_glass_brain_group_no_data(self, mock_plot):
        ...
        result = create_glass_brain_entry_point_group(
            averaged_img=mock_img, base_filename="empty", output_dir="/fake/output"
        )
...
class TestCreatePdfEntryPoint:
    ...
    def test_pdf_with_atlas_regions(self, mock_viz_class):
        ...
        result = create_pdf_entry_point(
            "001", "montage1", atlas_name="aal", selected_regions=[10, 20, 30]
        )
...
class TestCreatePdfEntryPointGroup:
    ...
    def test_pdf_group_failure(self, mock_viz_class, mock_nifti_img):
        ...
        result = create_pdf_entry_point_group(mock_nifti_img, "group", "/fake/output")
    ...
    def test_pdf_group_with_callback(self, mock_viz_class, mock_nifti_img):
        ...
        result = create_pdf_entry_point_group(
            mock_nifti_img, "group", "/fake/output", output_callback=mock_callback
        )
...
class TestCreateHtmlEntryPoint:
    ...
    def test_html_success(self, mock_viz_class):
        ...
        result = create_html_entry_point(
            subject_id="001", simulation_name="montage1", min_cutoff=0.3
        )

        assert result == 0  # Success exit code
        mock_viz.create_html_visualization.assert_called_once_with(
            "001", "montage1", 0.3
        )
...
class TestSlicesCLI:
    ...
    @pytest.mark.unit
    @patch(
        "sys.argv", ["img_slices.py", "--subject", "001", "--simulation", "montage1"]
    )
...
class TestHtmlReportCLI:
    ...
    @pytest.mark.unit
    @patch(
        "sys.argv", ["html_report.py", "--subject", "001", "--simulation", "montage1"]
    )
...
class TestParameterValidation:
    ...
    def test_glass_brain_valid_parameters(self, mock_viz_class):
        ...
        # Valid parameters
        result = create_glass_brain_entry_point(
            "001", "montage1", min_cutoff=0.0, max_cutoff=10.0, cmap="hot"
        )
