...
class TestCorticalAnalysis:
    ...
    def test_analyze_cortex_success(self):
        ...
        # Mock region info
        region_info = {
            1: {"name": "TestRegion", "voxel_count": 100, "color": (0.5, 0.5, 0.5)}
        }
        ...
        with patch.object(
            ...
            "load_brain_image",
            side_effect=[
                (mock_atlas_img, mock_atlas_arr),
                (mock_field_img, mock_field_arr),
            ],
        ):
            with patch.object(
                self.analyzer, "get_atlas_regions", return_value=region_info
            ):
                with patch.object(
                    self.analyzer, "find_region", return_value=(1, "TestRegion")
                ):
                    with patch.object(
    ...
    def test_analyze_cortex_no_voxels_in_region(self):
        ...
        # Mock region info
        region_info = {
            1: {"name": "TestRegion", "voxel_count": 0, "color": (0.5, 0.5, 0.5)}
        }
            ...
            with patch.object(
                ...
                with patch.object(
                    ...
                    result = self.analyzer.analyze_whole_head(
                        atlas_file="/path/to/atlas.mgz", visualize=False
                    )
                ...
                with patch("nibabel.load", return_value=mock_resampled_img):
                    with patch("os.unlink"):
                        result_img, result_data = self.analyzer.resample_to_match(
                            mock_source_img, (128, 128, 128), np.eye(4)
                        )
...
class TestFocalityMetrics:
    ...
    def test_calculate_focality_metrics_success(self):
        ...
        result = self.analyzer._calculate_focality_metrics(
            field_data, voxel_volume, region_name
        )
...
class TestRegionFinding:
    ...
    def test_find_region_by_name(self):
        ...
        region_id, region_name = self.analyzer.find_region(
            "Left-Hippocampus", region_info
        )
