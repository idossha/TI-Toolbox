...
%              element_data or node_data field (standard: 'TI_max', or the
...
%
% Examples:
%  mesh_get_histogram(m); % plot histogram for TI_max for gray matter
...
s.nbins = 100;
s.haxis = [];
s.focality_cutoffs = [50, 75, 90, 95]; % Added focality cutoffs
...
% scaling is from 0 to 99.9 percentile (TI_max, magnJ)
...
if nargout
    ...
    % Add vertical lines for focality cutoffs
    hold on;
    for cutoff = s.focality_cutoffs
        cutoff_value = prctile(data, cutoff);
