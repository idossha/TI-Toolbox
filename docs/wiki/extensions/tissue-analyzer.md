...
permalink: /wiki/extensions/tissue-analyzer/
---

The Tissue Analyzer extension provides comprehensive analysis of different tissue types from segmented NIfTI data. This tool supports analysis of cerebrospinal fluid (CSF), bone, and skin tissues with automated volume and thickness calculations.

## Key Features

- **Multi-Tissue Support**: Analyze CSF, bone, and skin tissues with configurable parameters
- **Volume Calculations**: Automatic volume measurements in both mm³ and cm³
- **3D Thickness Analysis**: Distance transform-based thickness calculations with statistical summaries
- **Spatial Filtering**: Intelligent filtering based on brain regions to exclude irrelevant anatomy
- **Publication-Quality Visualizations**: High-resolution figures for axial, coronal, and sagittal views
- **Comprehensive Reporting**: Detailed analysis summaries with voxel counts and statistical metrics
- **Label Name Mapping**: Automatic loading of tissue label names from labeling_LUT.txt files
- **Flexible Configuration**: Custom tissue labels and analysis parameters
...
```python
from tissue_analyzer import TissueAnalyzer, TISSUE_CONFIGS

# Load CSF configuration
config = TISSUE_CONFIGS['csf']
analyzer = TissueAnalyzer('segmented.nii.gz', 'output_dir', config)

# Run full analysis
results = analyzer.run_full_analysis()

# Access results
volume_cm3 = results['volumes']['csf_volume_cm3']
thickness_stats = results['thickness_stats']
```
...
### 3. Volume Analysis
- Calculate total tissue volume in mm³ and cm³
...
## Spatial Filtering Methodology

The tissue analyzer uses intelligent spatial filtering to focus on anatomically relevant regions:

- **Brain Reference**: Uses grey matter cortex and brainstem as anatomical landmarks
- **3D Bounding Box**: Extends analysis region around brain with configurable padding
- **Z-Coordinate Filtering**: Excludes lower anatomy below brain center
- **Tissue-Specific Optimization**: Different padding values for CSF (40mm), bone (30mm), and skin (35mm)
...
### Report Contents
- Input file information and voxel dimensions
- Volume measurements (mm³ and cm³)
- Thickness statistics (mean, std, range)
- Spatial filtering details
- Voxel counts by tissue label
...
### Dependencies
- **Core Libraries**: nibabel, numpy, matplotlib, scipy
- **Optional**: logging_util (for integrated logging)
- **Python Version**: Compatible with SimNIBS Python environment

## Troubleshooting

### Common Issues

**No tissue found**: Check that tissue labels exist in your segmentation file
**Label names not loading**: Verify labeling_LUT.txt is in expected locations
**Memory errors**: Reduce padding values or use smaller regions
**Visualization errors**: Ensure matplotlib backend supports high-DPI output

### Expected Locations for labeling_LUT.txt
1. Same directory as input NIfTI file
2. `derivatives/SimNIBS/sub-XX/m2m_sub-XX/segmentation/`
3. Parent directories of input NIfTI file (up to 3 levels)

### Performance Optimization
- **Large Files**: Consider using ROI-based analysis for large datasets
- **Memory Usage**: Monitor voxel counts in debug logs
- **Visualization**: PNG format loads faster than PDF for preview
