...
- **FastSurfer `--seg_only` (v3)** writes `aparc.DKTatlas+aseg.deep.mgz`. It does not produce the legacy thalamic nuclei or hippocampal/amygdala subfield atlases. See [pre-processing]({{ site.baseurl }}/wiki/pre-processing/#what-changed-from-freesurfer).
- **Legacy FreeSurfer `recon-all`** wrote the volumetric parcellations `aparc.DKTatlas+aseg.mgz`, `aparc.a2009s+aseg.mgz`, `aparc+aseg.mgz` and `aseg.mgz`, along with the finer `ThalamicNuclei.v13.T1.mgz` and `lh/rh.hippoAmygLabels-T1.v22.mgz` segmentations.
