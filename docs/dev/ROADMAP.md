...
| 2026-09-07 CX6 fourth consolidation gate (external audit response) | Typecheck clean; lint **0 errors** / 3 React Compiler warnings; `vitest run` **1,304 in 109 files**; host `pytest tests -q` **4,069 passed** / 37 skipped (the `test_scene_guide` order-dependent flake fixed at its cause, `c799e7e2`); container `tests/numerical/` + jobs/kernels/server against the real libraries **410 passed**; `dev/contracts_check.py` OK; route-import guard 23 modules; `actionlint` clean (17 findings fixed in `python-security.yml`); full offscreen mock e2e **317 passed** / 2 skipped; real leg **39 of 40** against the dev container | Passed apart from `mex.spec.ts` (see below) — full table in `docs/dev/BENCHMARKS.md § External audit response`, narrative in `docs/dev/HISTORY.md § 2026-09-07 (External audit response)`. SCI-07 is left open by design |
...
| **SCI-07 — does `channels` govern `hf_peak`/`hf_sar`?** A modelling decision, not a defect. The recommendation is Model B (sum same-carrier fields before any exposure metric, gated on the montage declaring `channels`, `channels=None` bit-identical) | It changes a **safety** metric's meaning, so it is the maintainer's call and not a lane's. Deciding it also settles whether `_envelope_from_PQ`'s cancellation rationalisation is worth doing in the same pass | `SCIENTIFIC-CORRECTIONS.md § Open decisions`, `DECISIONS.md § 2026-09-07 (CX6) — SCI-07 left open` |
| **`release-v3.yml` has never run.** A `workflow_dispatch` dry run (`dry_run=true`, which stops after the unsigned validation job) is the cheapest thing that would prove the plan → image → desktop-validate half | Only CI can run it. Locally, only macOS arm64 was packaged and verified; signing and notarisation have never been exercised on this branch | `RELEASE.md` final section, `DECISIONS.md § 2026-09-07 (CX6) — one release workflow` |
| **Tetravox PR #35 and the 0.3.12 tag** — still the blocker on the first real automatic embed install | No `idossha/tetravox` release yet carries `tetravox-embed-<v>.tgz`, its `.sha256` and its `.manifest.json` as assets, so a fresh image bakes the placeholder | the Tetravox row above |
| **The idle-electrode contour.** An idle marker's worst contrast against the opaque-GM scalp is 2/255 | A design call: a thin contour on *every* marker (not only the ones carrying a channel colour) would keep colour as the whole state signal, because the contour would be constant | `DECISIONS.md § 2026-09-06 (CX5) — Open: an idle electrode can be invisible`, `BENCHMARKS.md` |
| **`contracts/` tidy.** `openapi.v0.yaml`, `openapi.v1.json` and the generated `openapi.json` coexist, and `dev/build_contract.py` owns the generated one | Nothing is broken — `dev/contracts_check.py` is green over all of it — but three files for one contract is a shape a newcomer has to be told about rather than read | `DECISIONS.md`, `dev/build_contract.py` |
| **`DECISIONS.md`'s older entries are not in the Decision/Why/Cost/Revisit-if shape** the README prescribes; entries from 2026-09-07 onward are | A mechanical reformat of ~40 entries, worth doing in one pass rather than incidentally, and worth doing only once no lane is appending to the file | `docs/dev/README.md § Where a new fact goes` |
| **`tests/e2e/real/mex.spec.ts` is written against the pre-jobs-table Optimizer** and is the one red left in the real leg. It clicks a page-level `radiogroup "Method"` for an "mEx" radio, then a global Run name field, a page-level Subcortical radio and a leadfield strip | All four are the global sections ADR row 28 dissolved into the row, and `mex` is no longer a method at all — it is *derived* from eight electrodes / four pairs. Making the spec green means rewriting the whole flow the way `ex.spec.ts` now does it (per-row cells and the row editor), which is a rewrite rather than a repair, and a wrong one would be worse than an honest red | `docs/dev/BENCHMARKS.md § External audit response`, `desktop/src/renderer/pages/optimizer/rows.ts` (`OPT_METHODS`) |
| **The `cluster-permutation` real job fails on Dataset 000** — every voxel is excluded as degenerate, so nothing is testable. It now says so (`22129a34`) instead of raising a numpy "zero-size array" traceback, and the spec passes because it only requires a terminal state | Whether a 2-vs-1 unpaired contrast on those three images *should* be degenerate everywhere is a data question, not a code one. Worth answering, because a real spec that accepts a failed job proves less than it looks like it does | `docs/dev/SCIENTIFIC-CORRECTIONS.md` (SCI-06), `tit/stats/engine.py::ttest_voxelwise` |
| **Every real e2e run rewrites two tracked smoke payloads** (`tests/smoke/payloads/analyzer*.json`, `nifti_average.json`, `nilearn.json`) with a fresh run-id namespace, so the worktree is dirty after a gate | The recording is deliberate (Level A replays what the UI sent) and the run id must be unique per run (`never overwrite a pre-existing output`), so this is churn by design rather than a defect. It still costs every lane a `git checkout` it has to know about | `desktop/tests/e2e/_helpers.ts::recordPayload` |
