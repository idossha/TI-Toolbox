...
- **Save an already open TetraVox scene into your project** — **Viewer ▸ Saved scenes ▸ Save scene** now captures the active native scene even when its files were opened directly in TetraVox. Choose a name; TI-Toolbox saves it in the active BIDS project and lists it for reopening. Native snapshots reopen directly, fixing "Path escapes the project jail" for scene-relative datasets; already saved scenes need no migration. Compatible native scene-saving support is required.
- **Viewer saves require the live native scene** — **Viewer ▸ Saved scenes ▸ Save scene** no longer saves an outdated builder recipe. It requires native scene-saving support and confirms the saved file before refreshing; existing TetraVox builds without that capability show the action as unavailable. Save selection and Recent are removed; file-list Reset and existing saved files remain.
- **macOS viewer opens request activation** — opening a result also requests that the selected TetraVox app reopen, helping reveal an existing window. Actual OS foreground behavior remains a packaged release check.

- 2026-09-19: **TetraVox owns its updates; TI performs initial setup** — desktop startup discovers an existing native viewer and starts verified setup when absent on supported package targets. **Settings ▸ Viewer** shows setup progress and retry rather than a second set of update controls. Existing viewer installations remain usable after in-app updates. Automatic Linux/Windows setup and native foreground/load acknowledgment remain release gates.

- 2026-09-19: **Jobs columns share the available width and selecting a job reopens its details** — **Jobs** distributes spare table width across all six data columns, including beside the detail pane. Narrow tables scroll to keep resource values readable. Selecting a job in the table or grouped view restores a collapsed detail pane.

- 2026-09-19: **A job's Artifacts tab lists what is in its folder, and the Jobs table's columns fit their numbers** — the tab counted only the files a runner remembered to register: an ex-search said *Artifacts (3)* with ten files on disk, a flex run *(2)* with thirty-three, a simulation *(1)*. It now lists the job's output folder as it is on disk (every file, with its size; `GET /api/jobs/{id}/artifacts`), the same read the Results page does, and the runner's registered list only says where that folder is — the simulation runner now registers its montage folder, so a simulation's tab shows the whole run rather than the mesh's own subfolder. On the Jobs page the CPU and RSS columns were cut off (*1106 % · a…*) whichever way the detail pane sat, because the page kept its own column widths from the eight-column layout and gave the removed STAGE column's slack to ELAPSED; the two tables now share one set of measured widths (CPU and RSS fit "1106 % · avg 368 %" and "50.2 GB · avg 16.1 GB" whole, SUBJECTS takes the slack), checked at 1280 and 1440 with the pane open and closed. A job recorded before real sampling shows "—" rather than "0 %" / "0 KB".
- 2026-09-19: **A job's CPU and memory are real, whole-tree, and kept as peak and average** — the Jobs table showed **0 %** CPU for every job it ever ran: the server built a fresh `psutil.Process` on each 0.25 s poll and asked it for a CPU delta it had no baseline for, and it looked only at the runner's own process while SimNIBS, PARDISO, FastSurfer and the ex-search's joblib workers did the work in children. One sampler per running job now reads CPU % and memory over the job's **whole process tree** once a second, keeps the running **peak and average** (a simple mean over the samples, so time-weighted to within a second) in the job record, and the record keeps them after the job finishes and across a server restart. Memory is the tree's proportional set size on Linux, so pages a dozen workers share are counted once (summing RSS over an ex-search tree reported 54 GB for a 9 GB working set). The **CPU** and **RSS** columns show `peak · avg` — an ex-search on Dataset 000 / sub-101 reads `1106 % · avg 344 %` and `9.1 GB · avg 3.5 GB` on the 12-CPU dev container, a mesh analysis `106 % · avg 100 %`. The **STAGE** column is gone from the Jobs table — the run pages' terminals and the job's Summary carry progress — and the row's remaining columns take its width. `JobStatus` gains `cpu_percent_peak`, `cpu_percent_avg`, `rss_peak`, `rss_avg` (contract, DECISIONS ADR 34).
- 2026-09-19: **An MNI atlas label reaches the subject in seconds, once, and the console says what it is doing without ticking** — warping one atlas label into subject space took 86 s on Dataset 000 / sub-101 under emulation, and the search then did it again (72 s) because the ROI confirmation and the run each called SimNIBS's `mni_mask_to_sub`, which resamples the *whole* non-linear deformation field onto the subject's 0.5 mm grid (three trilinear passes over 100 million voxels, 56 s), fills its boundary zeros (8.5 s), gzip-decodes the 417 MB field (7.7 s) and writes an 800 MB float64 mask (3.6 s). `tit.opt.masks.prepare_mask` now runs the same arithmetic on only the block of the subject grid the mask can land in — the field itself says which voxels fall inside the label's MNI bounding box — and the result is **voxel-identical** to SimNIBS's (0 of 99 442 voxels differ for the cerebellum label; 0 for two Harvard-Oxford labels and a whole label volume; `tests/numerical/test_mni_warp.py` proves it against SimNIBS on synthetic fields including boundary zeros and a mask holding the MNI origin). The warped mask is kept under `<project>/.ti-toolbox/cache/masks/sub-<id>/mni-warp/`, keyed on the mask's voxels and affine, the target grid and the deformation field's path, size and mtime, so the confirmation warps a label once and the run — flex, ex, mEx (which now warp the selected label as a binary mask instead of the whole atlas) and the analyzer — reuse it. The deformation field is decoded once per process when a union of labels is warped. Measured on sub-101, label 11: single warp 87 s → 6 s (plus a one-time 6 s `import simnibs`), cache hit 0.1 s; the flex job's ROI setup 158 s → 14 s. The subject-space mask is written as `uint8` `.nii.gz` (450 KB instead of 805 MB per run) and flex's derived masks go to the project cache rather than `m2m_<id>/masks/.prepared`. The job console's stage lines (*Loading head model …*, *Preparing the FEM solver …*) now print once when the stage starts and once *done in N s* when it took three seconds or more — no *still running (N s)* every ten seconds in between; `stage_heartbeat` has no ticker thread and no `every` argument.
- 2026-09-18: **FastSurfer runs without a license, and your own FreeSurfer license is pasted once in Settings** — two things were wrong. (1) FastSurfer segmentation, which needs no license at all, was failing on the spot with *Native FastSurfer host is disconnected. Reconnect TI-Toolbox or disable native FastSurfer before retrying.* whenever a previous desktop session had been force-quit or had crashed with Apple GPU enabled: its `code/ti-toolbox/native-fastsurfer/availability.json` stayed behind, and a stale heartbeat was treated as a refusal rather than an absence. A stale heartbeat before a request now means no host is connected — the log says so and FastSurfer runs in the container on CPU (verified on Dataset 000 / sub-101: `run_fastsurfer.sh --seg_only` launched and segmented). A host that disappears mid-run still fails the job instead of silently restarting it on CPU. (2) FreeSurfer `recon-all` and the thalamic / hippocampal-amygdala subregion tools need a FreeSurfer license, but the v3 app gave you nowhere to supply one — no launcher set `FS_LICENSE` or mounted a file — so those stages could never run. The license is issued per registered person and may not be redistributed, so the toolbox never bundles or fetches one: register (free) at [surfer.nmr.mgh.harvard.edu/registration.html](https://surfer.nmr.mgh.harvard.edu/registration.html) and paste the `license.txt` you receive once under **Settings → Pre-processing → FreeSurfer license**. It is stored in your TI-Toolbox user directory (mounted into every job), used by recon-all, the subregion tools and QSIPrep/QSIRecon, and never uploaded or shown back beyond the registered email. Selecting a FreeSurfer step without one shows the registration link on the Pre-processing page, and the preflight refusal names exactly which selected stages need it. `$FS_LICENSE` still wins when set.
- 2026-09-18: **An analysis folder is data, one histogram and one scene** — an analysis used to leave seven to nine files: two or three Tetravox scenes (`roi.tetravox.json`, an ROI-only picture with the T1, the central surface and the `.annot`; `roi_field.tetravox.json`, the whole unmasked field volume with a threshold — voxel only), a 600-dpi matplotlib `histogram_histogram.pdf`, a mesh overlay whose first view was the simulation's own whole-surface field, hidden, and two host screenshots plus Tetravox's `job-result.json` trace. Now it leaves `results.csv`, `analysis.json`, the overlay the analysis computed from (`roi_overlay.msh` + `.opt` in mesh space — 49 MB instead of 55, the whole-surface view dropped and the `.opt` renumbered — or `roi_overlay.nii.gz` in voxel space), **`histogram.png`** — the same whole-head histogram as before (bars coloured by ROI contribution fraction, colour bar, ROI mean and focality cutoffs, stats box), area-weighted in mesh space and volume-weighted in voxel space, with the legend's repeated "of 99.9%ile" / "Count:" wording folded into one legend title, one 150-dpi PNG that previews inline in Results and stays legible as a thumbnail — and **one `scene.tetravox.json`** that points at that overlay: in voxel space the field masked to the ROI over the subject's T1, `inferno` from the display floor to the ROI's p99.9, zeros hidden; in mesh space the whole cortex at 25 % opacity under the ROI's `<field>_ROI` node field, a colour bar, `peel` transparency and the 3D pane large, so the ROI's field shows through the cortex rather than the flat single-colour surface a bare `.msh` open gave. The cursor is on the ROI in both. With Tetravox installed the app photographs the scene into one `scene.png` (the 3D pane is in the figure whenever the scene has a mesh) and no longer leaves `job-result.json` beside it — the trace goes to the app log. The Viewer's *analysis* view of a mesh analysis now colours `roi_overlay.msh` by its ROI field, reading the field's name and maximum from `results.csv`, instead of drawing it solid, and takes its cursor from the analysis's own scene for every analysis type. Optimizer runs keep their `roi.tetravox.json` target confirmation unchanged. `tit.plotting.plot_whole_head_roi_histogram` and `tit/plotting/focality.py` are removed (the histogram is `tit.analyzer.visualizer.save_histogram`); `roi_confirmation.confirm_roi` no longer takes `field_path`. Verified in the dev container on `ernie` / `L_Insula` / `lh.insula` (DK40) in both spaces, each scene opened headless with Tetravox 0.5.2.
- 2026-09-18: **A job whose inputs are missing is refused before it starts, and the refusal says what, where and how** — an analyzer run in voxel space on a cortical DK40 target was accepted, queued, and died minutes later with `FileNotFoundError: Atlas 'DK40' not found in .../fastsurfer/sub-101/mri, ...` because the subject had no FastSurfer parcellation. Every submission (`POST /api/jobs`, `/api/jobs/groups`) now sweeps the job's required inputs first — head model and head mesh, EEG net CSV and its electrode labels, the simulation's field mesh / central surface / tissue NIfTIs, atlas volumes or `.annot` files, ROI CSVs, the leadfield, mask files, MNI NIfTIs for the group tools, fsaverage projections, BIDS T1w/DWI for preprocessing — using the same path rules the runner uses, and answers **422 "Missing inputs"** with one entry per absent input: what it is, the exact path it was expected at, and how to produce it (*Run FastSurfer for sub-101, or analyze in mesh space instead*). No job record is created. The desktop shows the list as one persistent notice, one line per input, on every Run button; `POST /api/jobs/preflight` returns the same list without submitting.
- 2026-09-18: **Path-shaped names are refused at the door, and the code says so in a shape CodeQL can read** — every request value that names a subject, simulation, run, montage, ROI, atlas, EEG net or analysis is now validated before the route runs (`../x`, `/etc/passwd`, `a/b`, `.hidden` are a 422 naming the rule, where some were a 404 from deep inside a listing), and every `PathManager` accessor joins through one lexical containment check while every read, listing and write at a jail boundary goes through one physical (symlink-following) check that returns the path it checked. Two real findings fell out: the Viewer's argument preview stat'ed and probed layer paths from the request body before jailing them (an existence oracle for any file), and the mask-upload filename check backtracked quadratically on a long run of dots. The 178 open `py/path-injection` code-scanning alerts and the one `py/polynomial-redos` alert are resolved by these changes, not suppressed: local CodeQL on this checkout reports 0 of each. Real project names are untouched — a read-only walk of the maintainer's dataset (`ernie`, `101`, `MNI152`, runs with underscores and hyphens, EEG nets with dots, v2 run names with spaces) is now a test.
- 2026-09-17: **The Morel thalamus atlas is no longer shipped; Harvard-Oxford, the Diedrichsen cerebellum and Schaefer 400/7 are** — Morel is CC BY-NC-SA 4.0, which a GPL-3 project cannot redistribute, so `MorelMNI152_labeling_1mm.nii.gz`, its LUT and the surfaces derived from it in the MNI guide are gone from the repository and the image. A configuration that still names it stops with one sentence, *"The Morel atlas is no longer shipped (CC BY-NC-SA); see docs/wiki/atlases.md"*, before anything is loaded (flex/ex/mEx, the analyzer's mask validation, the ROI resolver and the target-preview route all check the manifest's new `not_shipped` list). The wiki and `resources/atlas/README.md` keep a *Not shipped* note with the licence reason, the Zenodo record (13918589) and the path to bringing it back as an optional, user-fetched download. In its place, four new MNI label volumes, all on the FSL MNI152NLin6Asym grid SimNIBS's warps assume (headers checked against the shipped `MNI152_T1_1mm.nii.gz`, and `tests/test_atlas_manifest.py` re-checks them): **Harvard-Oxford cortical (48) and subcortical (21)** maximum-probability maps, threshold 25, 1 mm, and the **Cerebellum-MNIfnirt** map (28), all CC BY-SA 4.0 and byte-identical to FSL's, with LUTs generated from FSL's XML; and **Schaefer 2018, 400 parcels / 7 networks, 1 mm** from CBIG (MIT), with CBIG's own LUT. Every shipped atlas and the template now has a full notice — copyright holder, licence, required attribution (the HCP clause-5a acknowledgement verbatim), citation — in a new repository-root `NOTICE`, referenced from `LICENSE` and the README. The MNI targeting pane was rebuilt and draws the Harvard-Oxford and Cerebellum surfaces (Schaefer's 400 parcels exceed its 256-region budget, so it is picker-only).
- 2026-09-17: **A finished job's log is there again** — every console asked the server for a job's events with `since=-1`, which a real server rejects (HTTP 422); only the mock accepted it. A job that had already finished streams nothing, so its whole transcript came from that one rejected request and the run page's Terminal came up empty — the Jobs page's **Raw log** survived only because it falls back to reading the log file. The request now asks for `since=0`, and the mock server answers `since` the way the real one does (inclusive, negatives rejected) so this class of defect cannot hide there again.
- 2026-09-17: **Scrolling back in a log stays where you put it** — with **Follow** on, scrolling up to read an earlier line was undone by the next line the job printed, and the only way out was to find the Follow switch. Scrolling away from the tail now turns Follow off, exactly as a terminal does; the switch turns it back on and returns to the tail.
- 2026-09-17: **A running job's log no longer loses its middle** — the live console merges one snapshot of a job's events with the socket's buffer, and that buffer kept only the last 500 events. A simulation writes thousands (7851 in one recorded run), so everything between the snapshot and the most recent 500 lines vanished from the console until the job finished. The buffer now holds a whole real transcript.
- 2026-09-17: **A job's Summary shows its last words** — the Summary tab's console excerpt polls while a job runs and stops when it ends, but nothing refetched it at that moment, so the pane kept showing text from up to three seconds before the end — without the final lines or the traceback — for as long as it stayed open. It now refreshes once when the job reaches its final state.

- 2026-09-17: **The plan panel's numbers are the run's numbers** — the CPUs, threads and duration a plan showed before you pressed Run were each wrong in their own way. (1) **CPU count:** `os.cpu_count()` inside a container reports the Docker VM's cores whatever `--cpus` said, so every plan derived from it promised cores that did not exist; the new `tit/cpu.py` reads the real limit (cgroup v2 `cpu.max`, cgroup v1 quota/period, the effective cpuset, `sched_getaffinity`, then `os.cpu_count()` — smallest wins) and the estimate, the scheduler budget, the reconstruction thread defaults and the exhaustive searches all use it. (2) **Threads:** an ex/mEx run with the default `n_jobs = -1` forked `os.cpu_count() - 1` workers while the plan said "2 CPU", and flex handed SimNIBS no `cpus` at all; the job runner now exports the admitted budget as `TIT_JOB_CPUS` beside `OMP_NUM_THREADS`/`MKL_NUM_THREADS`/`NUMBA_NUM_THREADS`, both searches read it, and the ex/mEx plan reports the worker count the run will actually fork. (3) **Parallel subjects:** `POST /api/plan/pre` clamps `parallel_subjects` to what the scheduler's budget could admit and says so in a warning, instead of multiplying one stage's cost into a figure no container could run. (4) **Duration:** the plan grid gains an **Est.** tile — `≈ 25 m`, with a tooltip naming the machine it was computed for and the benchmark it came from, and a dash for job kinds with no measured baseline (analyzer, stats, FreeSurfer preprocessing) rather than a number with nothing behind it. Each estimate model is now checked against its measured run in `tests/test_plan_accuracy.py`. Verified in the dev container (12 usable CPUs, emulated).
- 2026-09-17: **Caches live in one hidden folder, `.ti-toolbox/`** — a project used to collect toolbox scratch files in six visible places, including `derivatives/ti-toolbox/scene_cache/` (61 MB on our test dataset, unbounded on a large one) inside the tree that is supposed to hold your results. Everything the toolbox can rebuild — viewer surface payloads, warped ROI masks, viewer intensity statistics and the disk-usage scan — now lives under `<project>/.ti-toolbox/cache/`, with a README inside saying it is safe to delete at any time, and `.ti-toolbox/` is added to your `.bidsignore` so the BIDS validator ignores it. Existing projects migrate themselves the first time the toolbox needs a cache: the old directories are *moved*, not rebuilt, so nothing is recomputed and nothing is lost. Your results, configuration, notebooks, saved viewer scenes and job history do not move. The Overview disk-usage page lists the new folder as **Rebuildable cache**.
- 2026-09-17: **Overview shows the jobs you ran, and a click leads to them** — the project's front door listed what is on disk but never what the project had *done*, so getting from "I ran that yesterday" to its outputs meant the Jobs page and then Results. A **Recent jobs** section now sits under the subject matrix: the eight newest jobs, newest first, each one row of kind · subject(s) · run · state · when it started · how long it took, read from the same `/ws/jobs` store the rail and the Jobs page share (no request of its own). The whole row is a link, keyboard-reachable like any button: a finished job of a kind Results browses opens **Results** on its subject; a job that is running, queued, failed — or of a kind Results has no view for, such as `leadfield` or `project_init` — opens the **Jobs** page with that job selected, on its log. **See all** goes to the full list, and a project that has run nothing says so.
- 2026-09-17: **The ROI artefact is one small scene file, not six** — an analysis folder was leaving `roi_mask.nii` (125 MB), `roi_plate_roi.nii` (125 MB), `roi_plate.png`, `roi_plate.json`, `roi_plate.plate-request.json` and `roi_plate.tetravox-job.json` beside its own four outputs. Every optimization and analysis target now leaves **one file, `roi.tetravox.json`** — a TetraVox scene, the same thing *File ▸ Save Scene* writes and *Open in TetraVox* opens — which **points at files you already have**: your own `T1.nii.gz`, the atlas or mask you named with just your target's labels shown and coloured, or, for a cortical region, the hemisphere's central surface with its `.annot` attached. Nothing is copied or rasterised to make it. The centroid, voxel count, grey-matter overlap and framing rule are in the scene's own `meta` block and in the one terminal line, so nothing is lost with the sidecar. The analyzer's field analysis leaves a second scene, `roi_field.tetravox.json`, naming the **same `TI_max` file the table was computed from**, thresholded to the ROI's own range. The one exception is an MNI target, whose transformed mask exists in no file you have: it is written once, compressed, on the *atlas's* voxel grid — 15.9 KB for CIT168 into sub-101, where the old T1-grid copy was 125 MB. Open the scene from the job's **Artifacts** tab and it is the ROI, centred and filling the view, exactly as the job framed it. With TetraVox installed the app also photographs each scene into `roi.png` beside it when the job finishes; without it there is no picture and the scene still opens. `TIT_NO_ROI_PLATE=1` (or `TIT_NO_ROI_CONFIRMATION=1`) turns the whole thing off.
- 2026-09-17: **Every shipped MNI atlas declares what it is, and the ROI picker routes it accordingly** — the MNI atlas list was a bare list of filenames with no record of whether a file was a surface parcellation or a label volume, so the picker could not tell the two apart and handed the Subcortical mode every packaged MNI atlas whatever it was. `resources/atlas/manifest.json` is now the one description of each one — file, **kind** (`volume`/`surface`), template space, LUT, licence, required attribution, whether it may be redistributed, and what to cite — and `GET /api/catalog/atlases` serves the MNI list from it with `kind` on every row (subject-space atlases carry the same field, still detected from the extension: `.annot` is a surface, `.nii`/`.mgz` a volume). Cortical mode therefore never sees a volume and Subcortical never sees a surface. All four shipped MNI atlases are volumes, **including Glasser** — a cortical parcellation distributed as a NIfTI, which is why it is targeted through Subcortical; the picker now says that instead of "TI-Toolbox ships no MNI-space cortical parcellation".
- 2026-09-17: **An MNI atlas label is transformed into your subject before anything else touches it** — flex-search passed SimNIBS the whole multi-label MNI atlas with `mask_space="mni"` and let it warp internally, so the detached-island cleanup ran on the atlas in *MNI* voxels rather than on the mask the search would use, and the ROI plate — which documents itself as showing "the same `prepare_mask` call the runner makes" — transformed something else. Now the label is binarised, warped once into subject space with the registration in `m2m_<id>/toMNI/`, cleaned of islands there, and handed on as the same subject-space NIfTI mask a subject-space target produces. The transform is the only thing that differs between an MNI target and a subject one. Verified on `ernie`: the ROI plate's 124 241 voxels are now the search's own.
- 2026-09-17: **`RuntimeWarning: invalid value encountered in dot` no longer looks like a failed registration** — every MNI transform printed it, from `simnibs/utils/transformations.py:147`, and it was read as the cause when a run went wrong. It is noise: SimNIBS samples the deformation field with `cval=np.inf`, so target voxels outside the warp's field of view (the corners of every subject's T1 grid) arrive as infinity and become NaN in the next matrix multiply, then sample as background — which is what they should be. It is suppressed at the one place TI-Toolbox makes that call, and the *result* is checked instead.
- 2026-09-17: **The MNI atlases' licences and citations are on the wiki** — [Brain Atlases]({{ site.baseurl }}/wiki/atlases/) now carries a licence table (licence, may-it-be-redistributed, what to cite) for all four shipped atlases and the MNI152 template, the HCP acknowledgement Glasser requires, and the measured accuracy of the MNI-to-subject transform on `ernie`: 1.05 mm (CIT168 putamen), 2.20 mm (CIT168 caudate), 2.00 / 1.19 mm (MASSP thalamus left/right) against `charm`'s own segmentation of the same structures. It also states plainly that the Morel thalamus atlas is CC BY-NC-SA, cannot be redistributed by a GPL-3 project, and will move to an optional download before the next release — it is still shipped and still selectable today so no existing configuration breaks.
- 2026-09-17: **The analyzer's sphere and cortical analyses leave the ROI plate too** — only the mask analysis did. A sphere or a cortical region, in mesh or voxel space, is now put on the subject's T1 grid before it is measured (a voxel mask by nearest-neighbour resampling, surface nodes by one-voxel growth kept inside grey matter, the same rasterisation the search's cortical targets use) and confirmed as one plate with one colour and one count per region; voxel analyses add `roi_field_plate.png` at the end, mesh analyses cannot (a mesh field has no voxels to mask).
- 2026-09-17: **Cortical targets get their ROI plate, and a union of regions is one plate** — a flex search on a FreeSurfer `.annot` region failed its confirmation with *Cannot work out file type of lh.101_DK40.annot*, because the plate only knew volume masks; the region's vertices on the hemisphere's central surface are now rasterised onto the T1 grid, grown one voxel and kept where `final_tissues` is grey matter, so the plate shows the same cortex the search evaluates (its voxel count is indicative). And several regions no longer leave `roi_2/`, `roi_3/` directories: the search treats them as one union target, so the confirmation is one mask (one value per region), one plate — one row when the union is compact, one row per region when it spans more than 60 mm, as for bilateral precentral — and one sidecar with `voxels_by_region`.
...
- 2026-09-17: **The e2e gate is green again; eight specs described the previous UI** — `smoke` still enforced the bridge-key list from before the TetraVox detect-or-download surface (ADR row 14 now records the budget as 22 with the reason); `settings` looked for the old inline native-viewer status block rather than the Settings ▸ Viewer TetraVox card; `results` ×2 and `results-panes` named the candidate table's columns and section id from before the evaluated-candidates browser (`ROI mean (V/m)`, `results-candidates`); `optimizer-candidates` compared the replayed draft against a fixture that predated the opt-in `map_to_mni` field the real server records; `page-memory` waited for a Results deep link into the Viewer page that "Open in viewer" no longer takes (it launches native TetraVox and stays on Results, so the test now asserts the retained Viewer page survives that launch); `segmented-idiom` wanted a goal named "Focality" and the old threshold-mode labels, and — hidden behind that first failure — still asserted the retired "Allow unsafe overrides" gate on **Replace and rerun**. Spec updates throughout, plus one `data-testid` on the candidates section and the fixture field; no assertion was weakened and no test removed.
...
- **TetraVox detect-or-setup** — each desktop launch checks a path you located in Settings, conventional system locations, PATH and then the TI-local copy. When none exists on a supported package target, TI performs one verified per-user initial setup shared by startup, Retry and Results. TetraVox owns later updates; TI has no update controls, version pin or pruning of a working installation.
...
- **Native TetraVox viewing** — full scenes, volume and target previews open a discovered or TI-local native viewer in its own window. **Settings → Viewer** reports setup and can locate an existing application; the browser embed and container bundle updater are removed, while run-page surface panes retain their existing renderer. Capable native builds save their live camera and layer edits into the project. Browser sessions can download scenes for manual opening.
