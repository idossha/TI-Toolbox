...
- 2026-09-18: **FastSurfer recovers from a disconnected Apple GPU session** — a stale desktop heartbeat now falls back to container CPU execution with a log message. A host that disappears during a running job still fails that job instead of restarting it.
