...
- 2026-09-17: **Reciprocity search — the montage, straight out of the leadfield** — a third optimizer beside Flex and Ex that needs no FEM solve and no sweep. By the reciprocity theorem an electrode's leadfield column at your target *is* the scalp potential a dipole there would produce at that electrode, so every electrode pair on the net is ranked in one vectorised difference; only the best-ranked pairs are combined into montages and scored with the same verified TI envelope the other optimizers use (2 or 4 channels; 3 is refused, the envelope being defined for an even number of channels). The target is a point (with an optional radius) or the same ROI object Flex takes — a sphere or an atlas region — and the score reads the **mean** field over the ROI, not one centroid element. A run writes `summary.json`, `candidates.csv`, `reciprocity_scores.csv`, `montage.json`, three figures, and the `run_config.json`/`final_output.csv` pair an ex run writes, so it lists in Results and replays into the Simulator exactly like one. Verified on ernie's thalamus (EEG10-10, both thalami, 1 mA per channel) in the container: 2 850 pairs ranked in **21 ms**, 580 montages scored in 11.2 s, best Fz–I1 / F2–Iz at **246.7 mV/m** ROI mean — matching the exhaustive optimum over the same leadfield — with the whole job, leadfield read included, taking 36 s. Focality at weight 0.5 picks F1–I1 / F2–PO10 (235.1 mV/m, background p95 260.5), and the four-channel mTI run scores 150 montages in 24.9 s (339.1 mV/m). See [Reciprocity Search]({{ site.baseurl }}/wiki/reciprocity-search/).
