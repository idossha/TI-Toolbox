...
| **No `idossha/ti-toolbox:v3.0.0` image is published.** `docker-compose.yml` and `loader.py` resolve a tag that Docker Hub does not serve, so a clean clone cannot start v3 without building the image locally (`container/blueprint/build.sh`) | The image is built and pushed by `release-v3.yml`, which has never run (row above). Nothing here is a code gap; the artifact simply does not exist yet. |
| **The versions are not in lockstep.** `version.py` and `tit/__init__.py` say `2.5.0` (from the v2.5.0 merge); `desktop/package.json` says `3.0.0-dev`. `agent-plugin`'s `get_toolbox_version` reports `in_lockstep: false` | Deliberately not fixed by a lane: the bump is the maintainer's release action. `python3 dev/update/update_version.py --version 3.0.0` covers every site in one pass — verified by `--dry-run`: `version.py`, `tit/__init__.py`, `desktop/package.json`, `docker-compose.yml`, `CITATION.cff` and the dataset descriptions. |
| **A packaged app's first launch has never been exercised end to end.** Only macOS arm64 `--dir` was built and `verify:package` run against it; nobody has opened the produced `.app` on a machine that has never run the toolbox | It needs a signed build (never exercised on this branch) and a clean machine. `verify-package.mjs` covers the `compose-invalid` class of failure that killed every packaged build once, but it reads the bundle rather than launching it. |
...
| **Tetravox PR #35 is at 0.4.0 / protocol 3 and is not released.** The dev image already carries that embed, and `Settings ▸ Viewer engine` installs from the GitHub Releases index of `idossha/tetravox` | Nothing in this repository can close it: the release is cut in the Tetravox repository. Until it is, the auto-update check finds no release carrying `tetravox-embed-0.4.0.tgz` and a user's app stays on the bundle its image shipped — which works, but means the "update the viewer without updating the toolbox" promise is untested end to end against a real index. **Release it.** |
