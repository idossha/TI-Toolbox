...
| **The per-point follow-ups carried from the parked embed** — a per-point `radiusPx`, per-point opacity and a marker API — remain wanted by the native renderer even though the embed that would have supplied them is parked | The native pane draws its own dots, so these are now this project's to implement rather than Tetravox's to expose | `.../v3-tetravox-selection-pipeline/{TX,EL}.md`, ROADMAP row above |
